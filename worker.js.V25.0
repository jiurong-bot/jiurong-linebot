// worker.js - V25.0 (DLQ、Graceful Shutdown 與健康檢查)
...
/**
 * [V25.0 修改] 將徹底失敗的任務移至 Dead Letter Queue (failed_tasks)
...
async function moveTaskToDLQ(task, errorMessage) {
    ...
    try {
        ...
        // 複製原始任務資料到 failed_tasks
        await db.query(
            `INSERT INTO failed_tasks (id, recipient_id, message_payload, status, send_at, retry_count, last_error, created_at, updated_at)
             VALUES ($1, $2, $3, 'failed', $4, $5, $6, $7, NOW())`,
            [task.id, task.recipient_id, task.message_payload, task.send_at, task.retry_count, errorMessage, task.created_at]
        );
        // 從主任務表刪除
