// index.js - V12.0 (實作課程取消功能 & 優化新增流程)
      ...
      case 'batch':
        if (text === COMMANDS.TEACHER.CONFIRM_BATCH_CANCEL) {
          ...
          try {
            ...
            const coursesToCancelRes = await client.query("SELECT * FROM courses WHERE id LIKE $1 AND time > NOW() FOR UPDATE", [`${state.prefix}%`]);
            ...
            await client.query("DELETE FROM courses WHERE id LIKE $1 AND time > NOW()", [`${state.prefix}%`]);
