...
const COMMANDS = {
  ...
  ADMIN: {
    ...
    // [修改] CANCEL_ADD_TEACHER 使用通用取消指令
    CONFIRM_REMOVE_TEACHER: '✅ 確認移除',
    // [修改] CANCEL_REMOVE_TEACHER 使用通用取消指令
  },
  TEACHER: {
    ...
    // [修改] CANCEL_MANUAL_ADJUST 使用通用取消指令
  ...
  STUDENT: {
    ...
    SHOP: '@活動商城',
    // [修改] CANCEL_PURCHASE 使用通用取消指令
    // [修改] CANCEL_INPUT_LAST5 使用通用取消指令
    BOOK_COURSE: '@預約課程',
    MY_COURSES: '@我的課程',
    CANCEL_BOOKING: '@取消預約',
    CANCEL_WAITING: '@取消候補',
...
function getDateRange(period) {
    ...
    // Set time to start and end of day in Taipei time
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingCourseCancellation[userId]) {
    ...
    if (text === COMMANDS.GENERAL.CANCEL) { // [修改]
    ...
    switch (state.step) {
        ...
        case 'await_time':
            if (!/^\d{2}:\d{2}$/.test(text)) {
                return reply(replyToken, '時間格式不正確，請輸入四位數時間，例如：19:30', getCancelMenu()); // [修改]
            }
            state.time = text;
            state.step = 'await_sessions';
            return reply(replyToken, '請問這個系列總共要開設幾堂課？（請輸入數字）', getCancelMenu()); // [修改]
       case 'await_sessions':
            const sessions = parseInt(text, 10);
            if (isNaN(sessions) || sessions <= 0) {
                return reply(replyToken, '堂數必須是正整數，請重新輸入。', getCancelMenu()); // [修改]
            }
            state.sessions = sessions;
            state.step = 'await_capacity';
            return reply(replyToken, '請問每堂課的名額限制？（請輸入數字）', getCancelMenu()); // [修改]
        case 'await_capacity':
            const capacity = parseInt(text, 10);
            if (isNaN(capacity) || capacity <= 0) {
                return reply(replyToken, '名額必須是正整數，請重新輸入。', getCancelMenu()); // [修改]
            }
            state.capacity = capacity;
            state.step = 'await_points';
            return reply(replyToken, '請問每堂課需要消耗多少點數？（請輸入數字）', getCancelMenu()); // [修改]
        case 'await_points':
            const points = parseInt(text, 10);
            if (isNaN(points) || points < 0) {
                return reply(replyToken, '點數必須是正整數或 0，請重新輸入。', getCancelMenu()); // [修改]
            }
            ...
            return reply(replyToken, summary, [
                ...
                { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } // [修改]
            ]);
        case 'await_confirmation':
            if (text === '✅ 確認新增') {
                const client = await pgPool.connect();
    ...
    switch (state.step) {
      case 'await_student_search':
        ...
        if (res.rows.length === 0) {
          return reply(replyToken, `找不到學員「${text}」。請重新輸入或取消操作。`, getCancelMenu()); // [修改]
        }
        const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
      ...
      case 'await_operation':
        if (text === COMMANDS.TEACHER.ADD_POINTS || text === COMMANDS.TEACHER.DEDUCT_POINTS) {
          ...
          return reply(replyToken, `請輸入要 ${text === COMMANDS.TEACHER.ADD_POINTS ? '增加' : '扣除'} 的點數數量 (純數字)：`, getCancelMenu()); // [修改]
        } else {
      ...
      case 'await_amount':
        ...
        if (isNaN(amount) || amount <= 0) { return reply(replyToken, '點數格式不正確，請輸入一個大於 0 的正整數。', getCancelMenu()); } // [修改]
        state.amount = amount;
        state.step = 'await_reason';
        return reply(replyToken, '請輸入調整原因（例如：活動獎勵、課程補償等）：', getCancelMenu()); // [修改]
      ...
      case 'await_reason':
        ...
        return reply(replyToken, summary, [ { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST, text: COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ]); // [修改]
    ...
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
        ...
        return reply(replyToken, '請輸入您想調整點數的學員姓名或 User ID：', getCancelMenu()); // [修改]
        ...
        return reply(replyToken, '請輸入您想查詢的學員姓名或 User ID：', getCancelMenu()); // [修改]
...
async function handleStudentCommands(event, userId) {
  ...
  if (pendingBookingConfirmation[userId]) {
    ...
    if (text === COMMANDS.STUDENT.LATEST_ANNOUNCEMENT) {
      ...
      return reply(replyToken, '請輸入您想對老師說的話，或點選「取消」。', getCancelMenu()); // [修改]
      ...
      return reply(replyToken, flexMenu);
        ...
        try {
            ...
            if (orderRes.rows.length > 0) {
                ...
                return reply(replyToken, '請輸入您的匯款帳號後五碼 (5位數字)：', getCancelMenu()); // [修改]
            } else {
    ...
    } else if (text === COMMANDS.GENERAL.CANCEL) { // [修改]
      const purchaseState = pendingPurchase[userId];
      if (purchaseState) {
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V17.0 (智慧取消機制優化)`); // [修改]
...
async function handleEvent(event) {
    ...
    // [修改] 智慧取消機制的核心
    // 在處理任何新事件之前，先檢查並清除使用者可能存在的待辦任務。
    if (event.type === 'message' || event.type === 'postback') {
        // 例外：如果使用者正在輸入通用取消指令，則不清除狀態，讓對應的 handler 處理。
        if (!(event.type === 'message' && event.message.text === COMMANDS.GENERAL.CANCEL)) {
            const cleared = clearPendingConversations(userId);
            if (cleared) {
                 // 可以選擇性地通知使用者舊任務已取消，但為了流暢性，通常不需通知。
