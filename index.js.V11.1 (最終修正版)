...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingManualAdjust[userId]) {
    ...
    // For now, the entry point is what matters.
  }
      ...
      switch (state.step) {
          ...
          case 'await_image':
              ...
              if (event.message.type === 'image') {
                  // In a real scenario, you'd upload this image and get a URL.
                  // For this example, we'll use a placeholder.
                  // You would need to handle the image stream from the event.
  ...
  if (text === COMMANDS.TEACHER.STUDENT_MANAGEMENT || text === COMMANDS.TEACHER.POINT_MANAGEMENT) {
      const studentManagementMenu = [
        { type: 'action', action: { type: 'message', label: '查詢特定學員', text: COMMANDS.TEACHER.SEARCH_STUDENT } },
        { type: 'action', action: { type: 'message', label: '手動調整點數', text: COMMANDS.TEACHER.MANUAL_ADJUST_POINTS } },
        { type: 'action', action: { type: 'message', label: '查看學員留言', text: COMMANDS.TEACHER.VIEW_MESSAGES } },
        { type: 'action', action: { type: 'message', label: '查詢留言內容', text: COMMANDS.TEACHER.MESSAGE_SEARCH } }
      ];
  ...
  if (text === COMMANDS.TEACHER.VIEW_MESSAGES) {
      reply(replyToken, '正在查詢最新留言...').catch(e => console.error(e));
      (async () => {
        try {
            const messagesRes = await pgPool.query("SELECT * FROM feedback_messages WHERE status <> 'replied' ORDER BY timestamp DESC LIMIT 10");
            const newMessages = messagesRes.rows;
            if (newMessages.length === 0) {
              return push(userId, '目前沒有未回覆的學員留言。');
            }
            const messageBubbles = newMessages.map(msg => {
            ...
            await push(userId, [{ type: 'text', text: '以下是尚未回覆的學員留言：' }, { type: 'flex', altText: '學員留言列表', contents: { type: 'carousel', contents: messageBubbles } }]);
        } catch(err) {
            console.error('❌ 查詢留言時發生錯誤:', err);
            await push(userId, '查詢留言時發生錯誤，請稍後再試。');
        }
  ...
  if (text === COMMANDS.TEACHER.VIEW_PRODUCTS) {
      ...
      const productBubbles = productsRes.rows.map(p => ({
          ...
          body: { type: 'box', layout: 'vertical', contents: [
              ...
              { type: 'text', text: `點數：${p.price} 點`, size: 'lg', margin: 'md' },
...
async function handleStudentCommands(event, userId) {
  ...
  if (pendingFeedback[userId]) {
    ...
    if (feedbackState.step === 'await_message') {
      const messageId = `F${Date.now()}`;
      await pgPool.query(
        'INSERT INTO feedback_messages (id, user_id, user_name, message, timestamp) VALUES ($1, $2, $3, $4, NOW())',
        [messageId, userId, user.name, text]
      );
      delete pendingFeedback[userId];
      if (TEACHER_ID) {
          push(TEACHER_ID, `🔔 新留言通知\n來自: ${user.name}\n內容: ${text}\n\n請至「學員管理」->「查看學員留言」回覆。`).catch(e => console.error(e));
      }
      return reply(replyToken, '感謝您的留言，我們已收到您的訊息，老師會盡快查看！');
    }
  }
  ...
  if (text === COMMANDS.STUDENT.BUY_POINTS) {
      ...
      if (existingOrderRes.rows.length > 0) {
          await reply(replyToken, '您目前尚有未完成的訂單，請先至「點數管理」完成或取消該筆訂單。').catch(e => console.error(e));
          await pushPointsMenu(userId);
          return;
      }
  ...
  if (text === COMMANDS.STUDENT.PURCHASE_HISTORY) {
      reply(replyToken, '正在查詢您的購點紀錄...').catch(e => console.error(e));
      (async () => {
          try {
              ...
              await push(userId, [{ type: 'text', text: '以下是您近期的購點紀錄：' }, { type: 'flex', altText: '購點紀錄', contents: { type: 'carousel', contents: historyBubbles } }]);
          } catch(err) {
              console.error('❌ 查詢購點紀錄失敗:', err);
              await push(userId, '查詢購點紀錄時發生錯誤。');
          }
      })();
      return;
  }
  ...
  if (text === COMMANDS.STUDENT.BOOK_COURSE) {
      reply(replyToken, '正在查詢可預約的課程...').catch(e => console.error(e));
      (async () => {
          try {
              const allCourses = Object.values(await getAllCourses());
              const now = Date.now();
              ...
              await push(userId, { type: 'flex', altText: '可預約的課程列表', contents: { type: 'carousel', contents: courseBubbles } });
          } catch(err) {
  ...
  if (text === COMMANDS.STUDENT.MY_COURSES) {
      reply(replyToken, '正在查詢您的課程...').catch(e => console.error(e));
      (async () => {
          try {
              const allCourses = Object.values(await getAllCourses());
              const now = Date.now();
              ...
              if (myCourses.length === 0) {
                  return push(userId, '您目前沒有任何已預約或候補中的課程。');
              }
              ...
              await push(userId, { type: 'flex', altText: '我的課程列表', contents: { type: 'carousel', contents: courseBubbles } });
          } catch(err) {
  ...
  if (text === COMMANDS.STUDENT.SHOP) {
      ...
      const productBubbles = productsRes.rows.map(p => ({
          ...
          body: { type: 'box', layout: 'vertical', contents: [
              { type: 'text', text: p.name, weight: 'bold', size: 'xl' },
              { type: 'text', text: `${p.price} 點`, size: 'lg', margin: 'md', color: '#1A759F', weight: 'bold' },
              { type: 'text', text: p.description, wrap: true, size: 'sm', margin: 'md' },
          ]},
          footer: { type: 'box', layout: 'vertical', contents: [
  ...
  if (text === COMMANDS.STUDENT.INPUT_LAST5_CARD_TRIGGER || text === COMMANDS.STUDENT.EDIT_LAST5_CARD_TRIGGER) {
    // Input Last 5 logic - This should be handled via postback or a more specific flow.
