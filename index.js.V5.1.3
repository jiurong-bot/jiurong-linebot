    ...
    for (const prefix of sortedPrefixes) {
        ...
        courseBubbles.push({
            ...
            body: {
                ...
                contents: [
                    { type: 'text', text: courseMainTitle, weight: 'bold', size: 'xl', wrap: true }, // 使用修改後的課程名稱
  ...
  switch (stepData.step) {
    case 'input_last5':
      ...
      try {
        ...
        // [MOD-V5.1.2] 優化：通知老師有新訂單待確認時包含用戶名和訂單ID
        if (TEACHER_ID) {
        ...
        if (user.role === 'teacher') {
            ...
            if (action === 'add_course_start') {
                ...
                try {
                    ...
                    // [FIX-V5.1.2] 修正：確保 orderInTransaction.timestamp 轉換為有效的 ISO 格式
                    const updatedOrder = {
                        ...
                        timestamp: new Date(orderInTransaction.timestamp).toISOString() // 確保時間戳是ISO格式
                    };
