...
async function handleTeacherCommands(event, userId, userState) {
  ...
  if (userState) {
        ...
        switch (stateData.step) {
          case 'await_student_search':
            ...
            if (res.rows.length === 0) {
              return push(userId, { type: 'text', text: `找不到學員「${text}」。請重新輸入或取消操作。`, quickReply: { items: [{ type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST, text: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST } }] }});
            }
    ...
    if (unhandledStates.includes(stateName)) {
        await clearUserState(userId);
        return push(userId, `功能尚未開放，已為您取消目前操作。`);
    }
    return;
  }
...
async function handleStudentCommands(event, userId, userState) {
      ...
      switch (stateData.type) {
        case 'book':
            if (text === COMMANDS.STUDENT.CONFIRM_BOOKING) {
                ...
                try {
                    await clientDB.query('BEGIN');
                    const userForUpdate = await clientDB.query('SELECT points FROM users WHERE id = $1 FOR UPDATE', [userId]);
  ...
  if (text === COMMANDS.STUDENT.POINTS || text === COMMANDS.STUDENT.CHECK_POINTS || text === COMMANDS.STUDENT.RETURN_POINTS_MENU) {
    ...
    await push(userId, '正在查詢可預約的課程，請稍候...');
    try {
        const allCourses = Object.values(await getAllCourses());
        const now = Date.now();
        ...
        if (user.role === 'student') {
            if (action === 'select_purchase_plan') {
                ...
                if (selectedPlan) {
                    ...
                    return push(userId, { type: 'text', text: `您選擇了「${selectedPlan.label}」。\n請確認是否購買？`, quickReply });
                }
            }
            if (action === 'confirm_booking_start') {
                const courseId = data.get('courseId');
                const course = await getCourse(courseId);
                if (!course) { return push(userId, '抱歉，找不到該課程。'); }
                await setUserState(userId, 'booking_confirmation', { type: 'book', courseId: courseId });
                const message = `您確定要預約以下課程嗎？\n\n課程：${course.title}\n時間：${formatDateTime(course.time)}\n費用：${course.pointsCost} 點\n\n預約後將立即扣點，確認嗎？`;
                const quickReply = { items: [ { type: 'action', action: { type: 'message', label: COMMANDS.STUDENT.CONFIRM_BOOKING, text: COMMANDS.STUDENT.CONFIRM_BOOKING } }, { type: 'action', action: { type: 'message', label: COMMANDS.STUDENT.ABANDON_BOOKING, text: COMMANDS.STUDENT.ABANDON_BOOKING } } ] };
                return push(userId, { type: 'text', text: message, quickReply });
            }
        }
    }
}
...
app.get('/', (req, res) => res.send('九容瑜伽 LINE Bot (V18.0 最終穩定版) 正常運作中。'));

// --- 伺服器啟動 ---
app.listen(PORT, async () => {
  await initializeDatabase();
  console.log(`✅ 伺服器已啟動，監聽埠號 ${PORT}`);
  console.log(`Bot 版本: V18.0 (最終穩定版)`);
