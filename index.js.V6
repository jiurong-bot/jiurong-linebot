...
async function handleTeacherCommands(event, userId) {
  ...
  if (text === COMMANDS.TEACHER.VIEW_MESSAGES) {
      const messagesRes = await pgPool.query("SELECT * FROM feedback_messages WHERE status = 'new' ORDER BY timestamp ASC LIMIT 10");
      ...
      const messageBubbles = messagesRes.rows.map(msg => ({
          type: 'bubble',
          header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: `來自 ${msg.user_name} 的留言`, color: '#ffffff', weight: 'bold' }], backgroundColor: '#6a7d8b', paddingAll: 'lg' },
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V6 (新增學員留言功能)`);
