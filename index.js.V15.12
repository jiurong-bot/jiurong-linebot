...
// --- 原生 JavaScript 簡易快取系統 ---
const simpleCache = {
  storage: new Map(),
  // ttl_seconds: Time To Live in seconds
  set: function(key, value, ttl_seconds = 300) {
  ...
  get: function(key) {
    ...
    if (Date.now() > entry.expiresAt) {
      this.storage.delete(key); // 已過期，刪除並回傳 null
      return null;
    }
    return entry.value; // 未過期，回傳值
  },
  del: function(keys) {
...
async function initializeDatabase() {
  ...
  try {
    ...
    await client.query('CREATE INDEX IF NOT EXISTS idx_feedback_messages_status ON feedback_messages(status)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_products_status ON products(status)');
    console.log('✅ 已檢查/建立資料庫索引');
  ...
  if (pendingCourseCancellation[userId]) {
        ...
        if (text === COMMANDS.TEACHER.CONFIRM_BATCH_CANCEL) {
          ...
          (async () => {
            ...
            try {
              ...
              simpleCache.del(['cache_teacher_courses', 'cache_student_book_courses']);
    ...
    switch (state.step) {
      case 'await_student_search':
        reply(replyToken, `正在查詢學員「${text}」...`).catch(e => console.error(e));
        (async () => {
          try {
            ...
            if (res.rows.length === 0) {
              await push(userId, `找不到學員「${text}」。請重新輸入或取消操作。`);
              return;
            }
            ...
            await push(userId, { type: 'flex', altText: '請選擇要調整點數的學員', contents: { type: 'carousel', contents: userBubbles } });
          } catch(e) {
              console.error('手動調整點數-查詢學員失敗:', e);
              await push(userId, '查詢學員時發生錯誤。');
          }
        })();
        return;
    ...
    reply(replyToken, `正在查詢學員「${searchQuery}」的資料...`).catch(e => console.error(e));
    (async () => {
      try {
          ...
          if (res.rows.length === 0) {
              await push(userId, `找不到符合「${searchQuery}」的學員。`);
              return;
          }
          ...
          await push(userId, { type: 'flex', altText: '學員查詢結果', contents: { type: 'carousel', contents: userBubbles } });
      } catch (err) {
    ...
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
        reply(replyToken, '正在查詢課程系列，請稍候...').catch(e => console.error(e));
        (async () => {
          try {
              const cacheKey = 'cache_teacher_courses';
              let cachedData = simpleCache.get(cacheKey);

              if (!cachedData) {
                  const client = await pgPool.connect();
                  try {
                      const courseRes = await client.query('SELECT * FROM courses WHERE time > NOW() ORDER BY time ASC');
                      cachedData = courseRes.rows;
                      simpleCache.set(cacheKey, cachedData);
                  } finally {
                      if(client) client.release();
                  }
              }
              ...
              const courseGroups = {};
              for (const course of cachedData) {
              ...
              await push(userId, [{ type: 'text', text: introText }, { type: 'flex', altText: '課程管理選單', contents: { type: 'carousel', contents: courseBubbles.slice(0, 10) } }]);
          } catch (err) {
  ...
  if (pendingBookingConfirmation[userId]) {
    ...
    if (text === COMMANDS.STUDENT.LATEST_ANNOUNCEMENT) {
      ...
      return reply(replyToken, flexMenu);
        ...
        reply(replyToken, '正在查詢 7 天內可預約的課程，請稍候...').catch(e => console.error(e));
        (async () => {
          try {
              const cacheKey = 'cache_student_book_courses';
              let availableCourses = simpleCache.get(cacheKey);
              ...
              if (!availableCourses) {
                  ...
                  try {
                      ...
                      simpleCache.set(cacheKey, availableCourses);
                  } finally {
                      if(client) client.release();
                  }
              }
              
              const userBookedOrWaitingCoursesRes = await pgPool.query(`SELECT id FROM courses WHERE $1 = ANY(students) OR $1 = ANY(waiting)`, [userId]);
              const userBookedOrWaitingCourseIds = new Set(userBookedOrWaitingCoursesRes.rows.map(r => r.id));
              const finalCourses = availableCourses.filter(c => !userBookedOrWaitingCourseIds.has(c.id));

              if (finalCourses.length === 0) {
                  await push(userId, '抱歉，未來 7 天內沒有可預約的課程。\n您可至「我的課程」查看候補中的課程，或等候老師發布新課程。');
                  return;
              }

              const courseItems = finalCourses.slice(0, 10).map(c => {
              ...
              await push(userId, '查詢課程時發生錯誤，請稍後再試。');
          }
        })();
        return;
        ...
        reply(replyToken, '正在讀取商城商品，請稍候...').catch(e => console.error(e));
        (async () => {
          try {
              const cacheKey = 'cache_shop_products';
              let products = simpleCache.get(cacheKey);
              ...
              if (!products) {
                  ...
                  try {
                      ...
                      simpleCache.set(cacheKey, products);
                  } finally {
                      if(client) client.release();
                  }
              }
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V15.12 (原生快取與索引優化)`);
          ...
          if (action === 'manage_course_group') {
           ...
           if (action === 'cancel_course_group_confirm') {
            ...
            if (action === 'view_student_details') {
                reply(replyToken, '收到指令，正在查詢學員詳細資料，請稍候...').catch(e => console.error("立即回覆失敗:", e));
                (async () => {
                    const studentId = data.get('studentId');
                    const student = await getUser(studentId);
                    if (!student) {
                        await push(userId, '抱歉，查詢失敗，找不到該學員的資料。');
                        return;
                    }
                    const client = await pgPool.connect();
                    try {
                        ...
                        await push(userId, fullReport);
                    } catch (err) {
                        console.error('❌ 查詢學員詳細資料失敗:', err);
                        await push(userId, '查詢學員詳細資料時發生錯誤。');
                    } finally {
                        if (client) client.release();
                    }
                })();
