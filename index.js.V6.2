...
const FEEDBACK_TIMEOUT_MS = 1000 * 60;
...
async function initializeDatabase() {
  try {
    ...
    // V6.2: 修改 feedback_messages 資料表，新增 teacher_reply 欄位
...
const pendingReply = {};
const pendingMessageSearchQuery = {}; // V6.2 新增
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingReply[userId]) {
    ...
    // 推播訊息給學員
    push(replyData.targetUserId, `老師回覆您在「聯絡我們」的留言：\n\n${replyMessage}`).catch(e => console.error(e));

    // V6.2: 更新資料庫，儲存老師的回覆內容
    await pgPool.query("UPDATE feedback_messages SET status = 'replied', teacher_reply = $1 WHERE id = $2", [replyMessage, replyData.msgId]);
...
async function handleStudentCommands(event, userId) {
  ...
  if (text === COMMANDS.STUDENT.CONTACT_US) {
    ...
    const timeoutId = setTimeout(() => {
        if (pendingFeedback[userId]) {
            delete pendingFeedback[userId];
            push(userId, '留言已逾時，自動取消。若要再次留言請重新點選。').catch(e => console.error(e));
        }
    }, FEEDBACK_TIMEOUT_MS);
    pendingFeedback[userId] = { timestamp: Date.now(), timeoutId: timeoutId };
  ...
  if (text === COMMANDS.STUDENT.BUY_POINTS) {
    ...
    else { pendingPurchase[userId] = { step: 'select_plan', data: {} }; return reply(replyToken, [{ type: 'text', text: '請選擇要購買的點數方案：' }, buildBuyPointsFlex()]); }
  }
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V6.2 (新增留言查詢功能)`);
...
async function handleEvent(event) {
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        // V6.2: 處理留言查詢 postback
        if (action === 'start_message_search') { pendingMessageSearchQuery[userId] = true; return reply(replyToken, '請輸入學員姓名或留言內容進行查詢（支援模糊篩選）：', null); }

        if (action === 'start_student_search') { pendingStudentSearchQuery[userId] = true; return reply(replyToken, '請輸入要查詢的學員姓名或 ID（支援模糊篩選）：', null); }
            ...
            const timeoutId = setTimeout(() => {
                if (pendingReply[userId]) {
                    delete pendingReply[userId];
                    push(userId, '回覆留言逾時，自動取消。請重新點擊回覆。').catch(e => console.error(e));
                }
            }, FEEDBACK_TIMEOUT_MS);
            pendingReply[userId] = { msgId: msgId, targetUserId: targetUserId, timeoutId: timeoutId };
        ...
        if (user.role === 'teacher') {
            ...
            if (action === 'add_course_start') { pendingCourseCreation[userId] = { step: 1 }; await reply(replyToken, '請輸入課程名稱（例如：哈達瑜伽）：', [{ type: 'message', label: COMMANDS.STUDENT.CANCEL_ADD_COURSE, text: COMMANDS.STUDENT.CANCEL_ADD_COURSE }]); return; }
