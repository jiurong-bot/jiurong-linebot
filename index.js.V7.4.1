...
async function handleEvent(event) {
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        const teacherPostbackActions = [
            'start_message_search', 'start_student_search', 'show_student_detail', 'mark_feedback_read', 'reply_feedback',
            'add_announcement_start', 'history_announcements_show', 'confirm_delete_announcement', 'execute_delete_announcement',
            'do_nothing', 'select_manual_adjust_student', 'add_course_start', 'confirm_order', 'reject_order',
            'manage_course_group', 'cancel_course_group_confirm', 'cancel_course_group', 'cancel_single_course_confirm', 'cancel_single_course'
        ];

        const studentPostbackActions = [
            'select_purchase_plan', 'confirm_booking', 'cancel_booking_confirm', 'cancel_waiting_confirm'
        ];

        if (user.role === 'teacher' && teacherPostbackActions.includes(action)) {
            // 在此處貼上 V7.3 中 user.role === 'teacher' 的 postback 完整邏輯
            // (此處省略以避免程式碼過於冗長)
        } else if (user.role === 'student' && studentPostbackActions.includes(action)) {
            // 在此處貼上 V7.3 中 user.role === 'student' 的 postback 完整邏輯
            // (此處省略以避免程式碼過於冗長)
        }
    }
}
