...
// --- No pending conversation, handle new commands ---
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
            ...
            contents: {
                ...
                body: {
                    ...
                    contents: [
                        ...
                        // { type: 'button', style: 'link', height: 'sm', action: { type: 'postback', label: '自訂', data: `action=generate_report&type=${reportType}&period=custom` } }
                    ]
                },
                 footer: {
                    type: 'box',
        ...
        reply(replyToken, '📊 收到指令，正在為您生成點數分佈報告，請稍候...');
        (async () => {
            const client = await pgPool.connect();
            try {
...
async function handleEvent(event) {
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (action.startsWith('generate_report')) {
            ...
            reply(replyToken, `收到指令，正在為您生成 ${title}，請稍候...`);
