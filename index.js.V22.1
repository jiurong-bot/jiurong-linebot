...
// [V22.1 優化] 移除此函式中的「新增」按鈕，讓其專注於顯示已開課程
async function showCourseSeries(replyToken, page = 1) {
    ...
    try {
        ...
        if (pageSeries.length === 0 && page === 1) {
            return reply(replyToken, '目前沒有任何進行中的課程系列。');
        }
        ...
        const message = { type: 'flex', altText: '管理已開課程', contents: { type: 'carousel', contents: courseBubbles } };

        return reply(replyToken, message);

    } catch (err) {
        console.error('❌ 查詢課程系列時發生錯誤:', err);
