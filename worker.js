...
let lastPointsReport = 0;
let lastDataCleanup = 0; 
let lastPointsDetailReport = 0; // [新增] 上次執行點數詳單報表的時間戳
let isAlertCooldown = false; 

// =======================================================
// [新增] 每日學員點數詳單報表
// =======================================================
async function performPointsDetailReport() {
    console.log('📋 開始執行每日學員點數詳單報表任務...');
    ...
    try {
        ...
        let reportText = `您好，\n\n這是截至 ${reportDate} 的所有學員點數詳單：\n\n`;
        
        students.forEach(student => {
            reportText += `- ${student.name}: ${student.points} 點\n`;
        });
        ...
        const mailOptions = {
            ...
            to: MAIL_TO,
            subject: `[自動日報] 全體學員點數詳單 ${reportDate}`,
            text: reportText,
        };

        console.log(`正在將點數詳單寄送至 ${MAIL_TO}...`);
        await transporter.sendMail(mailOptions);
        console.log('✅ 每日學員點數詳單已成功寄出！');

    } catch (error) {
        console.error('❌ 在產生點數詳單過程中發生嚴重錯誤:', error);
    } finally {
        db.release();
    }
}
...
async function performPointsReport() {
    console.log('📈 開始執行每日學員點數統計報表任務...');
    if (!MAIL_FROM || !GMAIL_APP_PASSWORD) {
        console.error('❌ 報表錯誤：缺少必要的 Gmail 環境變數。');
        return;
    }
...
這是 ${reportDate} 的學員點數每日統計報告。

--- 概覽 ---
- 總學員數：${students.length} 人
- 點數總流通量：${totalPoints} 點
- 平均持有：${averagePoints} 點/人
- 零點數學員：${zeroPointStudents} 人
...
此為系統自動發送，請勿直接回覆。
        ...
        const mailOptions = {
            from: `"九容瑜伽自動報表" <${MAIL_FROM}>`,
            to: MAIL_TO,
            subject: `[自動日報] 學員點數統計報表 ${reportDate}`,
            text: reportText,
        };

        console.log(`正在將點數日報寄送至 ${MAIL_TO}...`);
        await transporter.sendMail(mailOptions);
        console.log('✅ 點數統計日報已成功寄出！');

    } catch (error) {
        console.error('❌ 在產生點數報表過程中發生嚴重錯誤:', error);
    } finally {
        db.release();
    }
}
...
async function main() {
  console.log('🚀 背景工作程式 (Worker) 已啟動... V37.0 (新增每日點數詳單)');
  while (true) {
    if (isShuttingDown) {
    ...
    if (now - lastPointsReport > POINTS_REPORT_INTERVAL_MS) {
        await performPointsReport();
        lastPointsReport = now;
    }

    // [新增] 定時執行點數詳單報表
    if (now - lastPointsDetailReport > POINTS_REPORT_INTERVAL_MS) {
        await performPointsDetailReport();
        lastPointsDetailReport = now;
    }
