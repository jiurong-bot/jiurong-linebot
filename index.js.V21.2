/*
 * index.js - V21.2 (防競態條件版)
 * =================================================================
 * 功能簡介 (Functional Overview):
 * - 重構 handleAdminCommands 委派邏輯，改為優先判斷「指令意圖」而非「資料庫狀態」，徹底解決快速操作下的競態條件問題。
 * - 為管理員的 postback data 加上 'admin_' 前綴，使其易於識別，增強程式碼的穩固性。
 * - 此版本旨在解決 V21.1 中因時序問題導致的指令處理錯誤。
 *
...
async function handleAdminCommands(event, user, userState) {
  ...
  // --- V21.2 防競態條件委派邏輯 (Anti-Race-Condition Delegation Logic) ---
  const NATIVE_ADMIN_TEXT_COMMANDS = Object.values(COMMANDS.ADMIN);
  const isNativeAdminCommand = NATIVE_ADMIN_TEXT_COMMANDS.includes(text) || postbackAction.startsWith('admin_');

  // 如果指令不是管理員原生指令，且使用者正在模擬其他角色，則委派事件
  if (!isNativeAdminCommand) {
  ...
  // --- 如果執行到這裡，代表是「純粹的管理員操作」或「使用者本來就是管理員角色」---
  ...
  switch(text) {
    ...
    case COMMANDS.ADMIN.REMOVE_TEACHER:
      ...
      // V21.2 修正：在 postback data 中加入 'admin_' 前綴
...
async function handleTeacherCommands(event, user, userState) {
  ...
  if (userState) {
    ...
    if (stateName === 'course_creation') {
        ...
        // Postback for weekday selection
    ...
    if (stateName === 'manual_adjust') {
        ...
        // Postback for student selection
...
async function handleEvent(event) {
    ...
    // --- V21.2 核心路由邏輯 ---
    // 1. 如果是 Admin User，永遠交給 Admin Handler (它內部會做委派)
    if (userId === ADMIN_USER_ID) {
        return handleAdminCommands(event, user, userState);
    }

    // 2. 對於非 Admin User，根據資料庫中的角色分派
    switch (user.role) {
        case 'teacher':
            return handleTeacherCommands(event, user, userState);
        case 'student':
...
app.get('/', (req, res) => res.send('九容瑜伽 LINE Bot (V21.2 防競態條件版) 正常運作中。'));

// --- 伺服器啟動 ---
app.listen(PORT, async () => {
  await initializeDatabase();
  console.log(`✅ 伺服器已啟動，監聽埠號 ${PORT}`);
  console.log(`Bot 版本: V21.2 (防競態條件版)`);
