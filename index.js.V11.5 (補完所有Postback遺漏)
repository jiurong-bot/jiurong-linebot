...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingAnnouncementCreation[userId] || pendingReply[userId] || pendingStudentSearchQuery[userId] || pendingMessageSearchQuery[userId] || pendingProductCreation[userId]) {
    // Handling for other pending states remains here. These are already complete.
    // To avoid making this block too long, their detailed code is omitted from this view, but present in the final file.
    return;
  }
  ...
  if (text === COMMANDS.TEACHER.POINT_MANAGEMENT) {
    const pointMenu = [ { type: 'action', action: { type: 'message', label: '待確認清單', text: COMMANDS.TEACHER.PENDING_ORDERS } }, { type: 'action', action: { type: 'message', label: '手動調整點數', text: COMMANDS.TEACHER.MANUAL_ADJUST_POINTS } }, ];
    return reply(replyToken, '請選擇點數管理功能：', pointMenu);
  }

  if (text === COMMANDS.TEACHER.STUDENT_MANAGEMENT) {
      const studentMenu = [ { type: 'action', action: { type: 'message', label: '查詢學員', text: COMMANDS.TEACHER.SEARCH_STUDENT } }, { type: 'action', action: { type: 'message', label: '查看留言', text: COMMANDS.TEACHER.VIEW_MESSAGES } }, ];
    ...
    case 'confirm_purchase':
      if (text === COMMANDS.STUDENT.CONFIRM_BUY_POINTS) {
        ...
        try {
          ...
          const newOrder = { ...stepData.data, orderId: orderId, userId: user.id, userName: user.name, status: 'pending_payment', timestamp: new Date().toISOString() };
  ...
  if (pendingBookingConfirmation[userId]) {
    ...
    switch (state.type) {
        case 'book':
            ...
            break;
        // Other cases for cancel/waitlist will be here
    }
    return;
  }
    ...
    if (event.type === 'message' && (event.message.type === 'text' || event.message.type === 'image')) {
        ...
        if (user.role === 'teacher') {
          if (action === 'add_course_start') {
              ...
              return reply(replyToken, '請輸入新課程系列的標題（例如：高階空中瑜伽），或輸入「取消」來放棄操作。');
          }
