// index.js - V6.1 (新增學員留言逾時取消與老師回覆功能)
...
async function initializeDatabase() {
  try {
    ...
    await pgPool.query(`CREATE TABLE IF NOT EXISTS feedback_messages (id VARCHAR(255) PRIMARY KEY, user_id VARCHAR(255) NOT NULL, user_name VARCHAR(255) NOT NULL, message TEXT NOT NULL, status VARCHAR(50) DEFAULT 'new', timestamp TIMESTAMPTZ NOT NULL)`);
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingReply[userId]) {
    ...
    // 更新資料庫狀態
    await pgPool.query("UPDATE feedback_messages SET status = 'replied' WHERE id = $1", [replyData.msgId]);
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V6.1 (新增學員留言逾時取消與老師回覆功能)`);
