...
const COMMANDS = {
  ...
  TEACHER: {
    ...
    ANNOUNCEMENT_MANAGEMENT: '@公告管理', // V7 新增
    VIEW_MESSAGES: '@查看留言',
    ADD_COURSE: '@新增課程',
    CANCEL_COURSE: '@取消課程',
    COURSE_LIST: '@課程列表',
    ...
    DEDUCT_POINTS: '- 扣點',
    MESSAGE_SEARCH: '@查詢留言',
    CONFIRM_ADD_ANNOUNCEMENT: '✅ 確認發布', // V7 新增
    CANCEL_ADD_ANNOUNCEMENT: '❌ 取消發布', // V7 新增
    CONFIRM_DELETE_ANNOUNCEMENT: '✅ 確認刪除', // V7 新增
    ABANDON_DELETE_ANNOUNCEMENT: '❌ 放棄刪除', // V7 新增
  },
  STUDENT: {
    POINTS: '@點數管理',
    CHECK_POINTS: '@剩餘點數',
    ...
    CONTACT_US: '@聯絡我們',
    LATEST_ANNOUNCEMENT: '@最新公告', // V7 新增
...
async function initializeDatabase() {
  try {
    ...
    testClient.release();
    // V7: users 表增加 last_seen_announcement_id 欄位
    await pgPool.query(`CREATE TABLE IF NOT EXISTS users (id VARCHAR(255) PRIMARY KEY, name VARCHAR(255) NOT NULL, points INTEGER DEFAULT 0, role VARCHAR(50) DEFAULT 'student', history JSONB DEFAULT '[]', last_seen_announcement_id INTEGER DEFAULT 0)`);
    ...
    // V7: 確保現有 users 表都有新欄位
    const columnsRes = await pgPool.query("SELECT column_name FROM information_schema.columns WHERE table_name='users' AND column_name='last_seen_announcement_id'");
...
async function saveUser(user, dbClient = pgPool) {
  try {
    ...
    // V7: 在 INSERT 和 UPDATE 中加入 last_seen_announcement_id
    await dbClient.query(
        `INSERT INTO users (id, name, points, role, history, last_seen_announcement_id) VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (id) DO UPDATE SET name = $2, points = $3, role = $4, history = $5, last_seen_announcement_id = $6`,
        [user.id, user.name, user.points, user.role, historyJson, user.last_seen_announcement_id || 0]
    );
  } catch (err) {
...
async function reply(replyToken, content, menu = null) {
  ...
  if (menu !== null && menu !== undefined) {
      ...
      if (validMenuItems.length > 0 && messages.length > 0) {
          if (!messages[messages.length - 1].quickReply) messages[messages.length - 1].quickReply = { items: [] };
...
const studentMenu = [
    ...
    { type: 'postback', label: '最新公告', data: `action=run_command&text=${COMMANDS.STUDENT.LATEST_ANNOUNCEMENT}` }
];
const WEEKDAYS = [
...
const pendingAnnouncementCreation = {}; // V7 新增
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingAnnouncementCreation[userId]) {
    ...
    if (text === COMMANDS.TEACHER.CANCEL_ADD_ANNOUNCEMENT) {
        ...
        return reply(replyToken, '已取消發布公告。', teacherMenu);
    }
    ...
    switch (state.step) {
        ...
        case 'await_confirmation':
            if (text === COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT) {
                ...
                delete pendingAnnouncementCreation[userId];
                await reply(replyToken, '✅ 公告已成功發布！正在推播給所有學員...', teacherMenu);
                ...
                return reply(replyToken, `請點擊 "${COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT}" 或 "${COMMANDS.TEACHER.CANCEL_ADD_ANNOUNCEMENT}"。`);
            }
            break;
    }
    return;
  ...
  if (pendingMessageSearchQuery[userId]) {
    ...
    if (foundMessages.length === 0) {
      return reply(replyToken, `找不到與「${query}」相關的留言紀錄。`, teacherMenu);
    }
  ...
  if (pendingManualAdjust[userId]) {
      ...
      switch (manualAdjustState.step) {
          ...
          case 'awaiting_operation':
              if (text === COMMANDS.TEACHER.ADD_POINTS) { manualAdjustState.operation = 'add'; manualAdjustState.step = 'awaiting_amount'; return reply(replyToken, `請輸入要為 **${manualAdjustState.targetUserName}** 增加的點數數量 (例如：5)：`, [{ type: 'message', label: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST, text: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST }]); }
              else if (text === COMMANDS.TEACHER.DEDUCT_POINTS) { manualAdjustState.operation = 'deduct'; manualAdjustState.step = 'awaiting_amount'; return reply(replyToken, `請輸入要為 **${manualAdjustState.targetUserName}** 扣除的點數數量 (例如：10)：`, [{ type: 'message', label: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST, text: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST }]); }
          ...
          case 'awaiting_amount':
              ...
              try {
                  ...
                  else { if (userInTransaction.points < amount) { await transactionClient.query('ROLLBACK'); delete pendingManualAdjust[userId]; return reply(replyToken, `學員 ${userInTransaction.name} 點數不足（目前 ${userInTransaction.points} 點，需扣 ${amount} 點）。`, teacherMenu); } userInTransaction.points -= amount; operationType = '扣點'; }
        ...
        contents: {
            ...
            contents: [
                ...
                },
                {
                    ...
                    body: { type: 'box', layout: 'vertical', justifyContent: 'center', alignItems: 'center', height: '150px', contents: [{ type: 'text', text: '查看或刪除過去的公告', size: 'md', color: '#AAAAAA', align: 'center', weight: 'bold' }] },
  ...
  if (text === COMMANDS.TEACHER.VIEW_MESSAGES) {
      ...
      if (messagesRes.rows.length === 0) {
          return reply(replyToken, '太棒了，目前沒有新的學員留言！', teacherMenu);
      }
...
function buildBuyPointsFlex() {
  const bubbles = PURCHASE_PLANS.map(plan => ({
      ...
      footer: {
          type: 'box',
          layout: 'vertical',
          contents: [{ type: 'button', style: 'primary', height: 'sm', action: { type: 'postback', label: '選擇此方案', data: `action=select_purchase_plan&plan=${plan.points}`, displayText: `選擇購買 ${plan.points} 點方案` } }]
      }
  }));
  return {
      type: 'flex',
...
async function handleStudentCommands(event, userId) {
  ...
  if (text === COMMANDS.STUDENT.BUY_POINTS) {
    const ordersRes = await pgPool.query(`SELECT * FROM orders WHERE user_id = $1 AND (status = 'pending_payment' OR status = 'pending_confirmation' OR status = 'rejected') ORDER BY timestamp DESC LIMIT 1`, [userId]);
    const pendingOrder = ordersRes.rows[0];
    if (pendingOrder) { return reply(replyToken, `您有一筆待完成的購點訂單 (ID: ${pendingOrder.order_id})，請在「點數管理」主頁面輸入後五碼，或選擇「❌ 取消購買」。`, [returnToPointsMenuBtn, { type: 'message', label: '❌ 取消購買', text: COMMANDS.STUDENT.CANCEL_PURCHASE }]); }
  ...
  if (text === COMMANDS.STUDENT.CANCEL_BOOKING) {
      ...
      const cancelOptions = enrolledCourses.slice(0, 10).map(course => (new Date(course.time).getTime() - now > EIGHT_HOURS_IN_MS) ? { type: 'postback', label: `取消 ${formatDateTime(course.time)} ${course.title}`, data: `action=cancel_booking_confirm&courseId=${course.id}`, displayText: `準備取消預約：${course.title}` } : null).filter(item => item !== null);
  ...
  if (text === COMMANDS.STUDENT.CANCEL_WAITING) {
      ...
      const cancelWaitingOptions = waitingCourses.slice(0, 10).map(course => ({ type: 'postback', label: `取消候補 ${formatDateTime(course.time)} ${course.title}`, data: `action=cancel_waiting_confirm&courseId=${course.id}`, displayText: `準備取消候補：${course.title}` }));
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V7.0 (新增公告管理功能)`);
...
async function handleEvent(event) {
    ...
    if (!user) {
      try {
        ...
        user = { id: userId, name: profile.displayName, points: 0, role: 'student', history: [], last_seen_announcement_id: 0 };
        displayName = profile.displayName;
        await saveUser(user);
        if (STUDENT_RICH_MENU_ID) await client.linkRichMenuToUser(userId, STUDENT_RICH_MENU_ID);
      } catch (err) {
        user = { id: userId, name: `新用戶 ${userId.substring(0, 8)}...`, points: 0, role: 'student', history: [], last_seen_announcement_id: 0 };
        await saveUser(user);
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (user.role === 'teacher') {
            if (pendingCourseCreation[userId]) {
                ...
                if (text === COMMANDS.STUDENT.CONFIRM_ADD_COURSE) {
                    if (stepData.step !== 7) { await reply(event.replyToken, '無效操作，請重新從「新增課程」開始。', teacherMenu); return; }
                    const transactionClient = await pgPool.connect();
                    try {
                        await transactionClient.query('BEGIN');
                    ...
                    } catch (err) { await transactionClient.query('ROLLBACK'); console.error('❌ 新增課程交易失敗:', err.stack); delete pendingCourseCreation[userId]; await reply(event.replyToken, '新增課程時發生錯誤，請稍後再試。', teacherMenu); }
                    finally { transactionClient.release(); }
                    return;
                } else if (text === COMMANDS.STUDENT.CANCEL_ADD_COURSE) { delete pendingCourseCreation[userId]; await reply(event.replyToken, '已取消新增課程。', teacherMenu); return; }

                switch (stepData.step) {
                    case 1: if (!text) { await reply(event.replyToken, '課程名稱不能為空，請重新輸入。'); return; } stepData.courseName = text; stepData.step = 2; await reply(event.replyToken, '請輸入總堂數（例如：5，代表您想建立 5 堂課）：', [{ type: 'message', label: COMMANDS.STUDENT.CANCEL_ADD_COURSE, text: COMMANDS.STUDENT.CANCEL_ADD_COURSE }]); break;
                    case 2: const totalClasses = parseInt(text); if (isNaN(totalClasses) || totalClasses <= 0 || totalClasses > 99) { await reply(event.replyToken, '總堂數必須是 1 到 99 之間的整數，請重新輸入。'); return; } stepData.totalClasses = totalClasses; stepData.step = 3; const weekdayOptions = WEEKDAYS.map(day => ({ type: 'message', label: day.label, text: day.label })); weekdayOptions.push({ type: 'message', label: COMMANDS.STUDENT.CANCEL_ADD_COURSE, text: COMMANDS.STUDENT.CANCEL_ADD_COURSE }); await reply(event.replyToken, '請選擇課程日期（星期幾）：', weekdayOptions); break;
                    case 3: const selectedWeekday = WEEKDAYS.find(day => day.label === text); if (!selectedWeekday) { await reply(event.replyToken, '請從列表中選擇有效的星期幾。'); return; } stepData.weekday = selectedWeekday.value; stepData.step = 4; await reply(event.replyToken, '請輸入課程時間（格式為 HH:mm，例如：19:00）：', [{ type: 'message', label: COMMANDS.STUDENT.CANCEL_ADD_COURSE, text: COMMANDS.STUDENT.CANCEL_ADD_COURSE }]); break;
                    case 4: if (!/^(?:2[0-3]|[01]?[0-9]):[0-5][0-9]$/.test(text)) { await reply(event.replyToken, '課程時間格式不正確，請使用 HH:mm 格式，例如：19:00。'); return; } stepData.time = text; stepData.step = 5; await reply(event.replyToken, '請輸入人數上限（例如：10）：', [{ type: 'message', label: COMMANDS.STUDENT.CANCEL_ADD_COURSE, text: COMMANDS.STUDENT.CANCEL_ADD_COURSE }]); break;
                    case 5: const capacity = parseInt(text); if (isNaN(capacity) || capacity <= 0) { await reply(event.replyToken, '人數上限必須是正整數，請重新輸入。'); return; } stepData.capacity = capacity; stepData.step = 6; await reply(event.replyToken, '請輸入課程所需扣除點數（例如：2）：', [{ type: 'message', label: COMMANDS.STUDENT.CANCEL_ADD_COURSE, text: COMMANDS.STUDENT.CANCEL_ADD_COURSE }]); break;
        ...
        if (user.role === 'teacher') {
            ...
            if (action === 'add_announcement_start') {
                ...
                if (res.rows.length === 0) { return reply(replyToken, '目前沒有任何歷史公告。', teacherMenu); }
                ...
                return reply(replyToken, `您確定要刪除這則公告嗎？\n此操作無法復原。`, [
                    { type: 'action', action: { type: 'postback', label: COMMANDS.TEACHER.CONFIRM_DELETE_ANNOUNCEMENT, data: `action=execute_delete_announcement&id=${annId}` } },
                    { type: 'action', action: { type: 'postback', label: COMMANDS.TEACHER.ABANDON_DELETE_ANNOUNCEMENT, data: 'action=do_nothing' } }
                ]);
            }
                ...
                try {
                    ...
                    return reply(replyToken, '公告已成功刪除。', teacherMenu);
                } catch (e) {
                    console.error('❌ 刪除公告失敗:', e);
                    return reply(replyToken, '刪除公告時發生錯誤，請稍後再試。', teacherMenu);
                }
            }
            ...
            if (action === 'add_course_start') { 
              ...
              await reply(replyToken, '請輸入課程名稱（例如：哈達瑜伽）：', [{ type: 'message', label: COMMANDS.STUDENT.CANCEL_ADD_COURSE, text: COMMANDS.STUDENT.CANCEL_ADD_COURSE }]); return; 
                ...
                try {
                    ...
                    if (!orderInTransaction || orderInTransaction.status !== 'pending_confirmation') { await transactionClient.query('ROLLBACK'); await reply(replyToken, `訂單 ${orderId} 狀態不正確或已被處理。`, teacherMenu); return; }
                ...
                try {
                    ...
                    if (!courseToDelete) { await transactionClient.query('ROLLBACK'); return reply(replyToken, '課程不存在或已被取消。', teacherMenu); }
                    let refundedCount = 0;
                ...
                } catch (err) { await transactionClient.query('ROLLBACK'); console.error('❌ 取消單堂課程交易失敗:', err.stack); await reply(replyToken, '取消課程時發生錯誤，請稍後再試。', teacherMenu); }
                finally { transactionClient.release(); }
                return;
            }
