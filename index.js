/*
 * index.js - V21.1 (指令處理器重構版)
 * =================================================================
 * 功能簡介 (Functional Overview):
 * - 重構所有指令處理器 (handleAdmin, handleTeacher, handleStudent)，使其能同時處理文字訊息 (text) 與按鈕回傳 (postback) 事件。
 * - 在 handleAdminCommands 中新增「委派邏輯」，當管理者處於模擬身份時，會將事件交由對應角色的處理器執行，解決 V21.0 的 postback 錯誤。
 * - 簡化 handleEvent 路由，使其只負責根據角色分派事件，讓程式碼結構更清晰。
 *
...
async function handleAdminCommands(event, user, userState) {
  ...
  if (event.type === 'message' && event.message.type === 'text') {
    ...
    // 讓 postback 可以觸發文字指令
    if (postbackData.get('action') === 'run_command') {
      text = postbackData.get('text');
    }
  }

  // --- 委派邏輯 (Delegation Logic) ---
  // 核心：檢查指令是否為返回管理模式的「緊急出口」
  ...
  // 如果管理者在資料庫中的角色不是 'admin'，代表他正處於模擬狀態
  // 將事件委派給對應的處理器
...
async function handleTeacherCommands(event, user, userState) {
  ...
  if (userState) {
    ...
    if (stateName === 'course_creation') {
        ...
        if (postbackData && postbackData.get('action') === 'set_course_weekday' && stateData.step === 'await_weekday') {
    ...
    if (stateName === 'manual_adjust') {
        ...
        if (postbackData && postbackData.get('action') === 'select_student_for_adjust' && stateData.step === 'await_student_search') {
...
app.get('/', (req, res) => res.send('九容瑜伽 LINE Bot (V21.1 指令處理器重構版) 正常運作中。'));

// --- 伺服器啟動 ---
app.listen(PORT, async () => {
  await initializeDatabase();
  console.log(`✅ 伺服器已啟動，監聽埠號 ${PORT}`);
  console.log(`Bot 版本: V21.1 (指令處理器重構版)`);
