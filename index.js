...
const pendingReportGeneration = {}; // 新增：用於報表生成的狀態物件
...
// --- No pending conversation, handle new commands ---
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
        ...
        // 使用 pushMessage 推送「請稍候」訊息，避免 replyToken 過期問題
        push(userId, '📊 收到指令，正在為您生成點數分佈報告，請稍候...').catch(e => console.error(e));
        
        // 立即回覆 webhook 請求，避免超時
        reply(replyToken, []).catch(err => {
            // 如果 reply 失敗 (例如 token 過期)，則忽略錯誤，因為後續的 push 會處理
            if (err.message.includes('Invalid reply token')) {
                 console.log('Ignoring expired reply token for initial ack.');
            } else {
                 console.error('Error sending empty reply:', err);
            }
        });
        ...
        (async () => {
            ...
            try {
                ...
                if (students.length === 0) {
                    return push(userId, '目前沒有任何學員資料可供分析。');
                }
                ...
                let report = `💎 全體學員點數報告 💎\n\n`;
                report += `總學員數：${students.length} 人\n`;
                report += `點數總計：${totalPoints} 點\n`;
                report += `平均持有：${averagePoints} 點/人\n`;
                report += `零點學員：${zeroPointStudents} 人\n\n`;
                report += `👑 點數持有 Top 5：\n${top5}`;
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V16.0 (報表系統升級)`);
