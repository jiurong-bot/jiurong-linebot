...
async function getUser(userId, dbClient) {
    ...
    try {
        ...
        return userData; // The raw row is already snake_case
    } finally {
        if (shouldReleaseClient && client) client.release();
    }
}
...
async function getCourse(courseId, dbClient) {
    ...
    try {
        ...
        // Return object with snake_case properties
        return { 
            id: row.id, 
            title: row.title, 
            ...
            if (/^\d{5}$/.test(text)) {
                ...
                try {
                    ...
                    if (orderRes.rows.length > 0) {
                        ...
                        // Directly use the snake_case object from the DB query result
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V18.0 (資料庫欄位命名統一)`);
