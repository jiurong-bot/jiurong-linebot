            ...
            contents: {
                ...
                body: { type: 'box', layout: 'vertical', spacing: 'md', paddingAll: 'lg',
                    contents: [
                        {
                          type: 'button', style: 'secondary', height: 'sm',
                          action: { type: 'postback', label: pendingOrdersLabel, data: `action=run_command&text=${encodeURIComponent(COMMANDS.TEACHER.PENDING_POINT_ORDERS)}` } // [V23.6 修改]
                        },
                        {
                          type: 'button', style: 'secondary', height: 'sm',
...
async function showCourseRosterDetails(replyToken, courseId) {
    ...
    try {
        ...
        const createStudentListComponent = (ids, title) => {
            const contents = [];
            contents.push({ type: 'text', text: title, weight: 'bold', color: '#1A759F', margin: 'lg' });
            if (ids.length === 0) {
                contents.push({ type: 'text', text: '無', margin: 'md', size: 'sm', color: '#999999' });
            } else {
                ids.forEach(id => {
                    ...
                    contents.push({
                        ...
                        contents: [
                            { type: 'image', url: user?.picture_url || placeholderAvatar, aspectRatio: '1:1', size: 'sm', flex: 0 },
                            { type: 'text', text: user?.name || '未知用戶', wrap: true, gravity: 'center' }
                        ]
                    });
                });
            }
        ...
        case 'book':
            if (text === COMMANDS.STUDENT.CONFIRM_BOOKING) {
                ...
                try {
                    ...
                    const historyEntry = { action: `預約課程：${getCourseMainTitle(course.title)}`, pointsChange: -course.points_cost, time: new Date().toISOString() };
        ...
        case 'cancel_book':
            if (text === COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING) {
                ...
                try {
                    ...
                    const historyEntry = { action: `取消預約：${getCourseMainTitle(course.title)}`, pointsChange: +course.points_cost, time: new Date().toISOString() };
                    ...
                    if (newWaiting.length > 0) {
                        ...
                        if (promotedUser) {
                             push(promotedUserId, `🎉 候補成功通知 🎉\n您候補的課程「${getCourseMainTitle(course.title)}」已有空位，已為您自動預約成功！`).catch(err => console.error(err));
                        }
                    }
                    ...
                    delete pendingBookingConfirmation[userId];
                    return reply(replyToken, `✅ 已為您取消「${getCourseMainTitle(course.title)}」的預約，並歸還 ${course.points_cost} 點。`);
                } catch (e) {
                    await client.query('ROLLBACK');
    ...
    try {
        ...
        for (const course of upcomingCourses) {
            if (!sentReminders[course.id]) {
                const reminderMsg = `🔔 課程提醒：\n您的課程「${getCourseMainTitle(course.title)}」即將在約一小時後 (${formatDateTime(course.time)}) 開始，請準備上課！`;
                for (const studentId of course.students) {
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V23.6 (老師功能強化)`); // [V23.6 修改]
...
async function handleEvent(event) {
    if (event.type === 'follow') {
        ...
        try {
            ...
            push(userId, `歡迎 ${user.name}！感謝您加入九容瑜伽。`).catch(err => console.error(`發送歡迎詞給新用戶 ${userId} 失敗:`, err.message));
            if (STUDENT_RICH_MENU_ID) await client.linkRichMenuToUser(userId, STUDENT_RICH_MENU_ID);
        } catch (error) {
    ...
    if ((event.type === 'message' && (event.message.type === 'text' || event.message.type === 'image'))) {
        ...
        // ... (The entire postback handling logic from previous parts goes here)
        if (action === 'view_course_series') {
            return showCourseSeries(replyToken, page);
        }
        if (action === 'view_course_roster_summary') { // [V23.6 新增]
            return showCourseRosterSummary(replyToken, page);
        }
        if (action === 'view_course_roster_details') { // [V23.6 新增]
