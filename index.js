...
const teacherMenu = [
  ...
  { type: 'action', action: { type: 'message', label: '查個別學員', text: '@查學員 userId' } },
  { type: 'action', action: { type: 'message', label: '加點', text: '@加點 userId 5' } },
  { type: 'action', action: { type: 'message', label: '扣點', text: '@扣點 userId 1' } },
...
async function handleEvent(event) {
  ...
  if (user.role === 'student') {
    ...
    if (msg === '@我的課程') {
      ...
      return replyWithMenu(event.replyToken, `✅ 您的預約紀錄：\n${my}`, studentMenu);
    }
  ...
  if (user.role === 'teacher') {
    ...
    if (msg.startsWith('@新增課程')) {
      ...
      if (parts.length < 5) return replyText(event.replyToken, '格式錯誤，請輸入：@新增課程 課名 日期 時間 名額');
      const name = parts[1];
    ...
    if (msg.startsWith('@查學員')) {
      ...
      if (parts.length === 2) {
        ...
        const history = stu.history.map(h => `${h.courseId} - ${h.time}`).join('\n') || '無紀錄';
        return replyWithMenu(event.replyToken, `學員 ID：${targetId}\n點數：${stu.points}\n預約紀錄：\n${history}`, teacherMenu);
      }
      return replyWithMenu(event.replyToken, '請輸入：@查學員 userId', teacherMenu);
    }

    if (msg.startsWith('@加點') || msg.startsWith('@扣點')) {
      const parts = msg.split(' ');
      if (parts.length !== 3) return replyWithMenu(event.replyToken, '格式錯誤，請輸入：@加點 userId 數量', teacherMenu);
      const [_, targetId, numStr] = parts;
      const student = db[targetId];
      const amount = parseInt(numStr);
      if (!student || isNaN(amount)) return replyText(event.replyToken, '找不到學員或數量錯誤');
      ...
      writeJSON(DATA_FILE, db);
      return replyWithMenu(event.replyToken, `✅ 已更新 ${targetId} 點數，目前點數：${student.points}`, teacherMenu);
    }
