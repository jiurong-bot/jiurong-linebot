...
async function getLatestAnnouncements(limit = 10) {
  const cacheKey = `latest_announcements_${limit}`;
  const cached = (typeof simpleCache !== 'undefined') ? simpleCache.get(cacheKey) : null;
  if (cached) return cached;
  const rows = await executeDbQuery(async (client) => {
    const res = await client.query('SELECT * FROM announcements ORDER BY created_at DESC LIMIT $1', [limit]);
    return res.rows;
  });
  if (typeof simpleCache !== 'undefined') simpleCache.set(cacheKey, rows, 30 * 1000); // 30 秒快取
  return rows;
}

async function getShopProducts(limit = 50) {
  const cacheKey = `shop_products_${limit}`;
  const cached = (typeof simpleCache !== 'undefined') ? simpleCache.get(cacheKey) : null;
  if (cached) return cached;
  const rows = await executeDbQuery(async (client) => {
    const res = await client.query('SELECT * FROM products WHERE status = $1 ORDER BY updated_at DESC LIMIT $2', ['available', limit]);
    return res.rows;
  });
  if (typeof simpleCache !== 'undefined') simpleCache.set(cacheKey, rows, 30 * 1000); // 30 秒快取
  return rows;
...
// === END announcements/shop cache ===

// === 快取化的購點方案存取器 (V46) ===
function getPurchasePlans() {
  const cacheKey = 'purchase_plans';
  const cached = (typeof simpleCache !== 'undefined') ? simpleCache.get(cacheKey) : null;
  if (cached) return cached;
  const plans = (typeof CONSTANTS !== 'undefined' && CONSTANTS.PURCHASE_PLANS) ? CONSTANTS.PURCHASE_PLANS : [];
  if (typeof simpleCache !== 'undefined') simpleCache.set(cacheKey, plans, 1000 * 60 * 60); // 1 小時
  return plans;
}
// === END getPurchasePlans ===
...
async function updateUserProfileIfNeeded(userId, currentUser = null) {
  ...
  // ✅ 改用 simpleCache 統一管理
  if (simpleCache.get(`profile_${userId}`)) return user;
  ...
  try {
    ...
    // 更新快取時間戳
    simpleCache.set(`profile_${userId}`, { timestamp: Date.now() }, 10 * 60 * 1000);
...
function buildBuyPointsFlex() {
    const plansContent = getPurchasePlans().flatMap((plan, index) => {
        const planItems = [
            {
                type: 'box',
                layout: 'horizontal',
            ...
            (async () => {
                await executeDbQuery(async (client) => {
                    ...
                    try {
                        ...
                        // --- 優化點 1: 一次性刪除所有相關的老師提醒 ---
                        const firstCourse = coursesToCancel[0];
                        const courseMainTitle = getCourseMainTitle(firstCourse.title);
                        const reminderTextPattern = `%${courseMainTitle}%`;
                        await client.query(
                            `DELETE FROM tasks 
                             WHERE recipient_id = $1 
                             AND status = 'pending' 
                             AND message_payload::text LIKE $2`,
                            [firstCourse.teacher_id, reminderTextPattern]
                        );
                        ...
                        if (affectedUsers.size > 0) {
                            ...
                            userIds.forEach((id, index) => {
                                caseStatement += ` WHEN $${index + 2} THEN ${affectedUsers.get(id)} `;
                                params.push(id);
                            });
                            caseStatement += ' END';

                            await client.query(
                                `UPDATE users SET points = points + (${caseStatement}) WHERE id = ANY($1)`,
                                params
                            );
                        }
    ...
    return executeDbQuery(async (client) => {
        ...
        const query = `
            SELECT
                ...
                u.points,
                (
                    SELECT JSON_AGG(courses_agg.*)
                    FROM (
                        SELECT c.title, c.time
                        FROM courses c
                        WHERE u.id = ANY(c.students) AND c.time > NOW()
                        ORDER BY c.time ASC LIMIT 3
                    ) AS courses_agg
                ) AS upcoming_courses,
                (
                    SELECT JSON_AGG(orders_agg.*)
                    FROM (
                        SELECT o.*
                        FROM orders o
        ...
        // 從 JSONB 欄位直接取得課程和訂單陣列，如果沒有則給予空陣列
        const coursesRes = { rows: studentData.upcoming_courses || [] };
        const ordersRes = { rows: studentData.recent_orders || [] };
        
        // --- 後續組合 Flex Message 的邏輯完全不變 ---
...
async function gracefulShutdown(signal) {
  ...
  try {
    try { if (typeof simpleCache !== 'undefined') simpleCache.stopCleanup(); } catch (e) { /* ignore */ }
    try { if (typeof pgPool !== 'undefined' && pgPool && typeof pgPool.end === 'function') { await pgPool.end(); console.log('[Shutdown] pgPool 已關閉。'); } } catch (e) { console.error('[Shutdown] 關閉 pgPool 發生錯誤：', e); }
    if (global.__PING_INTERVAL__) clearInterval(global.__PING_INTERVAL__);
    console.log('[Shutdown] Cleanup complete. Exiting now.');
  } catch (err) {
    console.error('[Shutdown] 無法完整清理：', err);
  } finally {
    process.exit(0);
  }
}
