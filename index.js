...
/**
 * [V33.0 新增] 建立一個通用的分頁 Flex Message Carousel。
 * @param {object} options - 設定物件
 * @param {string} options.altText - Flex Message 的替代文字。
 * @param {string} options.baseAction - 分頁按鈕的 postback action，例如 'action=view_history'。
 * @param {number} options.page - 當前頁碼。
 * @param {string} options.dataQuery - 用於獲取當頁資料的 SQL 查詢 (需包含 LIMIT 和 OFFSET 的 placeholder，例如 $1, $2)。
 * @param {Array<any>} options.queryParams - 上述查詢所需的所有參數 (不包含 LIMIT 和 OFFSET)。
 * @param {function(object): object} options.mapRowToBubble - 一個函式，將單筆資料庫 row 轉換成 Flex Bubble 物件。
 * @param {string} [options.noDataMessage='沒有任何資料。'] - 當第一頁沒有資料時顯示的訊息。
 * @param {string} [options.customParams=''] - 附加到分頁按鈕 postback data 的額外參數。
 * @returns {Promise<object|string>} - Flex Message 物件或無資料時的文字訊息。
 */
async function createPaginatedCarousel({ altText, baseAction, page, dataQuery, queryParams, mapRowToBubble, noDataMessage = '沒有任何資料。', customParams = '' }) {
    const offset = (page - 1) * CONSTANTS.PAGINATION_SIZE;

    return withDatabaseClient(async (client) => {
        // 準備參數，將分頁參數加到最後
        const finalParams = [...queryParams, CONSTANTS.PAGINATION_SIZE + 1, offset];
        const res = await client.query(dataQuery, finalParams);

        const hasNextPage = res.rows.length > CONSTANTS.PAGINATION_SIZE;
        const pageItems = hasNextPage ? res.rows.slice(0, CONSTANTS.PAGINATION_SIZE) : res.rows;

        if (pageItems.length === 0) {
            return page === 1 ? noDataMessage : '沒有更多資料了。';
        }

        // 使用 Promise.all 等待所有 bubble 都建立完成 (以防 mapRowToBubble 是非同步的)
        const itemBubbles = await Promise.all(pageItems.map(mapRowToBubble));

        const paginationBubble = createPaginationBubble(baseAction, page, hasNextPage, customParams);
        if (paginationBubble) {
            itemBubbles.push(paginationBubble);
        }

        return {
            type: 'flex',
            altText,
            contents: { type: 'carousel', contents: itemBubbles }
        };
    });
}
...
/**
 * [V33.0 重構] 使用通用的 createPaginatedCarousel 函式
 */
async function showPurchaseHistory(userId, page) {
    // 步驟 1: 定義如何將一筆訂單資料轉換成 Bubble
...
app.listen(PORT, async () => {
  try {
    ...
    console.log(`Bot 版本 V33.0 (程式碼重構)`);
