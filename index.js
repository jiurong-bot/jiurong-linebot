...
const COMMANDS = {
  ...
  ADMIN: {
    ...
    TOGGLE_NOTIFICATIONS: '@切換推播通知' // <--- [V25.1 新增]
  },
  TEACHER: {
...
async function handleAdminCommands(event, userId) {
  ...
  if (pendingTeacherAddition[userId]) {
    ...
    } else if (text === COMMANDS.ADMIN.TOGGLE_NOTIFICATIONS) { // [V25.1 新增] 處理切換指令的邏輯
        const currentStatus = await getNotificationStatus();
        ...
        const createStudentListComponent = (ids, title) => {
            ...
            if (ids.length > 0) {
                ids.forEach(id => {
                    ...
                    // [V23.7.1 排版優化] 調整為垂直置中排版 (頭像在上，姓名在下)
                    studentBoxes.push({
                        type: 'box',
                        layout: 'vertical',
                        alignItems: 'center',
            ...
            if (studentBoxes.length === 0) {
                ...
                // 將學員卡片放入水平排列的 box 中，每行最多顯示 4 位，以形成網格
                const rows = [];
        ...
        case 'book':
            if (text === COMMANDS.STUDENT.CONFIRM_BOOKING) {
                ...
                try {
                    ...
                    const courseForUpdate = await client.query('SELECT students, capacity, time, title, points_cost FROM courses WHERE id = $1 FOR UPDATE', [state.course_id]);
                    const currentCourse = courseForUpdate.rows[0];

                    if (userForUpdate.rows[0].points < currentCourse.points_cost) {
                        await client.query('ROLLBACK');
                        delete pendingBookingConfirmation[userId];
                        return reply(replyToken, `預約失敗，您的點數不足！\n目前點數：${userForUpdate.rows[0].points}\n需要點數：${currentCourse.points_cost}`);
                    }
                    if (currentCourse.students.length >= currentCourse.capacity) {
                        await client.query('ROLLBACK');
                        delete pendingBookingConfirmation[userId];
                        return reply(replyToken, '抱歉，課程名額已滿，已被其他同學搶先預約了。');
                    }

                    const newPoints = userForUpdate.rows[0].points - currentCourse.points_cost;
                    const newStudents = [...currentCourse.students, userId];
                    const historyEntry = { action: `預約課程：${getCourseMainTitle(currentCourse.title)}`, pointsChange: -currentCourse.points_cost, time: new Date().toISOString() };
                    ...
                    delete pendingBookingConfirmation[userId];
                    await reply(replyToken, `✅ 預約成功！\n課程：${getCourseMainTitle(currentCourse.title)}\n時間：${formatDateTime(currentCourse.time)}\n\n已為您扣除 ${currentCourse.points_cost} 點，期待課堂上見！`);
                    
                    // [V24.0 新增] 建立排程提醒任務
                    try {
                        const courseTime = new Date(currentCourse.time);
                        const reminderTime = new Date(courseTime.getTime() - ONE_HOUR_IN_MS);
                        if (reminderTime > new Date()) {
                            const reminderMsg = { type: 'text', text: `🔔 課程提醒：\n您的課程「${getCourseMainTitle(currentCourse.title)}」即將在約一小時後開始，請準備上課！` };
                            await enqueuePushTask(userId, reminderMsg, reminderTime);
                        }
                    } catch(e) {
                        console.error(`建立課程提醒任務失敗 for user ${userId}, course ${state.course_id}`, e);
                    }
        ...
        case 'cancel_book':
            if (text === COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING) {
                ...
                try {
                    ...
                    const newStudents = currentCourse.students.filter(id => id !== userId);
                    const historyEntry = { action: `取消預約：${getCourseMainTitle(currentCourse.title)}`, pointsChange: +currentCourse.points_cost, time: new Date().toISOString() };
                    ...
                    delete pendingBookingConfirmation[userId];
                    return reply(replyToken, `✅ 已為您取消「${getCourseMainTitle(currentCourse.title)}」的預約，並歸還 ${currentCourse.points_cost} 點。`);
                } catch (e) {
                    await client.query('ROLLBACK');
...
app.listen(PORT, async () => {
  try {
    ...
    console.log(`Bot 版本: V25.1 (新增開發者推播開關)`); // [V25.1 修改]

    // [V24.0 移除] 不再需要前景的 checkAndSendReminders
