            ...
            (async () => {
              ...
              try {
                ...
                if (coursesToCancelRes.rows.length === 0) {
                  await push(userId, `❌ 批次取消失敗：找不到可取消的「${backgroundStateData.prefix}」系列課程。`);
                  return;
                }
                ...
                for (const [studentId, refundAmount] of affectedUsers.entries()) {
                  if (refundAmount > 0) await client.query("UPDATE users SET points = points + $1 WHERE id = $2", [refundAmount, studentId]);
                }
...
async function handleStudentCommands(event, user) {
  ...
  if(studentState) {
    if(studentState.name === 'booking_confirmation') {
      ...
      switch (state.data.type) {
          case 'book':
              if (text === COMMANDS.STUDENT.ABANDON_BOOKING) {
                  user.state = null; await saveUser(user);
              ...
              if (text === COMMANDS.STUDENT.CONFIRM_BOOKING) {
                  ...
                  try {
                      ...
                      const courseForUpdate = await client.query('SELECT students, capacity FROM courses WHERE id = $1 FOR UPDATE', [state.data.courseId]);
                      ...
                      user.history = user.history ? [...user.history, historyEntry] : [historyEntry];
