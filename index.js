...
async function handleEvent(event) {
    ...
    if (!user) {
        ...
        try {
            ...
            if (profile.pictureUrl) {
                ...
                user.name = profile.displayName; // 同步更新名稱
                await saveUser(user);
            }
    ...
    if (event.type === 'postback') {
        ...
        if (action === 'confirm_booking' || action === 'cancel_booking_confirm' || action === 'cancel_waiting_confirm') {
            // ... (程式碼已包含在第二部分)
        }
        
        // --- 老師 Postback ---
        if (action === 'start_student_search' || action === 'start_message_search' || action === 'reply_feedback' || action === 'add_announcement_start' || action === 'add_course_start' || action === 'select_manual_adjust_student') {
            // ... (這些設定狀態的 postback 已包含在第二部分)
        }
        // ... (其他無狀態 postback)
    }
}
