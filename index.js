...
'@課程查詢',
      POINT_MANAGEMENT: '@點數管理',
        ...
        PURCHASE_HISTORY_INQUIRY: '@查詢購點紀錄', // V35.0 新增
      STUDENT_MANAGEMENT: '@學員管理',
        SEARCH_STUDENT: '@查詢學員',
        VIEW_MESSAGES: '@查看未回覆留言',
        MESSAGE_HISTORY_INQUIRY: '@查詢歷史留言', // V35.0 優化
...
'@商品管理',
        ...
        EXCHANGE_HISTORY_INQUIRY: '@查詢兌換紀錄', // V35.0 新增
      REPORT: '@統計報表',
        COURSE_REPORT: '@課程報表',
        ORDER_REPORT: '@訂單報表',
        POINT_REPORT: '@點數報表',
   ...
   ADD_POINTS: '+ 加點',
      DEDUCT_POINTS: '- 扣點',
      MESSAGE_SEARCH: '@查看歷史留言', // 保留舊的供相容，但實際上由 MESSAGE_HISTORY_INQUIRY 取代
...
FROM orders WHERE status = 'pending_confirmation'"),
...
async function cleanCoursesDB() {
    try {
        await executeDbQuery(async (client) => {
            ...
            const result = await client.query(`DELETE FROM courses WHERE time 
< $1`, [pastDate]);
...
bodyContents.push({ type: 'separator', margin: 'lg' });
    ...
    bodyContents.push({
        ...
        contents: [
            ...
            { type: 'text', text: `${user.points} 點`, weight: 
'bold', size: '3xl', margin: 'sm', color: '#1A759F' },
        ]
    });
return {
        type: 'flex',
...
const pendingPurchaseHistorySearch = {}; // V35.0 新增
const pendingExchangeHistorySearch = {}; // V35.0 新增
const pendingMessageHistorySearch = {}; // V35.0 新增
...
const cancellableConversationStates = {
    ...
    pendingPurchaseHistorySearch, // V35.0 新增
    pendingExchangeHistorySearch, // V35.0 新增
    pendingMessageHistorySearch, // V35.0 新增
};
/**
 * 清除使用者所有待處理的對話狀態。
...
// --- V35.0 新增：處理查詢指令的函式 ---
async function showPurchaseHistoryInquiryMenu(event, user) {
    return {
        type: 'text',
        text: '請選擇查詢購點紀錄的方式：',
        quickReply: {
            items: [
                { type: 'action', action: { type: 'postback', label: '📜 顯示全部紀錄', data: 'action=view_all_purchase_history&page=1' } },
...
async function showExchangeHistoryInquiryMenu(event, user) {
    return {
        type: 'text',
        text: '請選擇查詢兌換紀錄的方式：',
        quickReply: {
            items: [
                { type: 'action', action: { type: 'postback', label: '📜 顯示全部紀錄', data: 'action=view_all_exchange_history&page=1' } },
...
async function showMessageHistoryInquiryMenu(event, user) {
    return {
        type: 'text',
        text: '請選擇查詢歷史留言的方式：',
        quickReply: {
            items: [
                { type: 'action', action: { type: 'postback', label: '📜 顯示全部紀錄', data: 'action=view_all_message_history&page=1' } },
...
const teacherCommandMap = {
    ...
    [CONSTANTS.COMMANDS.TEACHER.PURCHASE_HISTORY_INQUIRY]: showPurchaseHistoryInquiryMenu, // V35.0 新增
    ...
    [CONSTANTS.COMMANDS.TEACHER.MESSAGE_HISTORY_INQUIRY]: showMessageHistoryInquiryMenu, // V35.0 優化
...
showShopOrderManagement,
    [CONSTANTS.COMMANDS.TEACHER.EXCHANGE_HISTORY_INQUIRY]: showExchangeHistoryInquiryMenu, // V35.0 新增
...
ℹ️ 「待處理任務」是系統即將要發送的排程訊息 
(如課程提醒)。若「失敗任務」數量持續增加，請檢查 Worker 紀錄。
    `.trim();
...
LIKE $1 AND time > NOW() FOR UPDATE", [`${backgroundState.prefix}%`]);
...
case 'single':
         ...
         if (studentIdsToNotify.length > 0) {
                      const batchTasks = studentIdsToNotify.map(studentId => ({
                          recipientId: studentId,
                          message: { type: 'text', text: `課程取消通知：\n老師已取消您預約的課程「${course.title}」，已歸還 ${course.points_cost} 
點至您的帳戶。` }
                      }));
await enqueueBatchPushTasks(batchTasks);
                  }
...
case 'await_confirmation':
    ...
    const prefix = await generateUniqueCoursePrefix(client); 
                        ...
                        for (let i = 0; i < state.sessions; i++) {
                            const courseDate 
= getNextDate(state.weekday, state.time, currentDate);
...
case 'await_operation':
        if (text === CONSTANTS.COMMANDS.TEACHER.ADD_POINTS || text === CONSTANTS.COMMANDS.TEACHER.DEDUCT_POINTS) { state.operation = text === CONSTANTS.COMMANDS.TEACHER.ADD_POINTS ?
'add' : 'deduct'; state.step = 'await_amount'; return { type: 'text', text: `請輸入要 ${text === CONSTANTS.COMMANDS.TEACHER.ADD_POINTS ?
...
case 'await_reason': state.reason = text; state.step = 'await_confirmation'; const opText = state.operation === 'add' ?
`增加 ${state.amount} 點` : `扣除 ${state.amount} 點`; const summary = `請確認調整內容：\n\n對象：${state.targetStudent.name}\n操作：${opText}\n原因：${state.reason}`;
...
// MA for Manual Adjust
                const pointsChange = state.operation === 'add' ?
...
if (res.rows.length === 0) {
  ...
  // V35.0 新增: 處理查詢購點紀錄的特定學員搜尋
    ...
    if (res.rows.length === 0) {
        return { type: 'text', text: `找不到與「${searchQuery}」相關的學員。請重新操作。` };
    }
    return showStudentSelectionForPurchaseHistory(res.rows, searchQuery);
  }
  
  // V35.0 新增: 處理查詢兌換紀錄的特定學員搜尋
    ...
    return showStudentSelectionForExchangeHistory(res.rows, searchQuery);
  }

  // V35.0 新增: 處理查詢歷史留言的特定學員搜尋
    ...
    return showStudentSelectionForMessageHistory(res.rows, searchQuery);
  }
...
case 'await_bio':
                state.profileData.bio = text.trim().toLowerCase() === '無' ?
null : text;
            ...
            if (text === CONSTANTS.COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING) {
                return withDatabaseClient(async (client) => {
                 ...
                 const userForUpdateRes = await client.query('SELECT points, history FROM users WHERE id = $1 FOR UPDATE', [userId]);
                        ...
                        if (courseForUpdateRes.rows.length === 
0) {
                            await client.query('ROLLBACK');
                            delete pendingBookingConfirmation[userId];
                            return '取消失敗，找不到此課程。';
...
const userBubbles = users.map(u => ({
        ...
        body: {
            ...
            spacing: 'md',
            contents: [
                { type: 'image', url: u.picture_url || placeholder_avatar, size: 'md', 
...
// V35.0 新增
async function showStudentSelectionForPurchaseHistory(users, originalQuery) {
    const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
    const userBubbles = users.map(u => ({
        type: 'bubble',
        body: {
        ...
        footer: {
            ...
            contents: [{
                ...
                action: { type: 'postback', label: '查看此學員紀錄', data: `action=view_all_purchase_history&user_id=${u.id}&page=1` }
            }]
        }
    }));
    return { type: 'flex', altText: `請選擇要查詢購點紀錄的學員`, contents: { type: 'carousel', contents: userBubbles } };
}

// V35.0 新增
async function showStudentSelectionForExchangeHistory(users, originalQuery) {
    const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
    const userBubbles = users.map(u => ({
        type: 'bubble',
        body: {
        ...
        footer: {
            ...
            contents: [{
                type: 'button', style: 'primary', color: '#1A759F', height: 'sm',
                action: { type: 'postback', label: '查看此學員紀錄', data: `action=view_all_exchange_history&user_id=${u.id}&page=1` }
            }]
        }
    }));
    return { type: 'flex', altText: `請選擇要查詢兌換紀錄的學員`, contents: { type: 'carousel', contents: userBubbles } };
}

// V35.0 新增
async function showStudentSelectionForMessageHistory(users, originalQuery) {
    const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
    const userBubbles = users.map(u => ({
        type: 'bubble',
        body: {
        ...
        footer: {
            ...
            contents: [{
                type: 'button', style: 'primary', color: '#1A759F', height: 'sm',
                action: { type: 'postback', label: '查看此學員紀錄', data: `action=view_all_message_history&user_id=${u.id}&page=1` }
            }]
        }
    }));
    return { type: 'flex', altText: `請選擇要查詢留言紀錄的學員`, contents: { type: 'carousel', contents: userBubbles } };
}
...
// [V35.0 新增] 顯示老師視角的購點紀錄 (可依學員篩選)
async function showTeacherPurchaseHistory(page, userId = null) {
    const offset = (page - 1) * CONSTANTS.PAGINATION_SIZE;
    return withDatabaseClient(async (client) => {
        let query = "SELECT * FROM orders WHERE status = 'completed' AND amount > 0";
        const queryParams = [];
        let paramIndex = 1;
        ...
        if (pageRows.length === 0 && page === 1) {
            return userId ? '這位學員沒有任何成功購點紀錄。' : '目前沒有任何成功購點紀錄。';
        }
        if (pageRows.length === 0) {
            return '沒有更多紀錄了。';
        }
        ...
        const listItems = pageRows.map(order => ({
            ...
            contents: [
                ...
                {
                    type: 'text', text: `+${order.points} 點 / ${order.amount} 元`,
                    gravity: 'center', align: 'end', flex: 3, weight: 'bold', size: 'sm', color: '#28A745',
                }
            ]
        }));
        
        const customParams = userId ? `&user_id=${userId}` : '';
        const paginationBubble = createPaginationBubble('action=view_all_purchase_history', page, hasNextPage, customParams);
        const footerContents = paginationBubble ? paginationBubble.body.contents : [];
        
        const headerText = userId ? `${pageRows[0].user_name} 的購點紀錄` : '學員購點紀錄';
        return {
            type: 'flex',
            altText: headerText,
            contents: {
...
// [V35.0 新增] 顯示老師視角的兌換紀錄 (可依學員篩選)
async function showTeacherExchangeHistory(page, userId = null) {
    const offset = (page - 1) * CONSTANTS.PAGINATION_SIZE;
    return withDatabaseClient(async (client) => {
        ...
        if (pageRows.length === 0 && page === 1) {
            return userId ? '這位學員沒有任何商品兌換紀錄。' : '目前沒有任何商品兌換紀錄。';
        }
        if (pageRows.length === 0) {
            return '沒有更多紀錄了。';
        }
        ...
        const listItems = pageRows.map(order => {
            const statusInfo = statusMap[order.status] || { text: '未知', color: '#6c757d' };
            return {
                type: 'box',
                layout: 'horizontal',
                paddingAll: 'md',
                contents: [
                    {
                        ...
                        contents: [
                            { type: 'text', text: `${order.user_name} 兌換`, weight: 'bold', size: 'sm' },
                            { type: 'text', text: order.product_name, size: 'sm', wrap: true, color: '#666666' },
                            { type: 'text', text: formatDateTime(order.created_at), size: 'xxs', color: '#AAAAAA', margin: 'sm' }
                        ]
                    },
                    {
                        type: 'box', layout: 'vertical', flex: 2, justifyContent: 'center', alignItems: 'flex-end',
                        contents: [
                            { type: 'text', text: statusInfo.text, color: statusInfo.color, size: 'xs', weight: 'bold' },
                            { type: 'text', text: `-${order.points_spent} 點`, size: 'sm', color: '#D9534F' }
                        ]
                    }
                ]
            };
        ...
        const customParams = userId ? `&user_id=${userId}` : '';
        const paginationBubble = createPaginationBubble('action=view_all_exchange_history', page, hasNextPage, customParams);
        const footerContents = paginationBubble ? paginationBubble.body.contents : [];
        
        const headerText = userId ? `${pageRows[0].user_name} 的兌換紀錄` : '學員兌換紀錄';
        return {
            type: 'flex',
            altText: headerText,
            contents: {
...
// [V35.0 新增] 顯示老師視角的歷史留言 (可依學員篩選)
async function showTeacherMessageHistory(page, userId = null) {
    const offset = (page - 1) * CONSTANTS.PAGINATION_SIZE;
    return withDatabaseClient(async (client) => {
        ...
        if (pageRows.length === 0 && page === 1) {
            return userId ? '這位學員沒有任何留言紀錄。' : '目前沒有任何學員留言紀錄。';
        }
        if (pageRows.length === 0) {
            return '沒有更多留言了。';
        }
        ...
        const listItems = pageRows.map(msg => {
            const statusInfo = statusMap[msg.status] || { text: msg.status, color: '#6c757d' };
            const replyContent = msg.teacher_reply
                ? [{ type: 'separator' }, { type: 'text', text: `老師回覆：${msg.teacher_reply}`, wrap: true, size: 'xs', color: '#495057' }]
                : [];
            return {
                type: 'box',
                layout: 'vertical',
                ...
                contents: [
                    ...
                    { type: 'text', text: `學員留言：${msg.message}`, wrap: true, size: 'sm' },
                    ...replyContent,
        ...
        const customParams = userId ? `&user_id=${userId}` : '';
        const paginationBubble = createPaginationBubble('action=view_all_message_history', page, hasNextPage, customParams);
        const footerContents = paginationBubble ? paginationBubble.body.contents : [];
        
        const headerText = userId ? `${pageRows[0].user_name} 的留言紀錄` : '學員歷史留言';
        return {
            type: 'flex',
            altText: headerText,
            contents: {
                ...
                header: {
                    type: 'box', layout: 'vertical',
                    contents: [{ type: 'text', text: headerText, weight: 'bold', size: 'lg', color: '#FFFFFF', wrap: true }],
                    backgroundColor: '#343A40'
                },
                body: {
                    type: 'box', layout: 'vertical', paddingAll: 'none',
...
return withDatabaseClient(async (client) => {
        ...
        const res = await client.query(
            ...
            [searchQuery, CONSTANTS.PAGINATION_SIZE 
+ 1, offset]
        );
...
return '沒有更多搜尋結果了。';
        ...
        const listItems = pageMessages.map(msg => {
          ...
          const statusInfo = statusMap[msg.status] ||
{ text: msg.status, color: '#6c757d' };
            const replyContent = msg.teacher_reply 
                ?
...
[{ type: 'text', text: `與「${query}」相關的留言`, weight: 'bold', size: 'lg', color: '#FFFFFF', wrap: true }], backgroundColor: '#343A40' },
...
text: '待處理商城訂單', weight: 'bold', size: 'lg', color: '#FFFFFF' }], backgroundColor: '#343A40' },
                body: { type: 'box', layout: 'vertical', paddingAll: 'none', contents: listItems.flatMap((item, index) => index === 0 ? [item] : [{ type: 'separator' }, item]) },
                footer: { type: 'box', layout: 'vertical', contents: footerContents }
            }
        };
            ...
            contents: [
                {
                 ...
                 flex: 4,
                    contents: [
                        ...
                        { type: 'text', text: 
`由 ${ann.creator_name} 於 ${formatDateTime(ann.created_at)} 發布`, size: 'xxs', color: '#AAAAAA', margin: 'lg', wrap: true }
                    ]
                },
                {
                    type: 'box',
...
case 'manage_course_group': return showSingleCoursesForCancellation(data.get('prefix'), page);
        ...
        case 'start_purchase_history_search': {
            ...
            setupConversationTimeout(userId, pendingPurchaseHistorySearch, 'pendingPurchaseHistorySearch', (u) => {
                enqueuePushTask(u, { type: 'text', text: '搜尋操作已逾時，自動取消。' });
            });
            return {
                type: 'text',
                text: '請輸入您想查詢購點紀錄的學員姓名：',
                quickReply: { items: getCancelMenu() }
            };
        }
        case 'view_all_purchase_history': 
            return showTeacherPurchaseHistory(page, data.get('user_id'));
        ...
        case 'start_exchange_history_search': {
            ...
            setupConversationTimeout(userId, pendingExchangeHistorySearch, 'pendingExchangeHistorySearch', (u) => {
                enqueuePushTask(u, { type: 'text', text: '搜尋操作已逾時，自動取消。' });
            });
            return {
                type: 'text',
                text: '請輸入您想查詢兌換紀錄的學員姓名：',
                quickReply: { items: getCancelMenu() }
            };
        }
        case 'view_all_exchange_history':
            return showTeacherExchangeHistory(page, data.get('user_id'));
        ...
        case 'start_message_history_search': {
            ...
            setupConversationTimeout(userId, pendingMessageHistorySearch, 'pendingMessageHistorySearch', (u) => {
                enqueuePushTask(u, { type: 'text', text: '搜尋操作已逾時，自動取消。' });
            });
            return {
                type: 'text',
                text: '請輸入您想查詢歷史留言的學員姓名：',
                quickReply: { items: getCancelMenu() }
            };
        }
        case 'view_all_message_history':
            return showTeacherMessageHistory(page, data.get('user_id'));
        // ==================================
        // [V34.0 & V34.1] 師資管理
        // ==================================
...
return withDatabaseClient(async (client) => {
                    if (reportType === 'course') {
                       ...
                       let totalStudents = 0, totalCapacity = 0;
                        ...
                        const attendanceRate = totalCapacity > 0 ? (totalStudents / totalCapacity * 100).toFixed(1) 
: 0;
                        ...
                        const res = await client.query("SELECT COUNT(*), SUM(amount) FROM orders WHERE status = 'completed' AND 
timestamp BETWEEN $1 AND $2", [start, end]);
                        const count = parseInt(res.rows[0].count, 10) || 0;
                        const sum = parseInt(res.rows[0].sum, 10) ||
0;
...
return withDatabaseClient(async (client) => {
                ...
                try {
                    ...
                    if (productRes.rows.length === 0) { await client.query('ROLLBACK'); 
return '找不到該商品。'; }
...
const result = await withDatabaseClient(async (client) => {
                ...
                try {
                    ...
                    const userRes = await client.query('SELECT 
* FROM users WHERE id = $1 FOR UPDATE', [user.id]);
      ...
      if (product.inventory <= 0) { await client.query('ROLLBACK'); return '抱歉，您慢了一步！商品已被兌換完畢。'; }
                    ...
                    await 
client.query('UPDATE users SET points = points - $1 WHERE id = $2', [product.price, userId]);
