// index.js - V20.0 (商城功能 Phase 2 - 完整管理)
...
const COMMANDS = {
  ...
  TEACHER: {
    ...
    SHOP_MANAGEMENT: '@商城管理',
      ADD_PRODUCT: '@上架新商品',
      VIEW_PRODUCTS: '@管理商品列表', // [修改] 名稱變更
      REMOVE_PRODUCT: '@下架商品', // [註解] 此指令未來可能移除，由管理列表內的按鈕取代
      SHOP_ORDER_MANAGEMENT: '@訂單管理',
      PENDING_SHOP_ORDERS: '@待處理訂單',
      // [新增] Phase 2 指令
      EDIT_PRODUCT: '@修改商品資訊',
      ADJUST_INVENTORY: '@調整庫存',
      CONFIRM_PRODUCT_EDIT: '✅ 確認修改',
      CONFIRM_INVENTORY_ADJUST: '✅ 確認調整庫存',
    VIEW_MESSAGES: '@查看留言',
    ADD_COURSE: '@新增課程',
    CANCEL_COURSE: '@取消課程',
    COURSE_LIST: '@課程列表',
  ...
  STUDENT: {
    ...
    SHOP: '@活動商城',
    MY_REDEMPTIONS: '@兌換紀錄', // [新增] Phase 2 指令
    BOOK_COURSE: '@預約課程',
    MY_COURSES: '@我的課程',
    CANCEL_BOOKING: '@取消預約',
    CANCEL_WAITING: '@取消候補',
...
async function initializeDatabase() {
  ...
  try {
    ...
    // [修改] 增加 inventory 庫存欄位, status 狀態欄位
    await client.query(`
      CREATE TABLE IF NOT EXISTS products (
        ...
        status VARCHAR(50) DEFAULT 'available', -- 'available' (可兌換) 或 'unavailable' (已下架)
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS product_orders (
        ...
        status VARCHAR(50) NOT NULL DEFAULT 'pending', -- 'pending' (待處理), 'completed' (已完成), 'cancelled' (已取消)
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW() -- [新增] 訂單最後更新時間
      )
    `);
    ...
    // [新增] 檢查 products 表格是否有 inventory 和 status 欄位
    ...
    const statusCol = await client.query("SELECT column_name FROM information_schema.columns WHERE table_name='products' AND column_name='status'");
    if (statusCol.rows.length === 0) await client.query("ALTER TABLE products ADD COLUMN status VARCHAR(50) DEFAULT 'available'");

    // [新增] 檢查 product_orders 表格是否有 updated_at 欄位
    ...
    if (updatedAtCol.rows.length === 0) await client.query('ALTER TABLE product_orders ADD COLUMN updated_at TIMESTAMPTZ DEFAULT NOW()');
...
async function saveProduct(product, dbClient) {
    ...
    const client = dbClient || await pgPool.connect();
    try {
        const { id, name, description, price, image_url, inventory, status, creator_id, creator_name } = product;
        if (id) {
            // 更新現有商品
            await client.query(
                `UPDATE products SET 
                    name = $1, description = $2, price = $3, image_url = $4, 
                    inventory = $5, status = $6 
                 WHERE id = $7`,
                [name, description, price, image_url, inventory, status, id]
            );
        } else {
            // 新增商品
            await client.query(
                `INSERT INTO products 
                    (name, description, price, image_url, inventory, status, creator_id, creator_name) 
...
const cancellableConversationStates = {
    ...
    pendingProductEdit, // [新增]
    pendingInventoryAdjust, // [新增]
};
            ...
            footer: {
                ...
                contents: [
                    { 
                      ...
                      color: '#52B69A',
                      height: 'sm', 
                      action: { type: 'postback', label: '✅ 確認', data: `action=confirm_shop_order&order_uid=${order.order_uid}` } 
                    },
                    { 
                      type: 'button', 
                      style: 'primary',
                      color: '#DE5246',
                      height: 'sm',
                      action: { type: 'postback', label: '❌ 取消', data: `action=cancel_shop_order&order_uid=${order.order_uid}` } 
                    }
                ]
            }
        }));
...
// [新增] Phase 2 - 顯示所有商品供老師管理
async function showAllProductsForManagement(replyToken, page = 1) {
    const offset = (page - 1) * PAGINATION_SIZE;
    const client = await pgPool.connect();
    try {
        ...
        if (pageProducts.length === 0 && page === 1) {
            return reply(replyToken, '目前沒有任何商品，請使用「@上架新商品」來新增。');
        }
        ...
        const productBubbles = pageProducts.map(p => {
            ...
            const statusText = isAvailable ? `狀態: 可兌換` : `狀態: 已下架`;
            const statusColor = isAvailable ? '#52B69A' : '#AAAAAA';
            const toggleStatusLabel = isAvailable ? '下架商品' : '重新上架';
            const toggleStatusAction = isAvailable ? 'unlist' : 'relist';
            ...
            return {
                ...
                body: {
                    ...
                    contents: [
                        ...
                        { type: 'text', text: p.description || ' ', wrap: true, size: 'sm', margin: 'sm', color: '#666666' },
                        { type: 'separator', margin: 'lg' },
                        {
                            ...
                            spacing: 'sm',
                            contents: [
                                { type: 'box', layout: 'baseline', spacing: 'sm', contents: [ { type: 'text', text: '價格', color: '#aaaaaa', size: 'sm', flex: 2 }, { type: 'text', text: `${p.price} 點`, wrap: true, color: '#666666', size: 'sm', flex: 5 } ] },
                                { type: 'box', layout: 'baseline', spacing: 'sm', contents: [ { type: 'text', text: '庫存', color: '#aaaaaa', size: 'sm', flex: 2 }, { type: 'text', text: `${p.inventory} 件`, wrap: true, color: '#666666', size: 'sm', flex: 5 } ] },
                                { type: 'box', layout: 'baseline', spacing: 'sm', contents: [ { type: 'text', text: '狀態', color: '#aaaaaa', size: 'sm', flex: 2 }, { type: 'text', text: statusText, wrap: true, color: statusColor, size: 'sm', flex: 5, weight: 'bold' } ] }
                            ]
                        }
                    ]
                },
                footer: {
                    ...
                    spacing: 'sm',
                    contents: [
                        { type: 'button', style: 'link', height: 'sm', action: { type: 'postback', label: '修改資訊', data: `action=start_product_edit&product_id=${p.id}` } },
                        { type: 'button', style: 'link', height: 'sm', action: { type: 'postback', label: '調整庫存', data: `action=start_inventory_adjust&product_id=${p.id}` } },
                        { type: 'button', style: 'secondary', height: 'sm', color: isAvailable ? '#DE5246' : '#52B69A', action: { type: 'postback', label: toggleStatusLabel, data: `action=toggle_product_status&product_id=${p.id}&new_status=${toggleStatusAction}` } }
                    ]
                }
            };
        });
        
        const paginationBubble = createPaginationBubble('action=manage_products_view', page, hasNextPage);
        if (paginationBubble) {
        ...
        return reply(replyToken, '查詢商品時發生錯誤，請稍後再試。');
    } finally {
        if(client) client.release();
    }
}
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingProductEdit[userId]) {
    ...
    switch (state.step) {
        case 'await_new_value':
            const newValue = text;
            const fieldName = state.fieldToEdit.name;
            const validation = state.fieldToEdit.validation;

            if (validation && !validation.regex.test(newValue)) {
                return reply(replyToken, validation.error, getCancelMenu());
            }
            
            state.product[fieldName] = validation?.isNumber ? parseInt(newValue, 10) : newValue;
            
            // 存檔並結束
            await saveProduct(state.product);
            delete pendingProductEdit[userId];
            await reply(replyToken, `✅ 已成功更新商品「${state.product.name}」的${state.fieldToEdit.label}。`);
            // 顯示更新後的卡片
            return showAllProductsForManagement(replyToken, 1);
  ...
  if (pendingInventoryAdjust[userId]) {
      ...
      if(state.step === 'await_adjustment_value') {
          const adjustment = parseInt(text, 10);
          if (isNaN(adjustment)) {
              return reply(replyToken, '請輸入有效的數字 (可為正數或負數)。', getCancelMenu());
          }
          
          const newInventory = state.product.inventory + adjustment;
          if (newInventory < 0) {
              return reply(replyToken, `調整失敗！庫存不能為負數。\n目前庫存為 ${state.product.inventory}，您試圖調整 ${adjustment}。`, getCancelMenu());
          }
          
          state.product.inventory = newInventory;
          await saveProduct(state.product);
          delete pendingInventoryAdjust[userId];
          
          await reply(replyToken, `✅ 庫存調整成功！\n商品「${state.product.name}」的新庫存為 ${newInventory}。`);
          return showAllProductsForManagement(replyToken, 1);
      }
      return;
  }
  ...
  if (pendingCourseCancellation[userId]) {
    ...
    // pendingProductCreation 流程已整合到 postback 中，此處可留空
    return;
  } else {
    // --- 主要指令路由 ---
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
        ...
        const shopMenu = [ 
            ...
            { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.VIEW_PRODUCTS } }, 
...
// [新增] Phase 2 - 顯示學員的兌換紀錄
async function showMyRedemptions(replyToken, userId, page) {
    const offset = (page - 1) * PAGINATION_SIZE;
    const client = await pgPool.connect();
    try {
        ...
        if (pageOrders.length === 0) {
            return reply(replyToken, '沒有更多兌換紀錄了。');
        }

        const redemptionBubbles = pageOrders.map(order => {
            let statusText, statusColor, statusIcon;
            switch (order.status) {
                case 'completed':
                    statusText = '已完成';
                    statusColor = '#52B69A'; // Green
                    statusIcon = '✅';
                    break;
                case 'pending':
                    statusText = '待處理';
                    statusColor = '#FF9E00'; // Orange
                    statusIcon = '🕒';
                    break;
                case 'cancelled':
                    statusText = '已取消';
                    statusColor = '#DE5246'; // Red
                    statusIcon = '❌';
                    break;
                default:
                    statusText = '未知狀態';
            ...
            return {
                ...
                header: {
                    ...
                    paddingAll: 'lg',
                    contents: [{ type: 'text', text: `${statusIcon} ${statusText}`, color: '#ffffff', weight: 'bold' }]
                },
                body: {
                    type: 'box',
                    layout: 'vertical',
                    ...
                    contents: [
                        ...
                        ...(order.status === 'completed' || order.status === 'cancelled'
                            ? [{ type: 'text', text: `處理時間: ${formatDateTime(order.updated_at)}`, size: 'xs', color: '#aaaaaa' }]
                            : [])
                    ]
                }
            };
        ...
        const paginationBubble = createPaginationBubble('action=view_my_redemptions', page, hasNextPage);
        if (paginationBubble) {
            redemptionBubbles.push(paginationBubble);
        }
        
        const message = {
            type: 'flex',
            altText: '我的兌換紀錄',
            contents: { type: 'carousel', contents: redemptionBubbles }
        };
...
async function handleStudentCommands(event, userId) {
  ...
  if (pendingBookingConfirmation[userId]) {
    ...
    if (!course && !state.type.startsWith('product')) { // 商品流程不需 getCourse
    ...
    switch (state.type) {
        ...
        case 'cancel_wait':
            ...
            break;
        // Phase 1 的 product_purchase 流程由 Postback 直接觸發，不需文字確認
    }
    ...
    } else if (text === COMMANDS.STUDENT.MY_REDEMPTIONS) {
        // [新增] Phase 2 - 呼叫顯示兌換紀錄
        return showMyRedemptions(replyToken, userId, 1);
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V20.0 (商城功能 Phase 2 - 完整管理)`); // [修改] 更新版本號
...
async function handleEvent(event) {
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        const newFlowActions = ['run_command', 'list_teachers_for_removal', 'generate_report', 'add_course_start', 'start_product_edit', 'start_inventory_adjust'];
    ...
    if (!user) {
        try {
            ...
            await saveUser(user);
            await push(userId, `歡迎 ${user.name}！感謝您加入九容瑜伽。現在您可以開始使用我們的服務了，試試點擊下方的「活動商城」或「預約課程」吧！`);
            if (STUDENT_RICH_MENU_ID) await client.linkRichMenuToUser(userId, STUDENT_RICH_MENU_ID);
        } catch (error) {
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        // --- 分頁檢視 ---
        if (action === 'view_course_series') return showCourseSeries(replyToken, page);
        if (action === 'list_teachers_for_removal') return showTeacherListForRemoval(replyToken, page);
        if (action === 'view_pending_orders') return showPendingOrders(replyToken, page);
        if (action === 'student_search_results') return showStudentSearchResults(replyToken, decodeURIComponent(data.get('query') || ''), page);
        if (action === 'view_unread_messages') return showUnreadMessages(replyToken, page);
        if (action === 'view_announcements_for_deletion') return showAnnouncementsForDeletion(replyToken, page);
        if (action === 'view_purchase_history') return showPurchaseHistory(replyToken, userId, page);
        if (action === 'view_available_courses') return showAvailableCourses(replyToken, userId, page);
        if (action === 'view_my_courses') return showMyCourses(replyToken, userId, page);
        if (action === 'view_shop_products') return showShopProducts(replyToken, page);
        if (action === 'view_pending_shop_orders') return showPendingShopOrders(replyToken, page);
        if (action === 'manage_course_group') return showSingleCoursesForCancellation(replyToken, data.get('prefix'), page);
        // --- [新增] Phase 2 分頁檢視 ---
        if (action === 'manage_products_view') return showAllProductsForManagement(replyToken, page);
        if (action === 'view_my_redemptions') return showMyRedemptions(replyToken, userId, page);
        ...
        if (action === 'select_announcement_for_deletion') {
            if (user.role === 'teacher') {
                ...
                if (annRes.rows.length === 0) return reply(replyToken, "找不到該則公告，可能已被刪除。");
                ...
                const menu = [ { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CONFIRM_DELETE_ANNOUNCEMENT, text: COMMANDS.TEACHER.CONFIRM_DELETE_ANNOUNCEMENT } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ];
        ...
        if (action === 'generate_report') {
            ...
            push(userId, `收到指令，正在為您生成 ${title}，請稍候...`);
            (async () => {
                const client = await pgPool.connect();
                try {
                    ...
                    if (reportType === 'course') {
                        ...
                        report += `總開課數：${totalCourses} 堂\n總參與人次：${totalStudents} 人次\n總消耗點數：${pointsConsumed} 點\n\n⭐ 熱門課程 Top 3：\n${top3Courses || '無資料'}`;
        ...
        if (user.role === 'admin') {
            if (action === 'select_teacher_for_removal') {
                ...
                return reply(replyToken, `您確定要移除老師「${targetName}」的權限嗎？該用戶將會變回學員身份。`, [ { type: 'action', action: { type: 'message', label: COMMANDS.ADMIN.CONFIRM_REMOVE_TEACHER, text: COMMANDS.ADMIN.CONFIRM_REMOVE_TEACHER } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ]);
            }
        }
        
        if (user.role === 'teacher') {
          // --- Phase 2: 商品管理 Postback ---
          if (action === 'start_product_edit') {
              const productId = data.get('product_id');
              const field = data.get('field');
              const product = await getProduct(productId);
              if (!product) return reply(replyToken, '找不到此商品。');
              const fieldsToEdit = {
                  name: { label: '商品名稱', prompt: '請輸入新的商品名稱：', value: product.name },
                  description: { label: '商品描述', prompt: '請輸入新的商品描述：', value: product.description },
                  price: { label: '所需點數', prompt: '請輸入新的所需點數(純數字)：', value: product.price, validation: { regex: /^\d+$/, error: '點數必須是正整數。', isNumber: true } },
                  image_url: { label: '商品圖片URL', prompt: '請輸入新的圖片網址(URL)：', value: product.image_url, validation: { regex: /^https?:\/\/.+/, error: '請輸入有效的網址。'} }
              };
              
              if (field && fieldsToEdit[field]) { // 如果是從按鈕選擇了要修改的欄位
                  const fieldToEdit = fieldsToEdit[field];
                  pendingProductEdit[userId] = { step: 'await_new_value', product, fieldToEdit: {name: field, ...fieldToEdit} };
                  setupConversationTimeout(userId, pendingProductEdit, 'pendingProductEdit', u => push(u, '修改商品操作逾時，自動取消。').catch(e=>console.error(e)));
                  return reply(replyToken, `${fieldToEdit.prompt}\n(目前: ${fieldToEdit.value || '無'})`, getCancelMenu());
              } else { // 第一次點擊"修改資訊"，顯示所有可修改欄位的按鈕
                  const editButtons = Object.keys(fieldsToEdit).map(key => ({
                      type: 'button', style: 'link', height: 'sm',
                      action: { type: 'postback', label: `修改 ${fieldsToEdit[key].label}`, data: `action=start_product_edit&product_id=${productId}&field=${key}` }
                  }));
                  const flexMessage = { type: 'flex', altText: `修改 ${product.name}`, contents: { type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: `修改商品：${product.name}`, weight: 'bold', color: 'white' }], backgroundColor: '#1A759F' }, body: { type: 'box', layout: 'vertical', spacing: 'sm', contents: editButtons }, footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } }] } } };
                  return reply(replyToken, flexMessage);
              }
          }
          if (action === 'start_inventory_adjust') {
              const productId = data.get('product_id');
              const product = await getProduct(productId);
              if (!product) return reply(replyToken, '找不到此商品。');
              pendingInventoryAdjust[userId] = { step: 'await_adjustment_value', product };
              ...
              return reply(replyToken, `請輸入「${product.name}」的庫存調整值。\n(例如: 輸入 10 表示增加10, 輸入 -5 表示減少5)\n目前庫存: ${product.inventory}`, getCancelMenu());
          }
          if (action === 'toggle_product_status') {
              const productId = data.get('product_id');
              const newStatus = data.get('new_status') === 'relist' ? 'available' : 'unavailable';
              const product = await getProduct(productId);
              if (!product) return reply(replyToken, '找不到此商品。');
              product.status = newStatus;
              await saveProduct(product);
              const statusText = newStatus === 'available' ? '重新上架' : '下架';
              await reply(replyToken, `✅ 已成功將商品「${product.name}」${statusText}。`);
              return showAllProductsForManagement(replyToken, page);
          }
          // --- Phase 2: 訂單處理 Postback ---
          if (action === 'confirm_shop_order' || action === 'cancel_shop_order') {
                const order_uid = data.get('order_uid');
                const db = await pgPool.connect();
                try {
                    await db.query('BEGIN');
                    const orderRes = await db.query("SELECT * FROM product_orders WHERE order_uid = $1 AND status = 'pending' FOR UPDATE", [order_uid]);
                    if (orderRes.rows.length === 0) {
                        await db.query('ROLLBACK');
                        return reply(replyToken, '找不到此訂單，可能已被其他老師處理。');
                    }
                    const order = orderRes.rows[0];

                    if (action === 'confirm_shop_order') {
                        await db.query("UPDATE product_orders SET status = 'completed', updated_at = NOW() WHERE order_uid = $1", [order_uid]);
                        await db.query('COMMIT');
                        await reply(replyToken, `✅ 訂單 ${order_uid} 已標示為「已完成」。`);
                        push(order.user_id, `🎉 兌換商品通知\n您兌換的「${order.product_name}」已處理完成！\n請隨時可至教室領取。`).catch(e => console.error(e));
                    } else { // cancel_shop_order
                        const studentRes = await db.query("SELECT * FROM users WHERE id = $1 FOR UPDATE", [order.user_id]);
                        const productRes = await db.query("SELECT * FROM products WHERE id = $1 FOR UPDATE", [order.product_id]);
                        
                        const student = studentRes.rows[0];
                        const product = productRes.rows[0];

                        // 退還點數
                        const newPoints = student.points + order.points_spent;
                        const historyEntry = { action: `取消兌換，歸還點數: ${order.product_name}`, pointsChange: +order.points_spent, time: new Date().toISOString(), reason: '老師手動取消' };
                        const newHistory = student.history ? [...student.history, historyEntry] : [historyEntry];
                        await db.query('UPDATE users SET points = $1, history = $2 WHERE id = $3', [newPoints, JSON.stringify(newHistory), order.user_id]);
                        ...
                        await db.query("UPDATE product_orders SET status = 'cancelled', updated_at = NOW() WHERE order_uid = $1", [order_uid]);
                        await db.query('COMMIT');
                        await reply(replyToken, `❌ 訂單 ${order_uid} 已被取消，點數與庫存已歸還。`);
                        push(order.user_id, `❗ 兌換取消通知\n您兌換的「${order.product_name}」已被老師取消。\n花費的 ${order.points_spent} 點已歸還至您的帳戶。`).catch(e => console.error(e));
                    }
                } catch (err) {
                    await db.query('ROLLBACK');
                    console.error(`處理商品訂單 ${order_uid} 時發生錯誤:`, err);
                    return reply(replyToken, '處理訂單時發生嚴重錯誤，操作已取消。');
                } finally {
                    if (db) db.release();
                }
          ...
          if (action === 'set_course_weekday') {
           ...
           if (action === 'cancel_course_group_confirm') {
                ...
                return reply(replyToken, message, [ { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CONFIRM_BATCH_CANCEL, text: COMMANDS.TEACHER.CONFIRM_BATCH_CANCEL } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ]);
            }
            if (action === 'confirm_single_course_cancel') {
                ...
                return reply(replyToken, message, [ { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CONFIRM_SINGLE_CANCEL, text: COMMANDS.TEACHER.CONFIRM_SINGLE_CANCEL } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ]);
            }
        ...
        if (user.role === 'student') {
            if (action === 'select_purchase_plan') {
                ...
                if (selectedPlan) {
                    ...
                    return reply(replyToken, `您選擇了「${selectedPlan.label}」。\n請確認是否購買？`, [ { type: 'action', action: { type: 'message', label: COMMANDS.STUDENT.CONFIRM_BUY_POINTS, text: COMMANDS.STUDENT.CONFIRM_BUY_POINTS } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ]);
                }
            }
            if (action === 'confirm_booking_start') {
                ...
                return reply(replyToken, message, [ { type: 'action', action: { type: 'message', label: COMMANDS.STUDENT.CONFIRM_BOOKING, text: COMMANDS.STUDENT.CONFIRM_BOOKING } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ]);
            }
            if (action === 'confirm_cancel_booking_start') {
                ...
                return reply(replyToken, message, [ { type: 'action', action: { type: 'message', label: COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING, text: COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ]);
            }
            if (action === 'confirm_cancel_waiting_start') {
                ...
                return reply(replyToken, message, [ { type: 'action', action: { type: 'message', label: COMMANDS.STUDENT.CONFIRM_CANCEL_WAITING, text: COMMANDS.STUDENT.CONFIRM_CANCEL_WAITING } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ]);
            }
            if (action === 'confirm_product_purchase') {
                ...
                if (productRes.rows.length === 0) return reply(replyToken, '抱歉，找不到此商品或商品已下架。');
                const product = productRes.rows[0];
                if (product.inventory <= 0) return reply(replyToken, '抱歉，此商品已售完。');
                if (user.points < product.price) return reply(replyToken, `您的點數不足！\n目前點數: ${user.points}\n需要點數: ${product.price}`);
                ...
                const menu = [ { type: 'action', action: { type: 'postback', label: '✅ 確認兌換', data: `action=execute_product_purchase&product_id=${product.id}` } }, { type: 'action', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ];
                return reply(replyToken, message, menu);
            }
            if (action === 'execute_product_purchase') {
                ...
                try {
                    ...
                    await db.query( 'INSERT INTO product_orders (order_uid, user_id, user_name, product_id, product_name, points_spent, updated_at) VALUES ($1, $2, $3, $4, $5, $6, NOW())', [orderUID, userId, user.name, product.id, product.name, product.price] );
                    await db.query('COMMIT');
                    delete pendingBookingConfirmation[userId];
                    await reply(replyToken, `✅ 兌換成功！\n已為您扣除 ${product.price} 點。\n老師將會盡快為您處理訂單，您可隨時至「@兌換紀錄」查看狀態。`);
                    if (TEACHER_ID) {
