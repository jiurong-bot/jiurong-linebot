...
// === 讀寫工具 === function loadJSON(file) { return fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : {}; } function writeJSON(file, data) { fs.writeFileSync(file, JSON.stringify(data, null, 2)); } function generateId() { return id_${Date.now()}_${Math.floor(Math.random() * 10000)}; }

// === 初始資料 === let db = loadJSON(DATA_FILE); let courses = loadJSON(COURSE_FILE); let purchases = loadJSON(PURCHASE_FILE);

const studentMenu = { type: 'template', altText: '選單', template: { type: 'buttons', title: '學員功能選單', text: '請選擇操作項目', actions: [ { type: 'message', label: '查詢課程', text: '@課程查詢' }, { type: 'message', label: '預約課程', text: '@預約' }, { type: 'message', label: '取消課程', text: '@取消課程' }, { type: 'message', label: '購買點數', text: '@購點' }, ], }, };

function replyText(token, text) { return client.replyMessage(token, { type: 'text', text }); }

function replyWithMenu(token, text, menu) { return client.replyMessage(token, [ { type: 'text', text }, menu, ]); }
...
const msg = event.message.text.trim(); const userId = event.source.userId; if (!db[userId]) { db[userId] = { role: 'student', points: 0, history: [] }; writeJSON(DATA_FILE, db); }
