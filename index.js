...
async function getUser(userId, dbClient) {
    ...
    try {
        ...
        // state 欄位從 DB 取出時可能就是物件，但為保險起見，若為字串則解析
...
async function getCourse(courseId, dbClient) {
    ...
    try {
        ...
        return { 
            ...
            waiting: row.waiting || [],
            reminderSent: row.reminder_sent // 包含新的欄位
        };
    } finally {
        if (shouldReleaseClient && client) client.release();
    }
...
async function handleTeacherCommands(event, user) {
  ...
  if(teacherState) {
    ...
    // --- 新增課程流程 ---
    else if (teacherState.name === 'course_creation') {
        const state = teacherState;
        ...
        switch (state.step) {
            case 'await_title':
                user.state = { ...state, step: 'await_weekday', data: { ...state.data, title: text }, timestamp: Date.now() };
                await saveUser(user);
    ...
    // --- 手動調整點數流程 ---
    else if (teacherState.name === 'manual_adjust') {
        // ... (與新增課程類似的邏輯，將 state 操作存入 user.state)
        // ... 此處省略以保持簡潔
        return;
    }
    // ... 其他老師的對話流程 ...
  ...
  if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
      ...
      try {
          ...
          const pageSize = 5; // 每頁顯示5筆
          const totalPages = Math.ceil(totalOrders / pageSize);
          const offset = (page - 1) * pageSize;
          const ordersRes = await client.query(`SELECT * FROM orders WHERE status = 'pending_confirmation' ORDER BY timestamp ASC LIMIT $1 OFFSET $2`, [pageSize, offset]);
          const orderBubbles = ordersRes.rows.map(order => ({ /* ... bubble ... */ }));
          
          if (page < totalPages) {
              orderBubbles.push({ type: 'bubble', body: { type: 'box', layout: 'vertical', justifyContent: 'center', alignItems: 'center', contents: [{ type: 'button', action: { type: 'postback', label: '下一頁', data: `action=view_pending_orders&page=${page + 1}` }, style: 'primary', color: '#AAAAAA' }] } });
          }
...
async function handleStudentCommands(event, user) {
  ...
  if(studentState) {
    if(studentState.name === 'booking_confirmation') {
      // ... (預約/取消預約的對話流程，改為讀寫 user.state)
      return;
    }
    else if(studentState.name === 'feedback') {
      // ... (留言的對話流程，改為讀寫 user.state)
      return;
    }
  }
  ...
  if (text === COMMANDS.STUDENT.CONTACT_US) {
      // ... (此處功能不變，但 postback 的處理會設定 user.state)
  }
  // ... 其他所有學員指令 ...
}
async function checkAndSendReminders() {
    ...
    try {
        ...
        for (const course of upcomingCourses) {
            ...
            for (const studentId of course.students) {
                // 對於已授權 Notify 的使用者，應優先使用 Notify
                // 此處暫時保留 push 作為示範
...
async function handleEvent(event) {
    ...
    if (!event.source.userId) return; // 忽略沒有來源 ID 的事件
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (action === 'view_pending_orders') {
            ...
            try {
                ...
                const orderBubbles = ordersRes.rows.map(order => ({
                    // ... Flex bubble 的 JSON 結構 ...
                }));
                ...
                return reply(replyToken, { type: 'flex', altText: `待確認訂單列表 (第${page}頁)`, contents: { type: 'carousel', contents: orderBubbles } });

            } catch (err) {
                console.error('處理訂單分頁時發生錯誤:', err);
                return reply(replyToken, '讀取訂單時發生錯誤。');
            } finally {
                if (clientDB) clientDB.release();
            }
        }

        // --- 其他 Postback 事件，傳遞給各個 handler ---
        switch(user.role) {
            case 'admin': await handleAdminCommands(event, user); break;
            case 'teacher': await handleTeacherCommands(event, user); break;
            default: await handleStudentCommands(event, user); break;
