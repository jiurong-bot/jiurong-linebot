...
async function handleEvent(event) {
  ...
  if (pendingTeacherLogin[userId]) {
    if (msg === TEACHER_PASSWORD) {
      ...
      writeJSON(DATA_FILE, db);
      return replyQuickReply(event.replyToken, '✅ 已切換為老師身份', teacherMenu);
    } else {
      delete pendingTeacherLogin[userId];
      return replyQuickReply(event.replyToken, '❌ 密碼錯誤，仍為學員身份', studentMenu);
    }
  }
...
function handleStudentCommands(event, userId, msg, user, db, courses) {
  ...
  if (/^預約 /.test(msg)) {
    ...
    if (course.students.length < course.max) {
      ...
      return replyQuickReply(replyToken, '目前已額滿，您已加入候補名單', studentMenu);
    }
  }
...
app.listen(PORT, () => {
  console.log(`九容瑜伽 LINE Bot V3.1 已啟動，監聽埠號：${PORT}`);
});
