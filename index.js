  ...
  COMMANDS: {
    ...
    TEACHER: {
      ...
      POINT_MANAGEMENT: '@點數管理',
        ...
        POINT_ORDER_HISTORY: '@查詢購點紀錄', // [V35.0 新增]
      STUDENT_MANAGEMENT: '@學員管理',
        SEARCH_STUDENT: '@查詢學員',
        VIEW_MESSAGES: '@查看未回覆留言',
        QUERY_HISTORICAL_MESSAGES: '@查詢歷史留言', // [V35.0 修改]
      ...
      SHOP_MANAGEMENT: '@商城管理',
        ...
        SHOP_ORDER_HISTORY: '@查詢商城訂單紀錄', // [V35.0 新增]
      REPORT: '@統計報表',
        COURSE_REPORT: '@課程報表',
        ORDER_REPORT: '@訂單報表',
        POINT_REPORT: '@點數報表',
...
const pendingPointOrderHistorySearch = {}; // [V35.0 新增]
const pendingShopOrderHistorySearch = {}; // [V35.0 新增]
const cancellableConversationStates = {
    ...
    pendingPointOrderHistorySearch, // [V35.0 新增]
    pendingShopOrderHistorySearch, // [V35.0 新增]
};
/**
 * 清除使用者所有待處理的對話狀態。
        ...
        contents: { 
            ...
            body: { 
                ...
                contents: [ 
                    ...
                    // [V35.0 新增]
                    { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '📜 查詢購點紀錄', data: `action=select_point_order_history_view_type` } } 
                ] 
            } 
        } 
    };
        ...
        contents: { 
            ...
            body: { 
                ...
                contents: [ 
                    ...
                    // [V35.0 修改]
                    { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '📜 查詢歷史留言', data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.TEACHER.QUERY_HISTORICAL_MESSAGES)}` } } 
                ] 
            } 
        } 
    };
        ...
        contents: { 
            ...
            body: { 
                ...
                contents: [ 
                    ...
                    // [V35.0 新增]
                    { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '📜 查詢商城訂單紀錄', data: `action=select_shop_order_history_view_type` } }
                ] 
            } 
        } 
    };
...
const teacherCommandMap = {
    ...
    [CONSTANTS.COMMANDS.TEACHER.QUERY_HISTORICAL_MESSAGES]: startMessageSearch, // [V35.0 修改]
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingProductCreation[userId]) {
    ...
    switch (state.step) {
      ...
      case 'await_amount': const amount = parseInt(text, 10);
        if (isNaN(amount) || amount <= 0) { return { type: 'text', text: '點數格式不正確，請輸入一個大於 0 的正整數。', quickReply: { items: getCancelMenu() } }; } state.amount = amount; state.step = 'await_reason'; return { type: 'text', text: '請輸入調整原因（例如：活動獎勵、課程補償等）：', quickReply: { items: getCancelMenu() } };
  ...
  // [V35.0 新增] 處理查詢購點紀錄
  } else if (pendingPointOrderHistorySearch[userId]) {
    const searchQuery = text;
    delete pendingPointOrderHistorySearch[userId];
    const res = await withDatabaseClient(client => 
        client.query(`SELECT id, name, picture_url FROM users WHERE role = 'student' AND (LOWER(name) LIKE $1 OR id = $2) LIMIT 10`, [`%${searchQuery.toLowerCase()}%`, searchQuery])
    );
    if (res.rows.length === 0) {
    ...
    return showStudentSelectionForPointHistory(res.rows, searchQuery);
  // [V35.0 新增] 處理查詢商城訂單紀錄
  } else if (pendingShopOrderHistorySearch[userId]) {
    const searchQuery = text;
    delete pendingShopOrderHistorySearch[userId];
    const res = await withDatabaseClient(client => 
        client.query(`SELECT id, name, picture_url FROM users WHERE role = 'student' AND (LOWER(name) LIKE $1 OR id = $2) LIMIT 10`, [`%${searchQuery.toLowerCase()}%`, searchQuery])
    );
    if (res.rows.length === 0) {
    ...
    return showStudentSelectionForShopHistory(res.rows, searchQuery);
...
// [V35.0 新增] 顯示查詢購點紀錄的學員選項
async function showStudentSelectionForPointHistory(users, originalQuery) {
    const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
    const userBubbles = users.map(u => ({
        type: 'bubble',
        body: {
        ...
        footer: {
            ...
            contents: [{
                ...
                color: '#1A759F',
                height: 'sm',
                action: { type: 'postback', label: '查看購點紀錄', data: `action=view_point_order_history&user_id=${u.id}&page=1` }
            }]
        }
    }));
    return {
...
// [V35.0 新增] 顯示購點歷史紀錄
async function showPointOrderHistory(page, userId = null) {
    const offset = (page - 1) * CONSTANTS.PAGINATION_SIZE;
    return withDatabaseClient(async (client) => {
        let query = "SELECT * FROM orders WHERE amount > 0"; // 只查詢實際購買的
        const queryParams = [];
        let paramIndex = 1;
        ...
        if (pageRows.length === 0 && page === 1) {
            return userId ? '這位學員沒有任何購點紀錄。' : '目前沒有任何購點紀錄。';
        }
        if (pageRows.length === 0) {
            return '沒有更多紀錄了。';
        }
        ...
        const statusMap = {
            ...
            pending_confirmation: { text: '🕒 待確認', color: '#FF9E00' },
            pending_payment: { text: '❗ 待付款', color: '#F28482' },
            rejected: { text: '❌ 已退回', color: '#D90429' }
        };
                ...
                contents: [
                    {
                        ...
                        contents: [
                            ...
                            { type: 'text', text: `+${order.points}點 / ${order.amount}元`, size: 'sm' },
        ...
        const paginationBubble = createPaginationBubble('action=view_point_order_history', page, hasNextPage, customParams);
...
// [V35.0 新增] 顯示查詢商城訂單紀錄的學員選項
async function showStudentSelectionForShopHistory(users, originalQuery) {
    const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
    const userBubbles = users.map(u => ({
        type: 'bubble',
        body: {
        ...
        footer: {
            ...
            contents: [{
                ...
                color: '#1A759F',
                height: 'sm',
                action: { type: 'postback', label: '查看商城訂單', data: `action=view_shop_order_history&user_id=${u.id}&page=1` }
            }]
        }
    }));
    return {
        type: 'flex',
        altText: `請選擇要查詢商城訂單的學員`,
        contents: { type: 'carousel', contents: userBubbles }
    };
}

// [V35.0 新增] 顯示商城訂單歷史紀錄
async function showShopOrderHistory(page, userId = null) {
    const offset = (page - 1) * CONSTANTS.PAGINATION_SIZE;
    return withDatabaseClient(async (client) => {
        ...
        if (pageRows.length === 0 && page === 1) {
            return userId ? '這位學員沒有任何商城訂單紀錄。' : '目前沒有任何商城訂單紀錄。';
        }
        if (pageRows.length === 0) {
            return '沒有更多紀錄了。';
        }
                ...
                contents: [
                    {
                        ...
                        flex: 3,
                        contents: [
                            { type: 'text', text: order.product_name, weight: 'bold', size: 'sm' },
                            { type: 'text', text: `兌換者: ${order.user_name}`, size: 'xs' },
                            { type: 'text', text: `花費: ${order.points_spent}點`, size: 'xs', color: '#666666' },
        ...
        const customParams = userId ? `&user_id=${userId}` : '';
        const paginationBubble = createPaginationBubble('action=view_shop_order_history', page, hasNextPage, customParams);
        const footerContents = paginationBubble ? paginationBubble.body.contents : [];
        
        const headerText = userId ? `${pageRows[0].user_name} 的商城訂單` : '所有商城訂單紀錄';
        return {
            type: 'flex',
            altText: '商城訂單紀錄',
            contents: {
...
async function showShopProducts(page) {
    ...
    return withDatabaseClient(async (client) => {
        ...
        const productBubbles = pageProducts.map(p => {
            ...
            return {
                type: 'bubble',
                hero: (p.image_url && p.image_url.startsWith('https')) ? { type: 'image', url: p.image_url, size: 'full', aspectRatio: '1:1', aspectMode: 'cover' } : undefined,
                body: {
                    type: 'box',
                    layout: 'vertical',
                    contents: [
