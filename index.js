    ...
    switch (stepData.step) {
      ...
      case 5:
        if (text === '確認新增課程') {
          ...
          // 將這個台北時間的 Date 物件轉換為 ISO 字串，
          // 注意：toISOString() 總是回傳 UTC 時間，
          // 為了確保儲存的字串能代表特定時區的時間點，
          // 可以考慮儲存帶時區偏移量的字串，或者僅儲存 UTC 並統一轉換。
          // 最簡潔且不易出錯的做法是儲存 UTC，顯示時才轉。
          // 這裡我們直接儲存 toISOString() 的結果，確保它是一個有效且可被 Date 解析的標準格式。
    ...
    if (course.waiting.length > 0) {
      ...
      if (db[nextUserId] && db[nextUserId].points > 0) {
        ...
        replyMessage += '\n候補學生已遞補。';
          ...
          // 可以考慮通知老師
          const TEACHER_ID = process.env.TEACHER_ID; // 確保你有設定這個環境變數
          if (TEACHER_ID) {
