...
async function initializeDatabase() {
  ...
  try {
    ...
    // --- 資料庫結構自動修正 ---
    const titleCol = await client.query("SELECT column_name FROM information_schema.columns WHERE table_name='announcements' AND column_name='title'");
    if (titleCol.rows.length > 0) {
        await client.query('ALTER TABLE announcements DROP COLUMN title');
        console.log('✅ 已成功為 announcements 表移除舊的 title 欄位。');
    }
...
function formatIdForDisplay(id) {
    ...
    // Insert a zero-width space every 8 characters to break up long IDs
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingCourseCancellation[userId]) {
    ...
    switch (state.step) {
        ...
        case 'await_confirmation':
            if (text === COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT) {
                ...
                delete pendingAnnouncementCreation[userId];
                return reply(replyToken, '✅ 公告已成功發布！學員將在下次查詢時看到。');
            } else {
                return reply(replyToken, '請點擊確認或取消按鈕。');
            }
    }
    ...
    if (text === COMMANDS.TEACHER.ABANDON_DELETE_ANNOUNCEMENT) {
        delete pendingAnnouncementDeletion[userId];
        return reply(replyToken, '已放棄刪除操作。');
    } else if (text === COMMANDS.TEACHER.CONFIRM_DELETE_ANNOUNCEMENT) {
        await pgPool.query('DELETE FROM announcements WHERE id = $1', [state.announcementId]);
        delete pendingAnnouncementDeletion[userId];
    ...
    reply(replyToken, `正在搜尋包含「${searchQuery}」的留言...`).catch(e => console.error(e));
    (async () => {
        try {
            const res = await pgPool.query("SELECT * FROM feedback_messages WHERE user_name LIKE $1 OR message LIKE $1 ORDER BY timestamp DESC LIMIT 10", [`%${searchQuery}%`]);
            if (res.rows.length === 0) {
                return push(userId, `找不到包含「${searchQuery}」的留言。`);
            }
            const messageBubbles = res.rows.map(msg => {
                const headerColor = msg.status === 'replied' ? '#52b69a' : (msg.status === 'read' ? '#6a7d8b' : '#de5246');
                const statusText = msg.status === 'replied' ? '已回覆' : (msg.status === 'read' ? '已讀' : '新留言');
                return { 
                    type: 'bubble', 
                    header: { 
                        type: 'box', 
                    ...
                    body: { 
                        ...
                        spacing: 'md', 
                        contents: [
                            { type: 'text', text: "【學員留言】", weight: 'bold', size: 'sm', color: '#aaaaaa' },
                            { type: 'text', text: msg.message, wrap: true, margin: 'sm' }, 
                            ...(msg.teacher_reply ? [
                                { type: 'separator', margin: 'md' },
                                { type: 'text', text: "【您的回覆】", weight: 'bold', size: 'sm', color: '#aaaaaa', margin: 'md' },
                                { type: 'text', text: msg.teacher_reply, wrap: true, margin: 'sm' }
                            ] : []),
                            { type: 'text', text: `時間: ${formatDateTime(msg.timestamp)}`, size: 'xs', color: '#aaaaaa', margin: 'lg' }
                        ] 
                    },
                    ...(msg.status !== 'replied' ? {
                        footer: { 
                            ...
                            spacing: 'sm', 
                            contents: [
                                { type: 'button', style: 'primary', color: '#1a759f', height: 'sm', action: { type: 'postback', label: '▶️ 回覆此留言', data: `action=reply_feedback&msgId=${msg.id}&userId=${msg.user_id}` } }
                            ] 
                        }
                    } : {})
                };
            });
            await push(userId, { type: 'flex', altText: '留言搜尋結果', contents: { type: 'carousel', contents: messageBubbles } });
        } catch (err) {
            console.error('❌ 搜尋留言失敗:', err);
            await push(userId, '搜尋留言時發生錯誤。');
        }
    })();
    return;
  } else if (pendingProductCreation[userId]) {
    ...
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
                ...
                body: {
                    ...
                    contents: [
                        ...
                        { type: 'separator' },
                        {
                            ...
                            layout: 'vertical',
                            contents: [
                                { type: 'text', text: '查詢留言', weight: 'bold', size: 'md' },
                                { type: 'text', text: '依關鍵字查詢歷史留言。', size: 'sm', color: '#666666', wrap: true, margin: 'md' },
                                { type: 'button', style: 'primary', color: '#6A7D8B', margin: 'md', action: { type: 'message', label: '開始查詢留言', text: COMMANDS.TEACHER.MESSAGE_SEARCH } }
                            ]
                        }
                    ]
                }
                    ...
                    contents: [
                        {
                            ...
                            layout: 'vertical',
                            contents: [
                                { type: 'text', text: '發布新公告', weight: 'bold', size: 'md' },
                                { type: 'text', text: '向所有學員發布一則新的公告訊息。', size: 'sm', color: '#666666', wrap: true, margin: 'md' },
                                { type: 'button', style: 'primary', color: '#DE5246', margin: 'md', action: { type: 'message', label: '開始發布', text: COMMANDS.TEACHER.ADD_ANNOUNCEMENT } }
                            ]
                        },
                        { type: 'separator' },
                        {
                            ...
                            layout: 'vertical',
                            contents: [
                                { type: 'text', text: '管理舊公告', weight: 'bold', size: 'md' },
                                { type: 'text', text: '查看已發布的公告並可選擇刪除。', size: 'sm', color: '#666666', wrap: true, margin: 'md' },
                                { type: 'button', style: 'secondary', margin: 'md', action: { type: 'message', label: '查看與刪除', text: COMMANDS.TEACHER.DELETE_ANNOUNCEMENT } }
                            ]
                        }
                    ]
                }
        ...
        return reply(replyToken, '請輸入要發布的公告內容：', [
            { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CANCEL_ADD_ANNOUNCEMENT, text: COMMANDS.TEACHER.CANCEL_ADD_ANNOUNCEMENT } }
        ]);
    } else if (text === COMMANDS.TEACHER.DELETE_ANNOUNCEMENT) {
        const client = await pgPool.connect();
        try {
            const res = await client.query("SELECT * FROM announcements ORDER BY created_at DESC LIMIT 10");
            if (res.rows.length === 0) {
                return reply(replyToken, '目前沒有任何可刪除的公告。');
            }
            const announcementBubbles = res.rows.map(ann => ({
                type: 'bubble',
                body: {
                    type: 'box',
                    layout: 'vertical',
                ...
                footer: {
                    ...
                    layout: 'vertical',
                    contents: [
                        { type: 'button', style: 'primary', color: '#DE5246', height: 'sm', action: { type: 'postback', label: '刪除此公告', data: `action=confirm_delete_announcement&announcementId=${ann.id}` } }
                    ]
                }
            }));
            return reply(replyToken, { type: 'flex', altText: '請選擇要刪除的公告', contents: { type: 'carousel', contents: announcementBubbles } });
        } catch (err) {
            console.error('❌ 查詢舊公告失敗:', err);
        ...
        setupConversationTimeout(userId, pendingMessageSearchQuery, 'pendingMessageSearchQuery', u => push(u, '查詢留言操作逾時，自動取消。').catch(e => console.error(e)));
        return reply(replyToken, '請輸入想查詢的學員姓名或訊息關鍵字：');
            ...
            if (text === COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING) {
                ...
                try {
                    ...
                    if (newWaiting.length > 0) {
                        ...
                        const promotedUserRes = await client.query('SELECT * FROM users WHERE id = $1 FOR UPDATE', [promotedUserId]);
                        if (promotedUserRes.rows.length > 0) {
                            const promotedUser = promotedUserRes.rows[0];
                            const promotedUserNewPoints = promotedUser.points - course.pointsCost;

                            if (promotedUserNewPoints < 0) {
                                newWaiting.unshift(promotedUserId);
                                newStudents.pop(); 
                                push(promotedUserId, `❗️ 候補失敗通知\n您候補的課程「${course.title}」因點數不足無法自動遞補，請確認您的點數。`).catch(err => console.error(err));
                            } else {
                                const promotedHistoryEntry = { action: `候補成功：${course.title}`, pointsChange: -course.pointsCost, time: new Date().toISOString() };
                                const promotedHistory = promotedUser.history ? [...promotedUser.history, promotedHistoryEntry] : [promotedHistoryEntry];
                                await client.query('UPDATE users SET points = $1, history = $2 WHERE id = $3', [promotedUserNewPoints, JSON.stringify(promotedHistory), promotedUserId]);

                                push(promotedUserId, `🎉 候補成功通知 🎉\n您候補的課程「${course.title}」已有空位，已為您自動預約成功，並扣除 ${course.pointsCost} 點！`).catch(err => console.error(err));
                            }
                        }
                    }
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V15.15 (管理者功能修復)`);
          ...
          if (action === 'manage_course_group') {
           ...
           if (action === 'cancel_course_group_confirm') {
                        ...
                        historyText += history.slice(-10).reverse().map(h => {
                            ...
                            if (actionText.includes('購點入帳')) {
                                actionText = actionText.replace(/(PO|O)(\d{10,})/g, (match, p1, p2) => {
                                    return p1 + formatIdForDisplay(p2);
                                });
                            }
            ...
            if (action === 'confirm_order' || action === 'reject_order') {
                ...
                const isConfirm = action === 'confirm_order';
                
                const client = await pgPool.connect();
                try {
                    await client.query('BEGIN');
                    const orderRes = await client.query("SELECT * FROM orders WHERE order_id = $1 AND status = 'pending_confirmation' FOR UPDATE", [orderId]);
                    if (orderRes.rows.length === 0) {
                        await client.query('ROLLBACK');
                        return reply(replyToken, '此訂單可能已被其他老師處理或狀態已變更。');
                    }
                    const order = orderRes.rows[0];
                    ...
                    if (isConfirm) {
                        ...
                        const historyEntry = { action: `購點入帳：${order.points}點`, pointsChange: +order.points, time: new Date().toISOString(), orderId: order.order_id };
                        const newHistory = student.history ? [...student.history, historyEntry] : [historyEntry];
                        ...
                        await reply(replyToken, `✅ 已確認訂單 ${formatIdForDisplay(orderId)}，並為 ${student.name} 加入 ${order.points} 點。`);
                        push(student.id, `🎉 購點成功！\n您購買的 ${order.points} 點已入帳，目前總點數為 ${newPoints} 點。`).catch(e => console.error(e));

                    } else { // isReject
                        await client.query("UPDATE orders SET status = 'rejected' WHERE order_id = $1", [orderId]);
                        await reply(replyToken, `❌ 已退回訂單 ${formatIdForDisplay(orderId)}。`);
                        push(student.id, `❗️ 訂單退回通知\n您提交的購點申請已被老師退回，請至「點數管理」查看詳情並重新提交。`).catch(e => console.error(e));
                    }
                    ...
                    await client.query('ROLLBACK');
                    console.error(`處理訂單 ${orderId} 失敗:`, e);
                    return reply(replyToken, '處理訂單時發生錯誤，操作已取消。');
                } finally {
                    if (client) client.release();
                }
                return;
            ...
            if (action === 'confirm_delete_announcement') {
                const announcementId = data.get('announcementId');
                const annRes = await pgPool.query("SELECT content FROM announcements WHERE id = $1", [announcementId]);
                if (annRes.rows.length === 0) {
                    return reply(replyToken, "找不到這則公告，可能已被刪除。");
                }
                const content = annRes.rows[0].content.substring(0, 40) + '...';
                pendingAnnouncementDeletion[userId] = { announcementId };
                ...
                return reply(replyToken, `您確定要刪除以下公告嗎？此動作無法復原。\n\n「${content}」`, [
