...
/**
 * [新增] 創建一個包含分頁按鈕的 Flex Message 氣泡。
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingCourseCancellation[userId]) {
    ...
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
        // [修改] 課程管理列表功能將在 postback 中處理，此處僅為範例保留，實際將在第六部分改為 postback
        const client = await pgPool.connect();
        try {
...
app.listen(PORT, async () => {
  checkEnvironmentVariables(); // [新增] 啟動時檢查環境變數
  await initializeDatabase();
  console.log(`✅ 伺服器已啟動，監聽埠號 ${PORT}`);
  console.log(`Bot 版本: V16.1 (分頁與啟動檢查)`);
