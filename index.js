...
async function handleTeacherCommands(event, userId) {
  ...
  // 移除 mainMenuBtn 的定義，因為不再使用

  // 處理學員查詢的第二步
  if (pendingStudentSearchQuery[userId]) {
      // 移除處理 COMMANDS.TEACHER.MAIN_MENU 的邏輯
      const query = text;
  ...
  if (text === COMMANDS.TEACHER.POINT_MANAGEMENT) {
    ...
    return reply(replyToken, flexMessage); // 移除 quickReply 中的 mainMenuBtn
  }
  ...
  if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT || text === COMMANDS.TEACHER.CANCEL_COURSE || text === COMMANDS.TEACHER.COURSE_LIST || text === COMMANDS.TEACHER.ADD_COURSE) {
    ...
    return reply(replyToken, [{ type: 'text', text: introText }, flexMessage]); // 移除 quickReply 中的 mainMenuBtn
  }
...
async function handleStudentCommands(event, userId) {
  ...
  // 移除處理 COMMANDS.STUDENT.MAIN_MENU 的邏輯
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V4.9.28 (移除返回主選單功能)`);
        ...
        if (user.role === 'teacher') {
            if (action === 'add_course_start') {
                ...
                return reply(replyToken, `確定要批次取消所有以 ${prefix} 開頭的課程嗎？此操作無法恢復，已預約學員將退還點數並收到通知。`, [
                    ...
                    // 移除 quickReply 中的取消按鈕，因為不再使用 COMMANDS.TEACHER.COURSE_MANAGEMENT
                ]);
