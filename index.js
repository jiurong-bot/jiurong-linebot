...
function getPurchasePlans() {
  ...
  const plans = CONSTANTS.PURCHASE_PLANS;
  simpleCache.set(cacheKey, plans, 1000 * 60 * 60); // 快取 1 小時
  return plans;
}
...
async function getLatestAnnouncements(limit = 10) {
  ...
  simpleCache.set(cacheKey, rows, 30 * 1000); // 30 秒快取
  return rows;
}
...
const simpleCache = {
  ...
  startCleanup(intervalMs = 60000) {
    ...
    this._cleanupTimer = setInterval(() => {
      const now = Date.now();
      for (const [key, entry] of this._cache.entries()) {
        if (entry.expires < now) this._cache.delete(key);
      }
    }, intervalMs);
  },
  clear(key) {
