// index.js - V20.0 (商城功能 Phase 2 - 訂單管理與紀錄)
...
const COMMANDS = {
  ...
  TEACHER: {
    ...
    SHOP_MANAGEMENT: '@商城管理',
      ADD_PRODUCT: '@上架新商品',
      VIEW_PRODUCTS: '@商品管理', // [修改] 名稱變更
      // REMOVE_PRODUCT: '@下架商品', // [移除] 由商品管理介面取代
...
const pendingProductEdit = {}; // [新增]
const pendingInventoryAdjust = {}; // [新增]
...
async function handleStudentCommands(event, userId) {
  ...
  if (pendingBookingConfirmation[userId]) {
    ...
    } else if (text === COMMANDS.STUDENT.EXCHANGE_HISTORY) { // [新增]
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V20.0 (商城功能 Phase 2 - 訂單管理與紀錄)`); // [修改] 更新版本號
...
async function handleEvent(event) {
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (action === 'view_products') { // for teacher
            return showProductManagementList(replyToken, 1);
        }
        if (action === 'view_pending_shop_orders') { 
            return showPendingShopOrders(replyToken, page);
        }
        if (action === 'view_exchange_history') { // for student
           ...
           if (action === 'cancel_course_group_confirm') {
            ...
            if (action === 'manage_product') {
                ...
                const editMenu = { type: 'flex', altText: `編輯 ${product.name}`, contents: {
                    type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: `編輯: ${product.name}`, color: '#ffffff', weight: 'bold', wrap: true }]},
