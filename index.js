// index.js - V17.3 (新增 postback 事件處理)
...
function checkEnvVariables() {
  console.log("--- DEBUG: Starting Environment Variable Check ---");
  const requiredEnv = [
  ...
  requiredEnv.forEach(envVar => {
      ...
      if (value) {
          // 為安全起見，不直接印出金鑰內容，只顯示狀態
          console.log(`- ✅ Found ${envVar}: (Type: ${typeof value}, Length: ${value.length})`);
      } else {
          console.log(`- ❌ Missing ${envVar}: undefined`);
      }
  });

  const missingEnv = requiredEnv.filter(envVar => !process.env[envVar]);
  if (missingEnv.length > 0) {
      console.error(`--- DEBUG: The following variables are missing: ${missingEnv.join(', ')}. The application would normally exit here. ---`);
      // process.exit(1); // 暫時關閉，以便觀察後續日誌
  } else {
      console.log('--- DEBUG: All required environment variables were found. ---');
  }
  console.log("--- DEBUG: Environment Variable Check Finished ---");
}
// --- 偵錯區塊結束 ---
...
const CONVERSATION_TIMEOUT_MS = 1000 * 60 * 10;
...
// index.js - V17.3
// --- 修改： handleEvent 函式，新增 postback 事件處理邏輯 ---
async function handleEvent(event) {
    ...
    // --- V17.3 核心修改區塊：統一處理指令來源 ---
    let commandText = null;
    let postbackData = null;
    
    if (event.type === 'message' && event.message.type === 'text') {
        commandText = event.message.text.trim();
    } else if (event.type === 'postback' && event.postback.data) {
        postbackData = event.postback.data;
        const params = new URLSearchParams(postbackData);
        commandText = params.get('text');
        
        // 額外處理 postback 帶有特殊指令的情況，例如「選購方案」
        if (!commandText && params.get('action')) {
          commandText = params.get('action');
        }
    }
    // --- 修正結束 ---

    // 處理管理員模式的切換，即使不是文字訊息，也能透過 postback 觸發
    if (userId === ADMIN_USER_ID && commandText === COMMANDS.ADMIN.PANEL) {
    ...
    // 如果沒有可處理的指令，且不是 postback 觸發，則跳過
    if (!commandText) {
      if (event.type === 'message' && event.message.type === 'text') {
          // 處理無法識別的純文字訊息
          await reply(token, '我不懂您的意思耶😕\n您可以試試點擊下方的選單按鈕。');
      }
      return;
    }
    
    // 將 commandText 和 postbackData 傳入個別的處理函式
    ...
    await handler(event, user, commandText, postbackData);
}

// --- 以下所有處理函式的簽名都已更新，多了一個 commandText 參數 ---
async function handlePurchaseFlow(event, user, text) {
...
// --- 修改：handleAdminCommands 函式簽名，並在內部使用傳入的 text ---
async function handleAdminCommands(event, user, text, postbackData) {
...
// --- 修改：handleTeacherCommands 函式簽名，並在內部使用傳入的 text ---
async function handleTeacherCommands(event, user, text, postbackData) {
  const replyToken = event.replyToken;
  const userId = user.id;
  ...
  if (teacherState) {
    ...
    if (teacherState.name === 'student_search') {
      ...
      try {
          ...
          const userBubbles = res.rows.map(u => ({ type: 'bubble', body: { type: 'box', layout: 'horizontal', spacing: 'md', contents: [ { type: 'image', url: u.picture_url || 'https://i.imgur.com/8l1Yd2S.png', size: 'md', aspectRatio: '1:1', aspectMode: 'cover', flex: 1 }, { type: 'box', layout: 'vertical', flex: 3, justifyContent: 'center', contents: [ { type: 'text', text: u.name, weight: 'bold', size: 'lg', wrap: true }, { type: 'text', text: `剩餘 ${u.points} 點`, size: 'sm', color: '#666666', margin: 'md' } ] } ] }, footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '查看詳細資料', data: `action=view_student_details&studentId=${u.id}` } }] } }));
...
// --- 修改：handleStudentCommands 函式簽名，並在內部使用傳入的 text ---
async function handleStudentCommands(event, user, text, postbackData) {
  const replyToken = event.replyToken;
  const userId = user.id;

  if (await handlePurchaseFlow(event, user, text)) {
    return;
  }
  ...
  if (text === COMMANDS.STUDENT.LATEST_ANNOUNCEMENT) {
      ...
      try {
          ...
          const myCourses = myCourses.rows;
...
// ... (接下來是未提供的程式碼，例如: handlePostback 函式、主程式邏輯等)
// index.js - V17.3
// ... (接續上一個回覆)
  } else if (text === COMMANDS.STUDENT.MY_COURSES) {
      const client = await pgPool.connect();
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V17.3 (新增 postback 事件處理)`);
