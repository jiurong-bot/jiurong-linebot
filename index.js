...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingProductEdit[userId]) {
    ...
    switch (state.step) {
      case 'await_content':
        ...
        const confirmMsg = {
          ...
          contents: {
            type: 'bubble',
            body: { type: 'box', layout: 'vertical', spacing: 'lg', contents: [ { type: 'text', text: '請確認公告內容', weight: 'bold', size: 'lg' }, { type: 'separator' }, { type: 'text', text: state.content, wrap: true } ] },
            footer: { type: 'box', layout: 'vertical', spacing: 'sm', contents: [ { type: 'button', style: 'primary', color: '#52b69a', action: { type: 'message', label: COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT, text: COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT } }, { type: 'button', style: 'secondary', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } } ] }
          }
        };
    ...
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
        ...
        const onTimeoutCallback = (u) => { if (pendingStudentSearchQuery[u]) { delete pendingStudentSearchQuery[u]; push(u, '查詢學員逾時，自動取消。').catch(e => console.error(e)); } };
        ...
        const announcementMenu = [ { type: 'action', action: { type: 'message', label: '頒佈新公告', text: COMMANDS.TEACHER.ADD_ANNOUNCEMENT } }, { type: 'action', action: { type: 'message', label: '刪除舊公告', text: COMMANDS.TEACHER.DELETE_ANNOUNCEMENT } }, ];
        ...
        const shopMenu = [ 
            ...
            { type: 'action', action: { type: 'message', label: '📦 管理商品列表', text: COMMANDS.TEACHER.VIEW_PRODUCTS } }, 
        ...
        const orderMenu = [ { type: 'action', action: { type: 'message', label: '處理中的訂單', text: COMMANDS.TEACHER.PENDING_SHOP_ORDERS } } ];
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V20.0 (商城功能 Phase 2 - 完整管理)`);
