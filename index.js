// index.js - V6.3 (優化留言查詢，增加對話逾時自動取消)
...
const CONVERSATION_TIMEOUT_MS = 1000 * 60 * 5; // V6.3: 對話逾時設定為 5 分鐘
...
async function handleTeacherCommands(event, userId) {
  ...
  // V6.3: 處理逾時
  if (pendingReply[userId] || pendingStudentSearchQuery[userId] || pendingMessageSearchQuery[userId] || pendingManualAdjust[userId] || pendingCourseCreation[userId]) {
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V6.3 (優化留言查詢，增加對話逾時自動取消)`);
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (pendingTeacherLogin[userId]) {
            ...
            if (text === TEACHER_PASSWORD) {
                ...
                if (TEACHER_MENU_ID) await client.linkRichMenuToUser(userId, TEACHER_MENU_ID);
            } else {
