...
async function handleTeacherCommands(event, userId) {
                        ...
                        try {
                            ...
                            for (const student of studentsRes.rows) {
                               try {
                                   ...
                                   console.error(`❌ 推播公告給學員 ${student.id} 失敗:`, pushError.message);
                               }
                            }
                            console.log(`📢 公告已成功推播給 ${successCount} / ${studentsRes.rows.length} 位學員。`);
  ...
  // 【問題 5 修正】確保學員查詢能正確獲取並顯示頭像 (邏輯已優化)
  if (pendingStudentSearchQuery[userId]) {
      const query = text;
      ...
      const updatedUsersWithFreshProfiles = await Promise.all(
          foundUsers.map(async (dbUser) => {
              try {
                  ...
                  console.error(`查詢用戶 ${dbUser.id} 最新資料失敗 (可能已封鎖好友):`, e.message);
                  return dbUser; 
              }
          })
      );
  ...
  // 【問題 2 修正】將公告管理選單從輪播(carousel)改為單一卡片條列式
    ...
    const announcementMenu = {
        ...
        contents: {
            ...
            header: { 
                type: 'box', 
                layout: 'vertical', 
                contents: [{ type: 'text', text: '公告管理', color: '#ffffff', weight: 'bold' }], 
  ...
  if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT || text === COMMANDS.TEACHER.CANCEL_COURSE || text === COMMANDS.TEACHER.COURSE_LIST || text === COMMANDS.TEACHER.ADD_COURSE) {
    ...
    for (const prefix of sortedPrefixes) {
        ...
        const courseMainTitle = earliestUpcomingCourse.title; // 課程名稱已在建立時修正，無需再處理
        courseBubbles.push({
...
async function handleStudentCommands(event, userId) {
  ...
  if (text === COMMANDS.STUDENT.CANCEL_PURCHASE) {
    // 如果使用者正處於選擇方案的流程中，直接取消並回覆
    if (pendingPurchase[userId]) {
        delete pendingPurchase[userId];
        return reply(replyToken, '已取消購買點數。', [returnToPointsMenuBtn]);
    }
    
    // 如果沒有正在進行的流程，才檢查資料庫裡是否有可取消的訂單
    const ordersRes = await pgPool.query(`SELECT * FROM orders WHERE user_id = $1 AND (status = 'pending_payment' OR status = 'rejected') ORDER BY timestamp DESC LIMIT 1`, [userId]);
    const pendingOrder = ordersRes.rows[0];
    if (pendingOrder) {
        ...
        try {
            ...
            return reply(replyToken, `已取消您的購點訂單 (ID: ${pendingOrder.order_id})。`, [returnToPointsMenuBtn]);
        } catch (err) {
    ...
    // 若無任何進行中流程，也無資料庫訂單，才顯示此訊息
    return reply(replyToken, '目前沒有待取消的購買流程或訂單。', [returnToPointsMenuBtn]);
  }
