...
app.post('/webhook', line.middleware(config), (req, res) => {
  res.status(200).end(); // 必須 200，避免 LINE 重試
...
function handleStudentCommands(event, userId, msg, user, db, courses) {
  ...
  if (msg === '@預約課程') {
    ...
    const list = allCourses.map(([id, c]) => `${id}: ${c.name} (${c.date})`).join('\n');
  ...
  if (msg === '@我的課程') {
    const my = user.history.map(h => {const c = courses[h.courseId];
    return c
...
function sendRoleSelection(replyToken) {
  ...
  return client.replyMessage(replyToken, {
    ...
    text: '請選擇您的身份',
    quickReply: {
      items: buttons.map(btn => ({ type: 'action', action: btn }))
    }
  });
}
