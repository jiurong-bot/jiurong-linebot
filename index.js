...
// [新增] 設計規範常數 (Design Tokens)
const DESIGN_TOKENS = {
  COLORS: {
    PRIMARY: '#52B69A',      // 主色 (薄荷綠): 確認、主要操作
    SECONDARY: '#1A759F',     // 輔色 (深海藍): 次要資訊、查看詳情
    DESTRUCTIVE: '#D9534F',    // 警示/負向色 (珊瑚紅): 取消、刪除、錯誤
    PENDING: '#FF9E00',       // 狀態色 (橘黃): 等待中、處理中
    DISABLED: '#AAAAAA',      // 禁用/中性色 (灰色): 售完、次要提示
    BACKGROUND: '#343A40',    // 深色背景 (用於標頭)
    WHITE: '#FFFFFF',         // 白色文字
    TEXT_PRIMARY: '#333333',   // 主要內文
    TEXT_SECONDARY: '#666666' // 次要內文
  },
  IMAGES: {
    DEFAULT_HERO: 'https://i.imgur.com/gQkSAnH.jpg' // 通用的課程/商品主圖
  }
};
...
/**
 * [優化] 創建一個包含分頁按鈕的 Flex Message 氣泡。
 * @param {string} baseAction - Postback 的基本動作字串。
 * @param {number} currentPage - 當前頁碼。
 * @param {boolean} hasNext - 是否有下一頁。
 * @param {string} [customParams=''] - 額外參數。
 * @returns {object|null}
 */
function createPaginationBubble(baseAction, currentPage, hasNext, customParams = '') {
    ...
    const buttonStyle = {
        ...
        style: 'link',
        height: 'sm',
        color: DESIGN_TOKENS.COLORS.SECONDARY, // [優化] 使用設計規範顏色
    };
 ...
 */
function createCourseBubble(course, userId) {
    const isBooked = course.students.includes(userId);
    const isWaiting = course.waiting.includes(userId);

    let statusText = `剩餘 ${course.capacity - (course.students?.length || 0)} 名`;
    let footerButton;
    ...
    if (isBooked) {
        ...
        footerButton = {
            ...
            height: 'sm',
            action: { type: 'postback', label: '取消預約', data: `action=confirm_cancel_booking_start&course_id=${course.id}` }
        };
    } else if (isWaiting) {
        statusText = `🕒 候補第 ${course.waiting.indexOf(userId) + 1} 位`;
        footerButton = {
            type: 'button',
            style: 'primary',
            color: DESIGN_TOKENS.COLORS.DESTRUCTIVE,
            height: 'sm',
            action: { type: 'postback', label: '取消候補', data: `action=confirm_cancel_waiting_start&course_id=${course.id}` }
        };
    } else {
        footerButton = {
            type: 'button',
            ...
            height: 'sm',
            action: { type: 'postback', label: '立即預約', data: `action=confirm_booking_start&course_id=${course.id}` }
        };
    }

    const courseMainTitle = course.title.replace(/ - 第 \d+ 堂$/, '');
    const courseSubTitle = course.title.includes(' - 第') ? course.title.replace(courseMainTitle + ' - ', '') : '單堂課程';

    return {
        type: 'bubble',
        hero: { type: 'image', url: DESIGN_TOKENS.IMAGES.DEFAULT_HERO, size: 'full', aspectRatio: '20:13', aspectMode: 'cover' },
        header: {
            type: 'box',
            layout: 'vertical',
            backgroundColor: DESIGN_TOKENS.COLORS.SECONDARY,
            contents: [{ type: 'text', text: courseMainTitle, color: DESIGN_TOKENS.COLORS.WHITE, size: 'lg', weight: 'bold', wrap: true }]
        },
        body: {
            type: 'box',
            layout: 'vertical',
            spacing: 'md',
            contents: [
                { type: 'text', text: courseSubTitle, size: 'xl', weight: 'bold', wrap: true, color: DESIGN_TOKENS.COLORS.TEXT_PRIMARY },
                {
                    type: 'box',
                    layout: 'vertical',
                    margin: 'lg',
                    spacing: 'sm',
                    contents: [
                        { type: 'box', layout: 'baseline', spacing: 'sm', contents: [ { type: 'text', text: '🗓️', flex: 0 }, { type: 'text', text: formatDateTime(course.time), flex: 5, size: 'md', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY } ] },
                        { type: 'box', layout: 'baseline', spacing: 'sm', contents: [ { type: 'text', text: '💎', flex: 0 }, { type: 'text', text: `${course.points_cost} 點`, flex: 5, size: 'md', weight: 'bold', color: DESIGN_TOKENS.COLORS.SECONDARY } ] },
                        { type: 'box', layout: 'baseline', spacing: 'sm', contents: [ { type: 'text', text: '🧘', flex: 0 }, { type: 'text', text: statusText, flex: 5, size: 'md', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY } ] }
                    ]
                }
            ]
        },
...
function createProductBubble(product) {
    ...
    const buttonColor = isSoldOut ? DESIGN_TOKENS.COLORS.DISABLED : DESIGN_TOKENS.COLORS.PRIMARY;
    const buttonAction = isSoldOut
        ? { type: 'message', label: buttonLabel, text: '此商品已售完' }
        : { type: 'postback', label: buttonLabel, data: `action=confirm_product_purchase&product_id=${product.id}` };

    return {
        type: 'bubble',
        hero: (product.image_url && product.image_url.startsWith('https')) ? { type: 'image', url: product.image_url, size: 'full', aspectRatio: '1:1', aspectMode: 'cover' } : undefined,
        body: {
            type: 'box',
            layout: 'vertical',
            spacing: 'md',
            contents: [
                { type: 'text', text: product.name, weight: 'bold', size: 'xl', wrap: true, color: DESIGN_TOKENS.COLORS.TEXT_PRIMARY },
                {
                    type: 'box',
                    layout: 'baseline',
                    margin: 'md',
                    contents: [
                        ...
                        { type: 'text', text: `${product.price} 點`, weight: 'bold', size: 'xxl', color: DESIGN_TOKENS.COLORS.SECONDARY, flex: 4 },
                        { type: 'text', text: `庫存: ${product.inventory}`, size: 'sm', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY, align: 'end', flex: 2 }
                    ]
                },
                { type: 'text', text: product.description || ' ', wrap: true, size: 'md', margin: 'md', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY },
            ]
        },
        footer: {
            type: 'box',
            layout: 'vertical',
            contents: [{ type: 'button', style: 'primary', color: buttonColor, height: 'sm', action: buttonAction }]
        }
    };
}
...
/**
 * [新增] 產生管理介面卡片 (for 老師)
 * @param {object} options - 卡片設定
 * @param {object} options.header - { text, color }
 * @param {Array<object>} options.bodyItems - [{ key, value, isBold? }]
 * @param {Array<object>} options.footerButtons - [{ label, data, style, color }]
 * @returns {object} Flex Message Bubble
 */
function createTeacherManagementBubble({ header, bodyItems, footerButtons }) {
    const bodyContents = bodyItems.map(item => {
        if (item.type === 'separator') return { type: 'separator', margin: 'md' };
        if (item.type === 'text') return { type: 'text', text: item.text, wrap: true, size: item.size || 'sm', color: item.color || DESIGN_TOKENS.COLORS.TEXT_SECONDARY, margin: 'md' };
        
        return {
            type: 'box',
            layout: 'horizontal',
            contents: [
                { type: 'text', text: item.key, flex: 2, color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY, size: 'sm' },
                { type: 'text', text: String(item.value), flex: 4, align: 'end', wrap: true, weight: item.isBold ? 'bold' : 'regular', size: 'sm', color: DESIGN_TOKENS.COLORS.TEXT_PRIMARY }
            ]
        };
    });

    const footerContents = footerButtons.map(btn => ({
        type: 'button',
        style: btn.style || 'primary',
        color: btn.color,
        height: 'sm',
        action: { type: 'postback', label: btn.label, data: btn.data }
    }));
    ...
    return {
        ...
        header: {
            ...
            contents: [{ type: 'text', text: header.text, color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold', size: 'md', wrap: true }]
        },
        body: { type: 'box', layout: 'vertical', spacing: 'md', contents: bodyContents },
        footer: { type: 'box', layout: 'vertical', spacing: 'sm', contents: footerContents }
    };
}
// index.js - V22.0 (介面體驗一致性優化) - Part 2
...
// index.js - V22.0 (介面體驗一致性優化) - Part 3
...
/**
 * [優化] 建立購買點數的 Flex Message，使用設計規範顏色
        ...
        const planItems = [
            {
                ...
                layout: 'horizontal',
                contents: [
                    { type: 'text', text: `${plan.points} 點`, size: 'md', color: DESIGN_TOKENS.COLORS.SECONDARY, flex: 3, gravity: 'center' },
                    { type: 'text', text: `售價：${plan.amount} 元`, size: 'md', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY, align: 'end', flex: 5, gravity: 'center' }
                ]
            },
            {
                type: 'button',
    ...
    return {
        ...
        contents: {
            ...
            header: {
                type: 'box',
                layout: 'vertical',
                contents: [{ type: 'text', text: '請選擇點數方案', weight: 'bold', size: 'md', color: DESIGN_TOKENS.COLORS.WHITE }],
            ...
            body: {
                ...
                contents: [
                    ...plansContent,
                    { type: 'text', text: '購買後請至「點數管理」輸入匯款資訊', size: 'xs', color: DESIGN_TOKENS.COLORS.DISABLED, align: 'center', margin: 'md' }
                ]
            },
            footer: {
                type: 'box',
...
/**
 * [優化] 建立點數管理主選單，使用設計規範顏色
 * @param {string} userId - 使用者ID
 * @returns {Promise<object>} Flex Message
 */
async function buildPointsMenuFlex(userId) {
    ...
    if (pendingOrder) {
        ...
        if (pendingOrder.status === 'pending_confirmation') { actionButtonLabel = '修改匯款後五碼'; actionCmd = COMMANDS.STUDENT.EDIT_LAST5_CARD_TRIGGER; cardTitle = '🕒 匯款已提交，等待確認'; cardColor = DESIGN_TOKENS.COLORS.PENDING; statusText = '已提交五碼，等待老師確認'; }
        else if (pendingOrder.status === 'rejected') { actionButtonLabel = '重新提交匯款後五碼'; actionCmd = COMMANDS.STUDENT.EDIT_LAST5_CARD_TRIGGER; cardTitle = '❌ 訂單被退回！'; cardColor = DESIGN_TOKENS.COLORS.DESTRUCTIVE; statusText = '訂單被老師退回'; additionalInfo = '請檢查匯款金額或後五碼，並重新提交。'; }
        else { actionButtonLabel = '輸入匯款後五碼'; actionCmd = COMMANDS.STUDENT.INPUT_LAST5_CARD_TRIGGER; cardTitle = '❗ 匯款待確認'; cardColor = DESIGN_TOKENS.COLORS.DESTRUCTIVE; statusText = '待付款'; }
        
        const bubbleBodyContents = [
          { type: 'text', text: `訂單 ID: ${formatIdForDisplay(pendingOrder.order_id)}`, weight: 'bold', size: 'md', align: 'center', color: DESIGN_TOKENS.COLORS.TEXT_PRIMARY },
          { type: 'text', text: `購買 ${pendingOrder.points} 點 / ${pendingOrder.amount} 元`, size: 'sm', align: 'center', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY },
          { type: 'text', text: `狀態: ${statusText}`, size: 'sm', align: 'center', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY },
          { type: 'text', text: `後五碼: ${pendingOrder.last_5_digits || '未輸入'}`, size: 'sm', align: 'center', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY },
          ...(additionalInfo ? [{ type: 'text', text: additionalInfo, size: 'xs', align: 'center', color: DESIGN_TOKENS.COLORS.DESTRUCTIVE, wrap: true, margin: 'sm' }] : []),
          { type: 'text', text: `提交時間: ${formatDateTime(pendingOrder.timestamp)}`, size: 'xs', align: 'center', color: DESIGN_TOKENS.COLORS.DISABLED, margin: 'lg' }
        ];

        pointBubbles.push({ type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: cardTitle, color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold', size: 'md' }], backgroundColor: cardColor, paddingAll: 'lg' }, body: { type: 'box', layout: 'vertical', spacing: 'md', justifyContent: 'center', alignItems: 'center', contents: bubbleBodyContents, paddingAll:'xl' }, footer: { type: 'box', layout: 'vertical', spacing: 'sm', contents: [{ type: 'button', style: 'primary', height: 'sm', color: cardColor, action: { type: 'postback', label: actionButtonLabel, data: `action=run_command&text=${actionCmd}` } }, { type: 'button', style: 'secondary', height: 'sm', action: { type: 'message', label: COMMANDS.GENERAL.CANCEL, text: COMMANDS.GENERAL.CANCEL } }] } });
    }

    const createInfoBubble = (title, bgColor, bodyContent, action) => ({
        type: 'bubble',
        header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: title, color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold', size: 'md' }], backgroundColor: bgColor, paddingAll: 'lg' },
        body: { type: 'box', layout: 'vertical', justifyContent: 'center', alignItems: 'center', height: '150px', contents: bodyContent },
        action: action
    });

    pointBubbles.push(createInfoBubble('剩餘點數', DESIGN_TOKENS.COLORS.PRIMARY, [{ type: 'text', text: `${user.points} 點`, weight: 'bold', size: 'xxl', align: 'center' }, { type: 'text', text: `上次查詢: ${new Date().toLocaleTimeString('zh-TW', { timeZone: 'Asia/Taipei', hour12: false })}`, color: DESIGN_TOKENS.COLORS.DISABLED, size: 'xs', align: 'center', margin: 'md' }], { type: 'postback', label: '重新整理', data: `action=run_command&text=${COMMANDS.STUDENT.POINTS}` }));
    pointBubbles.push(createInfoBubble('購買點數', DESIGN_TOKENS.COLORS.SECONDARY, [{ type: 'text', text: '點此選購點數方案', size: 'md', color: DESIGN_TOKENS.COLORS.DISABLED, align: 'center', weight: 'bold' }], { type: 'postback', label: '購買點數', data: `action=run_command&text=${COMMANDS.STUDENT.BUY_POINTS}` }));
    pointBubbles.push(createInfoBubble('購點紀錄', DESIGN_TOKENS.COLORS.BACKGROUND, [{ type: 'text', text: '查詢購買狀態與歷史', size: 'md', color: DESIGN_TOKENS.COLORS.DISABLED, align: 'center', weight: 'bold' }], { type: 'postback', label: '購點紀錄', data: `action=run_command&text=${COMMANDS.STUDENT.PURCHASE_HISTORY}` }));
...
// index.js - V22.0 (介面體驗一致性優化) - Part 4
...
/**
 * [優化] 顯示可供移除的老師列表 (支援分頁)，使用一致的管理介面卡片
...
async function showTeacherListForRemoval(replyToken, page) {
    ...
    const client = await pgPool.connect();
    try {
        const teacherRes = await client.query("SELECT id, name, approved_by, picture_url FROM users WHERE role = 'teacher' ORDER BY name LIMIT $1 OFFSET $2", [PAGINATION_SIZE + 1, offset]);
        ...
        const teacherBubbles = pageTeachers.map(t => createTeacherManagementBubble({
            header: { text: t.name, color: DESIGN_TOKENS.COLORS.BACKGROUND },
            bodyItems: [
              { key: 'User ID', value: formatIdForDisplay(t.id) },
              { key: '授權者 ID', value: t.approved_by ? formatIdForDisplay(t.approved_by) : 'N/A' },
            ],
            footerButtons: [{
                label: '移除此老師',
...
// index.js - V22.0 (介面體驗一致性優化) - Part 5

/**
 * [優化] 顯示待確認的訂單列表，使用一致的管理介面卡片
...
async function showPendingOrders(replyToken, page = 1) {
    ...
    try {
        ...
        const orderBubbles = pageOrders.map(order => createTeacherManagementBubble({
            header: { text: '待確認購點訂單', color: DESIGN_TOKENS.COLORS.PENDING },
            bodyItems: [
                { key: '學員姓名', value: order.user_name },
                { key: '訂單 ID', value: formatIdForDisplay(order.order_id) },
                { key: '購買點數', value: `${order.points} 點`, isBold: true },
                { key: '應付金額', value: `$${order.amount}`, isBold: true },
                { key: '後五碼', value: order.last_5_digits || '未輸入', isBold: true },
                { type: 'separator' },
                { type: 'text', text: `提交時間: ${formatDateTime(order.timestamp)}` }
            ],
            footerButtons: [
                { label: '✅ 確認入帳', data: `action=confirm_order&order_id=${order.order_id}`, color: DESIGN_TOKENS.COLORS.PRIMARY },
                { label: '❌ 退回訂單', data: `action=reject_order&order_id=${order.order_id}`, color: DESIGN_TOKENS.COLORS.DESTRUCTIVE, style: 'secondary' }
            ]
        }));
...
async function showStudentSearchResults(replyToken, query, page = 1) {
    ...
    try {
        ...
        const userBubbles = pageUsers.map(u => ({
            type: 'bubble',
            body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [ { type: 'box', layout: 'horizontal', spacing: 'md', alignItems: 'center', contents: [ { type: 'image', url: u.picture_url || placeholder_avatar, size: 'md', aspectRatio: '1:1', aspectMode: 'cover', flex: 1 }, { type: 'box', layout: 'vertical', flex: 3, contents: [ { type: 'text', text: u.name, weight: 'bold', size: 'lg', wrap: true }, { type: 'text', text: `💎 ${u.points} 點`, size: 'md', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY, margin: 'md' } ] } ] }, { type: 'box', layout: 'vertical', margin: 'lg', contents: [ { type: 'text', text: `User ID: ${u.id}`, size: 'xxs', color: DESIGN_TOKENS.COLORS.DISABLED, wrap: true } ] } ] },
            footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', style: 'primary', color: DESIGN_TOKENS.COLORS.SECONDARY, height: 'sm', action: { type: 'postback', label: '查看詳細資料', data: `action=view_student_details&studentId=${u.id}` } }] }
        }));
...
async function showUnreadMessages(replyToken, page = 1) {
    ...
    try {
        ...
        const messageBubbles = pageMessages.map(msg => {
            ...
            const headerText = isNew ? `新留言 from ${msg.user_name}` : `已讀留言 from ${msg.user_name}`;
            const headerColor = isNew ? DESIGN_TOKENS.COLORS.PENDING : DESIGN_TOKENS.COLORS.BACKGROUND;
            
            let footerButtons = [];
            if (isNew) {
                footerButtons.push({ label: '✅ 標為已讀', data: `action=mark_feedback_read&msgId=${msg.id}`, color: DESIGN_TOKENS.COLORS.PRIMARY, style: 'secondary' });
            }
            footerButtons.push({ label: '▶️ 回覆此留言', data: `action=reply_feedback&msgId=${msg.id}&userId=${msg.user_id}`, color: DESIGN_TOKENS.COLORS.SECONDARY });

            return createTeacherManagementBubble({
              header: { text: headerText, color: headerColor },
              bodyItems: [
                { type: 'text', text: `「${msg.message}」`, wrap: true, size: 'md' },
                { type: 'separator' },
                { type: 'text', text: `時間: ${formatDateTime(msg.timestamp)}`, size: 'xs' }
              ],
...
async function showAnnouncementsForDeletion(replyToken, page = 1) {
    ...
    try {
        ...
        const announcementBubbles = pageAnns.map(ann => createTeacherManagementBubble({
            header: { text: '可刪除公告', color: DESIGN_TOKENS.COLORS.BACKGROUND },
            bodyItems: [
              { type: 'text', text: `「${ann.content}」`, size: 'md', wrap: true, maxLines: 5 },
              { type: 'separator' },
              { type: 'text', text: `由 ${ann.creator_name} 於 ${formatDateTime(ann.created_at)} 發布` }
            ],
            footerButtons: [{
                label: '刪除此公告',
...
async function showCourseSeries(replyToken, page = 1) {
    ...
    try {
        const seriesRes = await client.query(
            `SELECT DISTINCT SUBSTRING(id, 1, 2) as prefix, MIN(time) as earliest_time
             FROM courses
             WHERE time > NOW()
             GROUP BY prefix
        ...
        const addCourseBubble = {
          ...
          body: {
            ...
            contents: [{
              ...
              layout: 'vertical',
              contents: [
                { type: 'text', text: '+', size: 'xxl', weight: 'bold', color: DESIGN_TOKENS.COLORS.DISABLED, align: 'center' },
                { type: 'text', text: '新增課程系列', size: 'md', weight: 'bold', color: DESIGN_TOKENS.COLORS.DISABLED, align: 'center', margin: 'md' }
              ],
              justifyContent: 'center',
              alignItems: 'center',
              height: '150px'
        ...
        const courseBubbles = courseGroups.map(group => createTeacherManagementBubble({
            header: { text: group.mainTitle, color: DESIGN_TOKENS.COLORS.SECONDARY },
            bodyItems: [
                { key: '系列代碼', value: group.prefix, isBold: true },
                { key: '最近堂數', value: formatDateTime(group.earliestTime) },
                { key: '費用', value: `${group.points_cost} 點` },
            ],
            footerButtons: [
                { label: '單次取消', data: `action=manage_course_group&prefix=${group.prefix}`, color: DESIGN_TOKENS.COLORS.PENDING, style: 'secondary' },
                { label: '批次取消', data: `action=cancel_course_group_confirm&prefix=${group.prefix}`, color: DESIGN_TOKENS.COLORS.DESTRUCTIVE, style: 'secondary' },
            ]
        }));
...
async function showPendingShopOrders(replyToken, page = 1) {
    ...
    try {
        ...
        const orderBubbles = pageOrders.map(order => createTeacherManagementBubble({
            header: { text: '待處理商品訂單', color: DESIGN_TOKENS.COLORS.PENDING },
            bodyItems: [
                { key: '學員', value: order.user_name },
                { key: '商品', value: order.product_name, isBold: true },
                { key: '花費', value: `${order.points_spent} 點` },
                { type: 'separator' },
                { type: 'text', text: `訂單ID: ${order.order_uid}` },
                { type: 'text', text: `時間: ${formatDateTime(order.created_at)}`, size: 'xs' }
            ],
            footerButtons: [
                { label: '✅ 確認處理', data: `action=confirm_shop_order&orderUID=${order.order_uid}`, color: DESIGN_TOKENS.COLORS.PRIMARY },
                { label: '❌ 取消訂單', data: `action=cancel_shop_order_start&orderUID=${order.order_uid}`, color: DESIGN_TOKENS.COLORS.DESTRUCTIVE, style: 'secondary' }
            ]
        }));
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingProductCreation[userId]) {
    ...
    if (!proceed && state.step !== 'await_image_url') {
        ...
        const confirmMsg = {
          ...
          contents: {
            ...
            header: {
                type: 'box',
                layout: 'vertical',
                backgroundColor: DESIGN_TOKENS.COLORS.PENDING,
                contents: [{ type: 'text', text: '預覽新公告', color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold' }]
            },
            body: {
              type: 'box',
              layout: 'vertical',
            ...
            footer: {
              ...
              spacing: 'sm',
              contents: [
                { type: 'button', style: 'primary', color: DESIGN_TOKENS.COLORS.PRIMARY, action: { type: 'message', label: COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT, text: COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT } },
...
// index.js - V22.0 (介面體驗一致性優化) - Part 6
  ...
  if (pendingCourseCancellation[userId]) {
    ...
    switch (state.step) {
      case 'await_student_search':
        ...
        const userBubbles = res.rows.map(u => ({
            type: 'bubble',
            body: { type: 'box', layout: 'horizontal', spacing: 'md', contents: [ { type: 'image', url: u.picture_url || placeholder_avatar, size: 'md', aspectRatio: '1:1', aspectMode: 'cover' }, { type: 'box', layout: 'vertical', flex: 3, justifyContent: 'center', contents: [ { type: 'text', text: u.name, weight: 'bold', size: 'lg', wrap: true }, { type: 'text', text: `ID: ${u.id}`, size: 'xxs', color: DESIGN_TOKENS.COLORS.DISABLED, margin: 'sm', wrap: true } ] } ] },
            footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', style: 'primary', color: DESIGN_TOKENS.COLORS.SECONDARY, height: 'sm', action: { type: 'postback', label: '選擇此學員', data: `action=select_student_for_adjust&studentId=${u.id}` } }] }
        }));
    ...
    if (text === COMMANDS.TEACHER.ADD_PRODUCT) {
        ...
        try {
            ...
            const cardColor = pendingCount > 0 ? DESIGN_TOKENS.COLORS.DESTRUCTIVE : DESIGN_TOKENS.COLORS.PENDING;
            ...
            const flexMessage = {
              ...
              contents: {
                type: 'carousel',
                contents: [
                  { type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: cardTitle, color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold', size: 'lg' } ], backgroundColor: cardColor }, body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [ { type: 'text', text: '審核學員的購點申請，確認匯款資訊並為其加點。', wrap: true, size: 'sm', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY } ] }, footer: { type: 'box', layout: 'vertical', contents: [ { type: 'button', action: { type: 'message', label: '查看清單', text: COMMANDS.TEACHER.PENDING_ORDERS }, style: 'primary', color: cardColor } ] } },
                  { type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: '手動調整點數', color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold', size: 'lg' } ], backgroundColor: DESIGN_TOKENS.COLORS.SECONDARY }, body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [ { type: 'text', text: '用於特殊情況(如活動獎勵、課程補償)，直接為學員增減點數。', wrap: true, size: 'sm', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY } ] }, footer: { type: 'box', layout: 'vertical', contents: [ { type: 'button', action: { type: 'message', label: '開始調整', text: COMMANDS.TEACHER.MANUAL_ADJUST_POINTS }, style: 'primary', color: DESIGN_TOKENS.COLORS.SECONDARY } ] } }
                ]
              }
            };
        ...
        const flexMessage = {
            ...
            contents: {
                type: 'bubble',
                header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: '學員管理', color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold', size: 'lg' }], backgroundColor: DESIGN_TOKENS.COLORS.BACKGROUND },
                body: { type: 'box', layout: 'vertical', spacing: 'lg', contents: [ { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: '查詢學員', weight: 'bold', size: 'md' }, { type: 'text', text: '依姓名或ID查詢學員的詳細資料與點數紀錄。', size: 'sm', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY, wrap: true, margin: 'md' }, { type: 'button', style: 'primary', color: DESIGN_TOKENS.COLORS.BACKGROUND, margin: 'md', action: { type: 'message', label: '開始查詢', text: COMMANDS.TEACHER.SEARCH_STUDENT } } ] }, { type: 'separator' }, { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: '查看留言', weight: 'bold', size: 'md' }, { type: 'text', text: '查看並回覆學員的留言。', size: 'sm', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY, wrap: true, margin: 'md' }, { type: 'button', style: 'primary', color: DESIGN_TOKENS.COLORS.BACKGROUND, margin: 'md', action: { type: 'message', label: '查看未回覆留言', text: COMMANDS.TEACHER.VIEW_MESSAGES } } ] } ] }
            }
        };
        return reply(replyToken, flexMessage);
        ...
        const flexMessage = {
          ...
          contents: {
            ...
            header: {
              type: 'box',
              layout: 'vertical',
              contents: [{ type: 'text', text: '🛍️ 商城管理', weight: 'bold', size: 'lg', color: DESIGN_TOKENS.COLORS.WHITE }],
              backgroundColor: DESIGN_TOKENS.COLORS.BACKGROUND,
              paddingBottom: 'none'
            },
            body: {
              ...
              spacing: 'lg',
              contents: [
                { type: 'text', text: '— 商品管理 —', color: DESIGN_TOKENS.COLORS.DISABLED, size: 'sm', align: 'center' },
                { type: 'button', style: 'secondary', height: 'sm', action: { type: 'message', label: '➕ 上架新商品', text: COMMANDS.TEACHER.ADD_PRODUCT } },
                { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '🛒 管理販售中商品', data: 'action=view_products&filter=available' } },
                { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '📦 管理已下架商品', data: 'action=view_products&filter=unavailable' } },
                { type: 'separator' },
                { type: 'text', text: '— 訂單管理 —', color: DESIGN_TOKENS.COLORS.DISABLED, size: 'sm', align: 'center' },
                { type: 'button', style: 'secondary', height: 'sm', action: { type: 'message', label: '📋 查看待處理訂單', text: COMMANDS.TEACHER.PENDING_SHOP_ORDERS } }
              ]
            }
          }
        };
        ...
        const reportMenu = {
            ...
            contents: {
                type: 'bubble',
                header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: '📊 統計報表', weight: 'bold', size: 'lg', color: DESIGN_TOKENS.COLORS.WHITE }], backgroundColor: DESIGN_TOKENS.COLORS.BACKGROUND },
        ...
        const timePeriodMenu = {
            ...
            contents: {
                type: 'bubble',
                header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: `📊 ${title}`, weight: 'bold', size: 'lg', color: DESIGN_TOKENS.COLORS.WHITE }], backgroundColor: DESIGN_TOKENS.COLORS.PRIMARY },
                body: { type: 'box', layout: 'vertical', spacing: 'sm', contents: [ { type: 'button', style: 'link', height: 'sm', action: { type: 'postback', label: '本週', data: `action=generate_report&type=${reportType}&period=week` } }, { type: 'button', style: 'link', height: 'sm', action: { type: 'postback', label: '本月', data: `action=generate_report&type=${reportType}&period=month` } }, { type: 'button', style: 'link', height: 'sm', action: { type: 'postback', label: '本季', data: `action=generate_report&type=${reportType}&period=quarter` } }, { type: 'button', style: 'link', height: 'sm', action: { type: 'postback', label: '今年', data: `action=generate_report&type=${reportType}&period=year` } }, ] },
                 footer: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: '請選擇要查詢的時間區間', size: 'sm', color: DESIGN_TOKENS.COLORS.DISABLED, align: 'center'}] }
            }
        };
...
// index.js - V22.0 (介面體驗一致性優化) - Part 7

/**
 * [優化] 顯示歷史購買紀錄，使用一致的管理介面卡片
 * @param {string} replyToken The reply token.
...
async function showPurchaseHistory(replyToken, userId, page) {
    ...
    try {
        ...
        const historyBubbles = pageOrders.map(order => {
            ...
            switch (order.status) {
                case 'completed': statusText = '✅ 已完成'; statusColor = DESIGN_TOKENS.COLORS.PRIMARY; break;
                case 'pending_confirmation': statusText = '🕒 等待確認'; statusColor = DESIGN_TOKENS.COLORS.PENDING; break;
                case 'pending_payment': statusText = '❗ 等待付款'; statusColor = DESIGN_TOKENS.COLORS.DESTRUCTIVE; break;
                case 'rejected': statusText = '❌ 已退回'; statusColor = DESIGN_TOKENS.COLORS.DESTRUCTIVE; break;
                default: statusText = '未知狀態'; statusColor = DESIGN_TOKENS.COLORS.DISABLED;
            }

            return {
                type: 'bubble',
                header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: statusText, color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold' }], backgroundColor: statusColor, paddingAll: 'lg' },
                body: {
                  type: 'box',
                  layout: 'vertical',
                  spacing: 'md',
                  contents: [
                    ...
                    { type: 'text', text: `金額: ${order.amount} 元`, size: 'sm', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY },
                    { type: 'text', text: `後五碼: ${order.last_5_digits || 'N/A'}`, size: 'sm', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY },
                    { type: 'separator', margin: 'lg' },
                    { type: 'text', text: `訂單ID: ${formatIdForDisplay(order.order_id)}`, size: 'xxs', color: DESIGN_TOKENS.COLORS.DISABLED, wrap: true },
                    { type: 'text', text: `時間: ${formatDateTime(order.timestamp)}`, size: 'xs', color: DESIGN_TOKENS.COLORS.DISABLED }
                  ]
                }
            };
        });
...
/**
 * [優化] 顯示可預約的課程，使用 createCourseBubble
 * @param {string} replyToken The reply token.
 * @param {string} userId The user's ID.
 * @param {number} page The page number.
 */
async function showAvailableCourses(replyToken, userId, page) {
    ...
    try {
        ...
        const courseBubbles = pageCourses.map(c => createCourseBubble(c, userId));
...
/**
 * [優化] 顯示我的課程，使用 createCourseBubble
 * @param {string} replyToken The reply token.
 * @param {string} userId The user's ID.
 * @param {number} page The page number.
 */
...
/**
 * [優化] 顯示商城商品，使用 createProductBubble
 * @param {string} replyToken The reply token.
 * @param {number} page The page number.
 */
async function showShopProducts(replyToken, page) {
    ...
    try {
        ...
        const productBubbles = pageProducts.map(p => createProductBubble(p));
...
async function showSingleCoursesForCancellation(replyToken, prefix, page) {
    ...
    try {
        ...
        const courseBubbles = pageCourses.map(c => createTeacherManagementBubble({
            header: { text: '選擇要取消的單堂課', color: DESIGN_TOKENS.COLORS.BACKGROUND },
            bodyItems: [
              { key: '課程', value: c.title, isBold: true },
              { key: '時間', value: formatDateTime(c.time) },
              { key: '已預約', value: `${c.students.length} / ${c.capacity}` }
            ],
            footerButtons: [{
                label: '取消此堂課',
                data: `action=confirm_single_course_cancel&course_id=${c.id}`,
                color: DESIGN_TOKENS.COLORS.DESTRUCTIVE
            }]
        }));
...
/**
 * [優化] 顯示商品管理列表 (for Teacher)，使用一致的管理介面卡片
...
async function showProductManagementList(replyToken, page = 1, filter = null) {
    ...
    try {
        ...
        const productBubbles = pageProducts.map(p => {
            ...
            const toggleColor = p.status === 'available' ? DESIGN_TOKENS.COLORS.PENDING : DESIGN_TOKENS.COLORS.PRIMARY;

            return createTeacherManagementBubble({
                header: {
                    text: p.name,
                    color: p.status === 'available' ? DESIGN_TOKENS.COLORS.PRIMARY : DESIGN_TOKENS.COLORS.DISABLED
                },
                bodyItems: [
                    { type: 'text', text: p.description || '無描述', size: 'sm', wrap: true },
                    { type: 'separator' },
                    { key: '價格', value: `${p.price} 點`, isBold: true },
                    { key: '庫存', value: p.inventory, isBold: true },
                ],
                footerButtons: [
                    { label: '✏️ 編輯資訊', data: `action=manage_product&product_id=${p.id}`, color: DESIGN_TOKENS.COLORS.SECONDARY },
                    { label: '📦 調整庫存', data: `action=adjust_inventory_start&product_id=${p.id}`, color: DESIGN_TOKENS.COLORS.SECONDARY, style: 'secondary' },
                    { label: toggleLabel, data: toggleAction, color: toggleColor, style: 'secondary' }
                ]
            });
        });
...
async function showStudentExchangeHistory(replyToken, userId, page = 1) {
    ...
    try {
        ...
        const historyBubbles = pageOrders.map(order => {
            let statusText, statusColor;
            switch (order.status) {
                case 'completed': statusText = '✅ 已完成/可領取'; statusColor = DESIGN_TOKENS.COLORS.PRIMARY; break;
                case 'pending': statusText = '🕒 處理中'; statusColor = DESIGN_TOKENS.COLORS.PENDING; break;
                case 'cancelled': statusText = '❌ 已取消'; statusColor = DESIGN_TOKENS.COLORS.DESTRUCTIVE; break;
                default: statusText = '未知狀態'; statusColor = DESIGN_TOKENS.COLORS.DISABLED;
            }
            return {
                type: 'bubble',
                ...
                body: {
                  ...
                  contents: [
                    ...
                    { type: 'text', text: `花費: ${order.points_spent} 點`, size: 'sm', color: DESIGN_TOKENS.COLORS.TEXT_SECONDARY },
                    { type: 'separator', margin: 'lg' },
                    { type: 'text', text: `訂單ID: ${formatIdForDisplay(order.order_uid)}`, size: 'xxs', color: DESIGN_TOKENS.COLORS.DISABLED, wrap: true },
                    { type: 'text', text: `兌換時間: ${formatDateTime(order.created_at)}`, size: 'xs', color: DESIGN_TOKENS.COLORS.DISABLED },
                    ...(order.status === 'completed' && order.updated_at ? [{ type: 'text', text: `完成時間: ${formatDateTime(order.updated_at)}`, size: 'xs', color: DESIGN_TOKENS.COLORS.DISABLED }] : [])
                ]}
            };
        });
...
async function handleStudentCommands(event, userId) {
  ...
  if (pendingBookingConfirmation[userId]) {
    ...
    const course = state.type !== 'product_purchase' ? await getCourse(state.course_id) : null;
    ...
    if (text === COMMANDS.STUDENT.LATEST_ANNOUNCEMENT) {
        ...
        try {
            ...
            const announcementMessage = { type: 'flex', altText: '最新公告', contents: { type: 'bubble', header: { type: 'box', layout: 'vertical', backgroundColor: DESIGN_TOKENS.COLORS.DESTRUCTIVE, contents: [ { type: 'text', text: '‼️ 最新公告', color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold', size: 'lg' } ]}, body: { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: announcement.content, wrap: true } ]}, footer: { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: `由 ${announcement.creator_name} 於 ${formatDateTime(announcement.created_at)} 發布`, size: 'xs', color: DESIGN_TOKENS.COLORS.DISABLED, align: 'center' } ]} } };
...
// index.js - V22.0 (介面體驗一致性優化) - Part 8
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V22.0 (介面體驗一致性優化)`); // [修改] 更新版本號
           ...
           if (action === 'cancel_course_group_confirm') {
            ...
            if (action === 'manage_product') {
                ...
                const editMenu = { type: 'flex', altText: `編輯 ${product.name}`, contents: {
                    type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: `編輯: ${product.name}`, color: DESIGN_TOKENS.COLORS.WHITE, weight: 'bold', wrap: true }], backgroundColor: DESIGN_TOKENS.COLORS.BACKGROUND},
