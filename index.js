...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingManualAdjust[userId]) {
    ...
    switch (state.step) {
      case 'await_student_search':
        ...
        const userBubbles = res.rows.map(u => ({ type: 'bubble', body: { type: 'box', layout: 'horizontal', spacing: 'md', contents: [ { type: 'image', url: u.picture_url || placeholder_avatar, size: 'md', aspectRatio: '1:1', aspectMode: 'cover', cornerRadius: '50%' }, { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: u.name, weight: 'bold', size: 'lg', wrap: true } ] } ] }, footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', style: 'primary', color: '#1A759F', height: 'sm', action: { type: 'postback', label: '選擇此學員', data: `action=select_student_for_adjust&studentId=${u.id}` } }] } }));
  ...
  // Other conversation flows... (Unchanged)
  ...
  // Other command handlers... (Unchanged, using the corrected async pattern)
