// index.js - V19.0 (商城功能 Phase 1 - 核心兌換)
...
const COMMANDS = {
  ...
  TEACHER: {
    ...
    SHOP_MANAGEMENT: '@商城管理',
      ...
      REMOVE_PRODUCT: '@下架商品',
      SHOP_ORDER_MANAGEMENT: '@訂單管理', // [新增]
      PENDING_SHOP_ORDERS: '@待處理訂單', // [新增]
    VIEW_MESSAGES: '@查看留言',
    ADD_COURSE: '@新增課程',
    CANCEL_COURSE: '@取消課程',
    COURSE_LIST: '@課程列表',
...
async function initializeDatabase() {
  ...
  try {
    ...
    // [新增] 檢查 products 表格是否有 inventory 欄位，若無則新增
...
async function showPendingShopOrders(replyToken, page = 1) {
    ...
    try {
        ...
        const orderBubbles = pageOrders.map(order => ({
            ...
            //     layout: 'vertical',
            //     contents: [
            //         { type: 'button', style: 'primary', action: { label: '✅ 確認出貨', data: `...` } }
            //     ]
            // }
        }));
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingCourseCancellation[userId]) {
    ...
    // [修改] 由於 Phase 1 尚未實作新增商品流程，此處暫時留空
    // 待 Phase 2 加入 pendingProductCreation 的完整流程
    return;
  } else {
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V19.0 (商城功能 Phase 1 - 核心兌換)`); // [修改] 更新版本號
...
async function handleEvent(event) {
    ...
    if (event.type === 'message' && event.message.type === 'text' && event.message.text.trim() === COMMANDS.GENERAL.CANCEL) {
        ...
        if (wasCleared) {
            return reply(event.replyToken, '已取消先前的操作。');
             ...
             // 如果在 student 的購點流程中，即使沒有 pending state，也可能要處理取消訂單
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (action === 'view_pending_shop_orders') { // [新增]
