...
const teacherMenu = [
  ...
  { type: 'message', label: '加點/扣點', text: '@加點 userId 數量' }, // 此功能目前未實作
  { type: 'message', label: '查學員', text: '@查學員' }, // 此功能目前未實作
  { type: 'message', label: '報表', text: '@統計報表' }, // 此功能目前未實作
  { type: 'message', label: '切換身份', text: '@切換身份' },
];
...
// ⏰ 課程時間格式化（轉台北時間並顯示）- 優化版本
function formatDateTime(isoString) {
  if (!isoString) return '無效時間';
  const date = new Date(isoString); // 這裡 Date 物件會自動解析 ISO 字串
  ...
  const weekday = parts.find(p => p.type === 'weekday').value; // 例如 "週一"
  const hour = parts.find(p => p.type === 'hour').value;
  const minute = parts.find(p => p.type === 'minute').value;

  // 提取星期中的單一字元，例如 "週一" 變成 "一"
  const displayWeekday = weekday.length === 2 ? weekday.slice(-1) : weekday;
    ...
    switch (stepData.step) {
      ...
      case 5:
        if (text === '確認新增課程') {
          ...
          const today = new Date(); // 當前日期時間 (取本地時區)

          const todayWeekday = today.getDay(); // 0 for Sunday, 1 for Monday...
          const targetWeekday = weekdays.indexOf(stepData.data.weekday); // 找到目標星期在陣列中的索引

          const [targetHour, targetMin] = stepData.data.time.split(':').map(Number);

          let dayDiff = (targetWeekday - todayWeekday + 7) % 7;

          // 如果是同一天，判斷時間是否已過。如果已過，則設定為下週
          if (dayDiff === 0) {
            if (today.getHours() > targetHour || (today.getHours() === targetHour && today.getMinutes() >= targetMin)) {
              dayDiff = 7; // 設定為下週
            }
          }

          // 創建一個新的 Date 物件，設定為目標日期和時間
          const targetDate = new Date(today); // 從當前日期開始
          targetDate.setDate(today.getDate() + dayDiff); // 加上日期差
          targetDate.setHours(targetHour, targetMin, 0, 0); // 設定小時、分鐘、秒、毫秒

          // 將時間轉換為 ISO 8601 格式的 UTC 字串，用於儲存
          // 這是最推薦的儲存方式，因為它不依賴於伺服器時區
    ...
    if (course.waiting.length > 0) {
      ...
      if (db[nextUserId] && db[nextUserId].points > 0) {
        ...
        replyMessage += '\n候補學生已遞補。';
          ...
          replyMessage += `\n候補學生 ${db[nextUserId].name} (ID: ${nextUserId}) 點數不足，未能遞補。已從候補名單移除。`;
          // 可以考慮通知老師
          client.pushMessage(TEACHER_ID, { // 你可能需要一個 TEACHER_ID 或通知所有老師
              type: 'text',
              text: `⚠️ 課程「${course.title}」有學生取消，但候補學生 ${db[nextUserId].name} (ID: ${nextUserId}) 點數不足，未能遞補。`
