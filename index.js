...
async function handleTeacherCommands(event, userId) {
  ...
  } else if (pendingAnnouncementCreation[userId] || pendingReply[userId] || pendingStudentSearchQuery[userId] || pendingMessageSearchQuery[userId] || pendingProductCreation[userId]) {
    // This is a placeholder for other pending states to ensure the else-if structure is complete.
    // The actual logic for these states would be here.
    return;
  } else {
...
async function handleStudentCommands(event, userId) {
  ...
  } else if (await handlePurchaseFlow(event, userId)) {
    return;
  } else {
