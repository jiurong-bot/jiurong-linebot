...
async function initializeDatabase() {
  try {
    ...
    // V7.1 修改點: users 表增加 picture_url 欄位
...
// V7.1 修改點: 存取 user 物件時加入 pictureUrl
...
async function reply(replyToken, content, menu = null) {
  ...
  if (menu !== null && menu !== undefined) {
      ...
      const validMenuItems = menuItems.slice(0, 13).map(item => {
          ...
          // 如果是 postback action (舊格式或新格式)
          if ((item.type === 'action' && item.action.type === 'postback') || (item.label && item.data)) {
              return {
                  type: 'action',
                  action: {
                      type: 'postback',
                      label: item.label || item.action.label,
                      data: item.data || item.action.data,
                      displayText: item.displayText || item.action.displayText
                  }
              };
          }
          return null; // 過濾掉不符合格式的項目
      }).filter(Boolean);
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingStudentSearchQuery[userId]) {
      ...
      if (foundUsers.length === 0) {
          ...
          return reply(replyToken, [{ type: 'text', text: `找到 ${foundUsers.length} 位符合的學員：` }, { type: 'flex', altText: '請選擇學員', contents: { type: 'carousel', contents: userBubbles.slice(0, 10) } }]);
      }
  }
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V7.1 (查詢學員時顯示頭像)`);
...
async function handleEvent(event) {
    ...
    // V7.1 修改點: 建立新使用者時，一併取得並儲存 pictureUrl
    if (!user) {
      try {
        ...
        user = { 
            ...
            pictureUrl: profile.pictureUrl // 儲存頭像網址
        };
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (action === 'start_student_search') { 
          ...
          return reply(replyToken, '請輸入要查詢的學員姓名或 ID（支援模糊篩選）：'); 
            ...
            // V7.1: 使用與單一查詢結果相同的 Flex Message 格式
            const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
            const historyRecords = (foundUser.history?.length > 0) 
            ...
            const detailFlex = {
                ...
                contents: {
                    type: 'bubble',
                    hero: { type: 'image', url: foundUser.picture_url || placeholder_avatar, size: 'full', aspectRatio: '1:1', aspectMode: 'cover' },
                    body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [
                        { type: 'text', text: foundUser.name, weight: 'bold', size: 'xl' },
                        { type: 'box', layout: 'vertical', spacing: 'sm', margin: 'lg', contents: [
                            { type: 'box', layout: 'baseline', spacing: 'sm', contents: [
