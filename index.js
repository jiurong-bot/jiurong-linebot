    ...
    try {
        ...
        if (pendingOrder) {
            ...
            if (pendingOrder.status === 'pending_payment' || pendingOrder.status === 'rejected') {
                ...
                return reply(replyToken, [{ type: 'text', text: '已取消您的待付款訂單。' }, flexMenu]);
            }
        }
        ...
        console.error('❌ handleCancel 執行失敗:', err);
        // [問題2]
        return reply(replyToken, '取消操作時發生錯誤，請稍後再試。');
    } finally {
        if (client) client.release();
    }
}
...
async function handlePurchaseFlow(event, userId) {
    ...
    // [問題3] 取消操作已由 handleCancel 集中處理，此處僅處理流程中非取消的輸入
    if (text === COMMANDS.GENERAL.CANCEL) {
        // 此處的 cancel 主要是為了處理 postback button 的情況，文字輸入的取消會被 handleEvent 優先攔截
        await handleCancel(userId, event.replyToken);
        return true;
    }
...
async function handleStudentCommands(event, userId) {
  ...
  if (pendingBookingConfirmation[userId]) {
    ...
    // [問題3] 統一由 handleCancel 處理，此處僅處理確認操作
    switch (state.type) {
        case 'book':
            if (text === COMMANDS.STUDENT.CONFIRM_BOOKING) {
                ...
                try {
                    ...
                    const currentUser = await getUser(userId, client); // 重新獲取最新的 user history
                 ...
                 return reply(replyToken, '請點擊按鈕確認或取消。', getCancelMenu());
            }
            break;
    ...
    // [問題3] 取消已由 handleCancel 處理
    if (pendingFeedback[userId].step === 'await_message') {
    ...
    try {
        if (text === COMMANDS.STUDENT.LATEST_ANNOUNCEMENT) {
          ...
          return pushPointsMenu(userId).then(() => reply(replyToken, [])).catch(err => { if (!err.message.includes('Invalid reply token')) console.error(err) });
          ...
          // [問題3] 取消指令已由 handleCancel 處理，這裡不再需要判斷
        ...
        console.error('❌ handleStudentCommands 執行期間發生錯誤:', err);
        // [問題2]
        return reply(replyToken, '系統處理您的指令時發生錯誤，請稍後再試。');
    }
  }
}
/**
  ...
  if (pendingTeacherAddition[userId]) {
    ...
    switch (state.step) {
      case 'await_student_info':
        const client = await pgPool.connect();
        try {
            const studentRes = await client.query(`SELECT id, name, role FROM users WHERE role = 'student' AND (LOWER(name) LIKE $1 OR id = $2)`, [`%${text.toLowerCase()}%`, text]);
      ...
      case 'await_confirmation':
        if (text === COMMANDS.ADMIN.CONFIRM_ADD_TEACHER) {
          ...
          if(TEACHER_RICH_MENU_ID) await client.linkRichMenuToUser(targetUser.id, TEACHER_RICH_MENU_ID).catch(e => console.error(e));
        } else {
    ...
    if (text === COMMANDS.ADMIN.CONFIRM_REMOVE_TEACHER) {
        ...
        if(STUDENT_RICH_MENU_ID) await client.linkRichMenuToUser(targetUser.id, STUDENT_RICH_MENU_ID).catch(e => console.error(e));
    } else {
        return reply(replyToken, '請點擊確認或取消按鈕。');
    }
  } else {
    try {
        ...
        console.error('❌ handleAdminCommands 執行失敗:', err);
        return reply(replyToken, '處理管理者指令時發生錯誤。');
    }
  }
}
/**
  ...
  if (pendingCourseCancellation[userId]) {
    ...
    switch(state.type) {
      case 'batch':
        if (text === COMMANDS.TEACHER.CONFIRM_BATCH_CANCEL) {
          ...
          reply(replyToken, '✅ 指令已收到，正在為您批次取消課程。\n完成後將會另行通知，請稍候...').catch(err => { if (!err.message.includes('Invalid reply token')) { console.error('Error sending empty reply:', err); } });
    ...
    switch (state.step) {
      case 'await_student_search':
        const client = await pgPool.connect();
        try {
            const res = await client.query(`SELECT id, name, picture_url FROM users WHERE role = 'student' AND (LOWER(name) LIKE $1 OR id = $2) LIMIT 10`, [`%${text.toLowerCase()}%`, text]);
      ...
      case 'await_reason':
        ...
        return reply(replyToken, summary, [ { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST, text: COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST } }, getCancelMenu()[0] ]);
    ...
    try {
        if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
            ...
            setupConversationTimeout(userId, pendingStudentSearchQuery, 'pendingStudentSearchQuery', (u) => { if (pendingStudentSearchQuery[u]) { delete pendingStudentSearchQuery[u]; push(u, '查詢學員逾時，自動取消。').catch(e => console.error(e)); }});
            ...
            reply(replyToken, '📊 報表生成中，資料量較大，請稍候... 完成後將會推播通知您。').catch(e => console.error(e));
            
            generateReportTask()
                .then(reportText => push(userId, reportText))
                .catch(err => {
                    console.error('❌ 背景生成點數報表失敗:', err);
                    // [問題2]
                    push(userId, '❌ 產生點數報表時發生錯誤，請稍後再試。');
                });
        ...
        console.error('❌ handleTeacherCommands 執行期間發生錯誤:', err);
        // [問題2]
        return reply(replyToken, '系統處理您的指令時發生錯誤，請稍後再試。');
    }
  }
    ...
    try {
        ...
        for (const course of upcomingCourses) {
            if (!sentReminders[course.id]) {
                ...
                for (const studentId of course.students) {
                    await push(studentId, reminderMsg).catch(e => console.error(`發送課程提醒給 ${studentId} 失敗`, e));
                }
...
app.post('/webhook', (req, res) => {
  ...
  Promise.all(req.body.events.map(event => handleEvent(event).catch(err => {
      ...
      // [問題2] 作為最後防線，嘗試回覆使用者
      if (event.replyToken) {
          reply(event.replyToken, '系統發生未預期的錯誤，請稍後再試或聯繫管理員。').catch(e => console.error("最終防線回覆失敗:", e));
      }
  })));
});
...
app.listen(PORT, async () => {
  ...
  // 版本號更新
  console.log(`Bot 版本: V19.0 (程式碼重構)`);
...
async function handleEvent(event) {
    ...
    if (user.role === 'student') {
        const clientDB = await pgPool.connect();
        try {
            const annRes = await clientDB.query('SELECT id FROM announcements ORDER BY created_at DESC LIMIT 1');
            if (annRes.rows.length > 0 && annRes.rows[0].id > (user.last_seen_announcement_id || 0)) {
                // 此處保留未來可實作「主動推播新公告」的邏輯
            }
        } catch (err) {
        ...
        try {
            ...
            if (action === 'select_announcement_for_deletion') {
                if (user.role === 'teacher' || user.role === 'admin') {
                    ...
                    const client = await pgPool.connect();
                    try {
                        const annRes = await client.query("SELECT content FROM announcements WHERE id = $1", [ann_id]);
            ...
            if (action === 'run_command') {
                ...
                if (commandText) {
                    const simulatedEvent = { ...event, type: 'message', message: { type: 'text', id: `sim_${Date.now()}`, text: commandText } };
                    // 模擬事件，遞迴呼叫 handleEvent
            ...
            if (user.role === 'teacher' || user.role === 'admin') {
              ...
              if (action === 'set_course_weekday') {
               ...
               if (action === 'cancel_course_group_confirm') {
                    ...
                    const client = await pgPool.connect();
                    try {
                        const courseRes = await client.query("SELECT title FROM courses WHERE id LIKE $1 LIMIT 1", [`${prefix}%`]);
                ...
                if (action === 'select_student_for_adjust') {
                  ...
                  return reply(replyToken, `您要為學員「${targetStudent.name}」進行何種點數調整？`, [ { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.ADD_POINTS, text: COMMANDS.TEACHER.ADD_POINTS } }, { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.DEDUCT_POINTS, text: COMMANDS.TEACHER.DEDUCT_POINTS } }, getCancelMenu()[0] ]);
                }
            ...
            // [問題2]
            return reply(replyToken, '處理您的請求時發生錯誤。');
        }
    }
}
