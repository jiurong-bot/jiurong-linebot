...
const CONSTANTS = {
  ...
  COMMANDS: {
    ...
    TEACHER: {
      ...
      ADD_POINTS: '+ 加點',
      DEDUCT_POINTS: '- 扣點',
      MESSAGE_SEARCH: '@查詢歷史留言', // V35.0 文字調整 (原為 @查看歷史留言)
...
async function handleTeacherCommands(event, userId) {
  ...
  // V35.0 新增: 處理老師查詢購點紀錄的對話
    ...
    return showStudentSelectionForHistory(res.rows, 'purchase');
  }

  // V35.0 新增: 處理老師查詢兌換紀錄的對話
      ...
      return showStudentSelectionForHistory(res.rows, 'exchange');
  }
...
/**
 * [V35.0 新增] 顯示學員選擇輪播，用於各種歷史紀錄查詢
 * @param {Array<object>} users - 符合搜尋條件的使用者陣列
 * @param {string} historyType - 紀錄類型 ('purchase', 'exchange', 'message')
 * @returns {object} - Flex Message 物件
 */
async function showStudentSelectionForHistory(users, historyType) {
    const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
    
    const typeMap = {
        purchase: { label: '購點紀錄', action: 'view_teacher_purchase_history' },
        exchange: { label: '兌換紀錄', action: 'view_teacher_exchange_history' },
        message: { label: '留言紀錄', action: 'view_historical_messages' }
    };
    const currentType = typeMap[historyType];

    const userBubbles = users.map(u => {
        let actionData;
        if (historyType === 'message') {
            // 留言紀錄直接使用姓名作為查詢參數
            actionData = `${currentType.action}&query=${encodeURIComponent(u.name)}&page=1`;
        } else {
            actionData = `${currentType.action}&user_id=${u.id}&page=1`;
        }
        ...
        return {
            ...
            footer: {
                ...
                contents: [{
                    ...
                    color: '#1A759F',
                    height: 'sm',
                    action: { type: 'postback', label: `查看此學員${currentType.label}`, data: actionData }
                }]
            }
        };
    });
    ...
    return {
        type: 'flex',
        altText: `請選擇要查詢${currentType.label}的學員`,
        contents: { type: 'carousel', contents: userBubbles }
    };
}
...
/**
 * [V35.0 新增] 顯示老師查詢的購點紀錄
 * @param {number} page - 當前頁碼
 * @param {string|null} userId - (可選) 要篩選的學員 ID
 * @returns {Promise<object|string>} - Flex Message 物件或無資料時的文字訊息
 */
async function showTeacherPurchaseHistory(page, userId = null) {
    ...
    return withDatabaseClient(async (client) => {
        ...
        if (pageRows.length === 0 && page === 1) {
            return userId ? '這位學員沒有任何購點紀錄。' : '目前沒有任何已完成的購點紀錄。';
        }
        if (pageRows.length === 0) {
            return '沒有更多紀錄了。';
        }
        ...
        const customParams = userId ? `&user_id=${userId}` : '';
        const paginationBubble = createPaginationBubble('action=view_teacher_purchase_history', page, hasNextPage, customParams);
        const footerContents = paginationBubble ? paginationBubble.body.contents : [];
        
        const headerText = userId ? `${pageRows[0].user_name} 的購點紀錄` : '所有購點紀錄';
        ...
        return {
            ...
            contents: {
                type: 'bubble',
                size: 'giga',
                header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: headerText, weight: 'bold', size: 'lg', color: '#FFFFFF' }], backgroundColor: '#343A40' },
...
async function showTeacherExchangeHistory(page, userId = null) {
    ...
    return withDatabaseClient(async (client) => {
        ...
        if (pageRows.length === 0 && page === 1) {
            return userId ? '這位學員沒有任何兌換紀錄。' : '目前沒有任何商品兌換紀錄。';
        }
        if (pageRows.length === 0) {
            return '沒有更多紀錄了。';
        }
            ...
            contents: [
                {
                    ...
                    contents: [
                        ...
                        { type: 'text', text: order.product_name, size: 'sm', color: '#666666', wrap: true },
        ...
        const customParams = userId ? `&user_id=${userId}` : '';
        const paginationBubble = createPaginationBubble('action=view_teacher_exchange_history', page, hasNextPage, customParams);
        const footerContents = paginationBubble ? paginationBubble.body.contents : [];
        
        const headerText = userId ? `${pageRows[0].user_name} 的兌換紀錄` : '所有兌換紀錄';
...
/**
 * [V35.0 新增] 顯示所有歷史留言 (老師視角)
 * @param {number} page - 當前頁碼
 * @returns {Promise<object|string>} - Flex Message 物件或無資料時的文字訊息
 */
async function showAllMessages(page) {
    const offset = (page - 1) * CONSTANTS.PAGINATION_SIZE;
    return withDatabaseClient(async (client) => {
        const res = await client.query(
        ...
        const paginationBubble = createPaginationBubble('action=view_all_messages', page, hasNextPage);
        const footerContents = paginationBubble ? paginationBubble.body.contents : [];
        return {
            type: 'flex',
            altText: '所有歷史留言',
            contents: {
                type: 'bubble',
                size: 'giga',
                header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: '所有歷史留言', weight: 'bold', size: 'lg', color: '#FFFFFF', wrap: true }], backgroundColor: '#343A40' },
                body: { type: 'box', layout: 'vertical', paddingAll: 'none', contents: listItems.flatMap((item, index) => index === 0 ? [item] : [{ type: 'separator' }, item]) },
                footer: { type: 'box', layout: 'vertical', contents: footerContents }
            }
        };
...
app.listen(PORT, async () => {
  try {
    ...
    console.log(`Bot 版本 V34.8 (資料查詢優化)`);
