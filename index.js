...
const pendingTeacherLogin = {}; // 確保未重複定義

// webhook route app.post('/webhook', line.middleware(config), (req, res) => { res.status(200).end(); // 回應 200 給 LINE 平台避免 timeout Promise.all(req.body.events.map(handleEvent)).catch(err => console.error('Webhook Error:', err)); });

// 健康檢查 route app.get('/', (req, res) => { res.send('九容瑜伽 LINE Bot 正常運作中'); });

// keep-alive ping setInterval(() => { fetch('https://your-render-app-url.onrender.com').catch(err => console.error('Keep alive failed:', err)); }, 5 * 60 * 1000);
...
// ✅ Express GET / 路由（可供 uptime robot 或外部 ping）
app.get('/', (req, res) => {
