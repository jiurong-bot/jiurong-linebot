...
async function handleEvent(event) {
  ...
  if (!db[userId]) {
    db[userId] = { role: 'student', points: 0, history: [] };
  ...
  if (msg === '@我是老師') {
    user.role = 'teacher';
    writeJSON(DATA_FILE, db);
    return replyText(event.replyToken, '✅ 您已成為老師，請重新輸入指令');
  }
  ...
  if (user.role === 'teacher') {
    if (msg === '@今日名單') {
      ...
      const list = Object.entries(courses)
        ...
        .map(([id, c]) =>
          `${id}: ${c.name} (${c.date})\n學員：${c.students.map(id => id).join(', ') || '無'}`
        )
        .join('\n\n') || '今日尚無課程';
    ...
    if (/^@查學員/.test(msg)) {
      ...
      return replyText(event.replyToken, `學員點數：${student.points}\n預約紀錄：\n${history}`);
    }
    ...
    return replyText(event.replyToken, '老師功能指令無效，請重新輸入');
  }
  ...
  if (user.role === 'student') {
    ...
    if (msg === '@取消課程') {
      ...
      return replyWithMenu(
        event.replyToken,
        list ? `請輸入「取消 課程ID」，例如：取消 course_123\n\n${list}` : list
      );
    }
