...
const COMMANDS = {
  ...
  TEACHER: {
    ...
    DEDUCT_POINTS: '- 扣點',
    MESSAGE_SEARCH: '@查詢留言',
    CONFIRM_ADD_ANNOUNCEMENT: '✅ 確認頒佈', // [修改]
    CANCEL_ADD_ANNOUNCEMENT: '❌ 取消頒佈', // [修改]
    CONFIRM_DELETE_ANNOUNCEMENT: '✅ 確認刪除', // [修改]
    ABANDON_DELETE_ANNOUNCEMENT: '❌ 放棄刪除', // [修改]
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingAnnouncementCreation[userId]) {
    ...
    if (text === COMMANDS.GENERAL.CANCEL || text === COMMANDS.TEACHER.CANCEL_ADD_ANNOUNCEMENT) {
      delete pendingAnnouncementCreation[userId];
      return reply(replyToken, '已取消頒佈公告。');
    }
    switch (state.step) {
      case 'await_content':
        state.content = text;
      ...
      case 'await_confirmation':
        if (text === COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT) {
          ...
          return reply(replyToken, '請點擊「確認頒佈」或「取消頒佈」。');
        }
    }
    return;
  }
  ...
  if (pendingAnnouncementDeletion[userId]) {
    ...
    if (text === COMMANDS.GENERAL.CANCEL || text === COMMANDS.TEACHER.ABANDON_DELETE_ANNOUNCEMENT) {
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V18.1 (公告管理功能實作)`);
