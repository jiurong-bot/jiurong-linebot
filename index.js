...
async function initializeDatabase() {
  try {
    ...
    // V8.3 修正點: 檢查 announcements 表是否缺少 creator_id 和 creator_name 欄位
        ...
        case 'await_confirmation':
            if (text === COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT) {
                try {
                    ...
                    delete pendingAnnouncementCreation[userId]; // V8.3 修正: 失敗時也清除狀態
  ...
  if (pendingManualAdjust[userId]) {
      ...
      switch (manualAdjustState.step) {
          case 'awaiting_student_info':
              const studentRes = await pgPool.query(`SELECT * FROM users WHERE role = 'student' AND (LOWER(name) LIKE $1 OR id = $2) LIMIT 10`, [`%${text.toLowerCase()}%`, text]);
    ...
    const announcementMenu = {
      ...
      contents: {
        ...
        header: {
          type: 'box',
          layout: 'vertical',
          contents: [{ type: 'text', text: '公告管理', weight: 'bold', size: 'lg', color: '#FFFFFF' }],
          ...
          contents: [
            {
              ...
              layout: 'horizontal',
              contents: [
                { type: 'text', text: '發布新消息給所有學員', size: 'md', color: '#666666', flex: 4, wrap: true },
                { type: 'button', style: 'primary', height: 'sm', action: { type: 'postback', label: '新增公告', data: 'action=add_announcement_start' }, flex: 2 }
              ]
            },
            { type: 'separator', margin: 'md' },
            {
              ...
              layout: 'horizontal',
              contents: [
                { type: 'text', text: '查看或刪除過去の公告', size: 'md', color: '#666666', flex: 4, wrap: true },
                { type: 'button', style: 'primary', height: 'sm', action: { type: 'postback', label: '歷史公告', data: 'action=history_announcements_show' }, flex: 2 }
              ]
            }
          ]
        }
        ...
        const planItems = [
            {
                ...
                layout: 'horizontal',
                contents: [
                    { type: 'text', text: `${plan.points} 點`, size: 'md', color: '#1A759F', flex: 2, gravity: 'center' },
                    { type: 'text', text: `售價：${plan.amount} 元`, size: 'md', color: '#666666', align: 'end', flex: 3 }
                ]
            },
            {
                type: 'button',
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (user.role === 'teacher') {
            if (action === 'add_announcement_start') {
                // V8.3 修正: 進入發布公告流程時，清除所有進行中的狀態
                for (const key in pendingTeacherState) {
                    clearTimeout(pendingTeacherState[key].timeoutId);
                    delete pendingTeacherState[key];
                }
              ...
              await reply(replyToken, '請輸入課程名稱（例如：哈達瑜伽）：', [{ type: 'action', action: { type: 'message', label: COMMANDS.STUDENT.CANCEL_ADD_COURSE, text: COMMANDS.STUDENT.CANCEL_ADD_COURSE } }]); return; 
                ...
                return reply(replyToken, `確定要取消課程：「${course.title}」(${formatDateTime(course.time)}) 嗎？\n已預約學員將退還點數並收到通知。`, [{ type: 'action', action: { type: 'postback', label: '✅ 確認取消', data: `action=cancel_single_course&courseId=${course.id}`, displayText: `確認取消 ${course.title}` } }]);
