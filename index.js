...
async function handleTeacherCommands(event, userId) {
  ...
  } else if (pendingAnnouncementCreation[userId] || pendingReply[userId] || pendingMessageSearchQuery[userId] || pendingProductCreation[userId]) {
          ...
          if (action === 'manage_course_group') {
           ...
           if (action === 'cancel_course_group_confirm') {
            ...
            if (action === 'view_student_details') {
                ...
                const allCourses = Object.values(await getAllCourses());
                const now = Date.now();
                const studentCourses = allCourses.filter(c => new Date(c.time).getTime() > now && (c.students.includes(studentId) || c.waiting.includes(studentId)));
                studentCourses.sort((a, b) => new Date(a.time) - new Date(b.time));
                
                let courseText = '【未來課程】\n';
                if (studentCourses.length > 0) {
                    courseText += studentCourses.map(c => {
                ...
                if (history.length > 0) {
                    historyText += history.slice(-10).reverse().map(h => {
                        return ` - ${formatDateTime(h.time)}\n   ${h.action} (${h.pointsChange > 0 ? '+' : ''}${h.pointsChange}點)`;
                    }).join('\n\n');
                } else {
                    historyText += '無';
                }
