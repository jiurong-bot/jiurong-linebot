        ...
        const planItems = [
            {
                type: 'box',
                layout: 'horizontal',
                // V8.3 修正 (a): 透過設定 flex 比例，確保售價金額能完整顯示
                contents: [
...
async function handleStudentCommands(event, userId) {
  ...
  if (text === COMMANDS.STUDENT.CANCEL_PURCHASE) {
    // V8.3 修正 (b): 調整判斷順序。優先檢查是否在選擇方案的暫存狀態中
    if (pendingPurchase[userId]) {
        delete pendingPurchase[userId];
        return reply(replyToken, '已取消購買點數。', [returnToPointsMenuBtn]);
    }

    // 如果沒有暫存狀態，才繼續執行原有的邏輯，檢查資料庫中是否有已建立的訂單
    const ordersRes = await pgPool.query(`SELECT * FROM orders WHERE user_id = $1 AND (status = 'pending_payment' OR status = 'pending_confirmation' OR status = 'rejected') ORDER BY timestamp DESC LIMIT 1`, [userId]);
    const pendingOrder = ordersRes.rows[0];
    if (pendingOrder) {
