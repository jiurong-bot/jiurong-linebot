 ...
 */
async function 
createPaginatedCarousel(options) {
  const {
    altText,
    baseAction,
    page,
        ...
        await executeDbQuery(async (client) => {
            if (user.role === 'teacher') {
                const [newMessages, pendingPointOrders, pendingShopOrders, upcomingCoursesRes] = await Promise.all([
                    ...
                    client.query("SELECT 
COUNT(*) FROM orders WHERE status = 'pending_confirmation'"),
...
async function generateUniqueCoursePrefix(dbClient) {
    return executeDbQuery(async (client) => {
        ...
        while (!isUnique) {
            ...
            const 
res = await client.query('SELECT id FROM courses WHERE id LIKE $1', [`${prefix}%`]);
    ...
    return executeDbQuery(async (client) => {
        ...
        await client.query(
            `INSERT INTO users (id, name, points, role, history, last_seen_announcement_id, picture_url, approved_by) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             ON CONFLICT (id) DO UPDATE SET name = $2, points = $3, role = $4, history = $5, last_seen_announcement_id = $6, picture_url 
= $7, approved_by = $8`,
    ...
    return executeDbQuery(async (client) => {
        ...
        await client.query(
            `INSERT INTO courses (id, title, time, capacity, points_cost, students, waiting, teacher_id) 
             ...
             ON CONFLICT 
(id) DO UPDATE SET title = $2, time = $3, capacity = $4, points_cost = $5, students = $6, waiting = $7, teacher_id = $8`,
...
if (!user) return { type: 'text', text: '無法獲取您的使用者資料。' };
    ...
    const bodyContents = [{
        ...
        contents: [
            ...
            { type: 'text', 
text: `${user.points} 點`, weight: 'bold', size: '3xl', margin: 'sm', color: '#1A759F' },
        ]
    }];
return {
        type: 'flex',
                ...
                contents: [
                    {
                        ...
                        action: { type: 'postback', label: '➕ 購買點數', 
data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.STUDENT.BUY_POINTS)}` }
                    },
                    {
                        type: 'button', style: 'secondary', height: 'sm',
                        action: { type: 'postback', label: 
'📜 查詢購點紀錄', data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.STUDENT.PURCHASE_HISTORY)}` }
                    }
                ]
            }
        }
...
const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
        ...
        return {
            ...
            contents: {
             ...
             hero: { type: 'image', url: previewProfile.image_url ||
placeholder_avatar, size: 'full', aspectRatio: '1:1', aspectMode: 'cover' },
                body: {
                    type: 'box', layout: 'vertical', paddingAll: 'lg', spacing: 'md',
                    contents: [
                        { type: 'text', text: 
previewProfile.name, weight: 'bold', size: 'xl' },
                        { type: 'text', text: previewProfile.bio ||
'尚未填寫簡介', wrap: true, size: 'sm', color: '#666666' }
                    ]
                },
                footer: {
...
// --- 對話狀態管理 (V41.0 重構) ---
const userConversationState = new Map(); // 單一 Map 來管理所有對話狀態
const userProfileCache = new Map();
const userLastInteraction = {};
const repliedTokens = new Set();
// sentReminders 似乎是快取用途，暫時保留
const sentReminders = {};


/**
 * [V41.0 重構] 清除使用者所有待處理的對話狀態。
...
/**
 * [V41.0 重構] 為使用者的對話設定超時取消機制。
 * @param {string} userId - 使用者的 ID。
 * @param {object} stateData - 要儲存的狀態資料，必須包含 type 和 step。
 * @param {function(string): void} onTimeout - 超時後要執行的回呼函式，會接收 userId。
 */
function setupConversationTimeout(userId, stateData, onTimeout) {
    // 先清除可能存在的舊計時器
    const existingState = userConversationState.get(userId);
    if (existingState?.timeoutId) {
        clearTimeout(existingState.timeoutId);
    }

    const timeoutId = setTimeout(() => {
        // 檢查超時當下，狀態是否仍然存在且未被改變
        const latestState = userConversationState.get(userId);
        if (latestState && latestState.timeoutId === timeoutId) {
            userConversationState.delete(userId);
            if (onTimeout) {
                onTimeout(userId);
            }
    ...
    // 將計時器 ID 和狀態資料一起存入 Map
    userConversationState.set(userId, { ...stateData, timeoutId });
}

async function handlePurchaseFlow(event, userId, purchaseState) {
            ...
            body: { 
              ...
              paddingAll: 'lg', 
                contents: [ 
                    ...
                    { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '➕ 新增課程系列', data: 
...
'separator', margin: 'xl' },
                    ...
                    { type: 'button', style: 'secondary', height: 'sm', 
action: { type: 'postback', label: '👤 個人資訊', data: 'action=manage_personal_profile' } }
                ] 
            } 
        } 
    };
...
async function startStudentSearch(event, user) {
    const userId = user.id;
    const stateData = { type: 'studentSearch', step: 'await_query' };
setupConversationTimeout(userId, stateData, (u) => { 
...
async function startMessageSearch(event, user) {
    const userId = user.id;
    const stateData = { type: 'messageSearch', step: 'await_query' };
setupConversationTimeout(userId, stateData, (u) => { 
...
async function startAddAnnouncement(event, user) {
    const userId = user.id;
    const stateData = { type: 'addAnnouncement', step: 'await_content', data: {} };
setupConversationTimeout(userId, stateData, (u) => { 
...
async function startAddProduct(event, user) {
    const userId = user.id;
    const stateData = { type: 'addProduct', step: 'await_name', data: {} };
setupConversationTimeout(userId, stateData, u => { 
...
async function startManualAdjust(event, user) {
    const userId = user.id;
    const stateData = { type: 'manualAdjust', step: 'await_student_search', data: {} };
setupConversationTimeout(userId, stateData, (u) => { 
...
const teacherCommandMap = {
    ...
    [CONSTANTS.COMMANDS.TEACHER.MANAGE_PREORDER_PRODUCTS]: (event, user) 
=> showPreorderProducts(1),
...
color: '#aaaaaa', size: 'sm', flex: 2 }, { type: 'text', text: `${recipientName}`, color: '#666666', size: 'sm', flex: 5, wrap: true } ] },
                    ...
                    { type: 
'box', layout: 'vertical', spacing: 'sm', contents: [ { type: 'text', text: '錯誤原因', color: '#aaaaaa', size: 'sm' }, { type: 'text', text: errorMessage.substring(0, 100), color: '#666666', size: 'sm', wrap: true, margin: 'md' } ] }
                ]},
                footer: { type: 'box', layout: 'horizontal', spacing: 'sm', contents: [
                    { type: 'button', style: 'secondary', flex: 
1, height: 'sm', action: { type: 'postback', label: '🗑️ 刪除', data: `action=delete_failed_task&id=${task.id}` } },
...
await executeDbQuery(async (db) => {
  ...
  settings.student_order_result = dbSettings.get('student_order_result_enabled') ?? true;
        ...
        settings.student_welcome_message = dbSettings.get('student_welcome_message_enabled') ??
true;
...
ℹ️ 「待處理任務」是系統即將要發送的排程訊息 (如課程提醒)。若「失敗任務」數量持續增加，請檢查 
Worker 紀錄。
    `.trim();
...
return {
            ...
            header: {
                type: 'box',
                layout: 'vertical',
                contents: [{ type: 'text', text: `🚨 錯誤代碼`, weight: 'bold', color: 
'#FFFFFF' },
        ...
        type: 'box', layout: 'baseline', spacing: 'sm', contents: [
                  ...
                  { type: 'text', text: '使用者ID', color: '#aaaaaa', size: 'sm', flex: 2 },
                    ...
                    {
                        type: 'box', layout: 'baseline', spacing: 'sm', contents: [
                            { type: 'text', text: '發生情境', color: '#aaaaaa', 
size: 'sm', flex: 2 },
...
pendingPointOrders.forEach(order => {
                ...
                if (order.status === 'pending_confirmation') {
                    ...
                    actionButtonLabel = '輸入匯款後五碼'; actionCmd = CONSTANTS.COMMANDS.STUDENT.INPUT_LAST5_CARD_TRIGGER; cardColor = '#f28482'; 
statusText = '待付款';
                }
...
{ type: 'text', text: `狀態: ${statusText}`, size: 'sm', color: cardColor, weight: 'bold' },
                        ...
                        ...(additionalInfo ?
[{ type: 'text', text: additionalInfo, size: 'xs', color: '#B00020', wrap: true, margin: 'sm' }] : []),
                        {
...
{ typeText = '⚠️ 手動扣點'; pointsText = `${order.points}`; pointsColor = '#D9534F'; }
                } else { // 一般購點
                        ...
                        
{
                            ...
                            contents: [
                                { type: 'text', text: typeText, weight: 
'bold', size: 'sm' },
...
bodyContents : [{type: 'text', text: '目前沒有任何紀錄。', align: 'center'}]
                }
            }
        };
});
...
async function showExchangeHistoryList(event, user) {
   ...
   layout: 'vertical',
                ...
                contents: [
                    ...
                    { type: 'button', style: 'link', height: 
'sm', action: { type: 'postback', label: '搜尋特定學員', data: `action=start_exchange_history_search` } }
                ]
            }
        }
    };
...
event.message.text.trim().normalize() : '';
  ...
  // 優先處理有延續性的對話 (V41.0 Refactor)
  if (currentState) {
    switch (currentState.type) {
        case 'addProduct': {
            let proceed = true;
            let errorMessage = '';
            switch (currentState.data.step) {
                case 'await_name': 
                    currentState.data.name = text;
                currentState.data.step = 'await_description'; 
            ...
            case 'await_description': 
                    currentState.data.description = text === '無' ? null : text; 
                    currentState.data.step = 'await_price';
                userConversationState.set(userId, currentState);
...
if (isNaN(price) || price < 0) { 
                        ...
                        currentState.data.price = price;
                    currentState.data.step = 'await_inventory'; 
                        userConversationState.set(userId, currentState);
...
if (isNaN(inventory) || inventory < 0) { 
                        ...
                        currentState.data.inventory = inventory;
                    currentState.data.isPreorder = (inventory === 0);
                        currentState.data.step = 'await_image_url';
                    userConversationState.set(userId, currentState);
...
imageUrl = uploadResponse.url;
                    ...
                    if (!proceedToNextStep) { 
                        return { type: 'text', text: errorImageUrlMessage, quickReply: { items: getCancelMenu() } };
}
                    currentState.data.image_url = imageUrl; 
                    currentState.data.step = 'await_confirmation';
                userConversationState.set(userId, currentState);
                    const summaryText = `請確認商品資訊：\n\n` +
                                      `名稱：${currentState.data.name}\n` +
                                      `描述：${currentState.data.description ||
'無'}\n` +
                                      `價格：${currentState.data.price} 元\n` +
                                      `庫存：${currentState.data.inventory}\n` +
                                      `狀態：${currentState.data.isPreorder ?
'開放預購' : '直接上架'}\n` +
                                      `圖片：${currentState.data.image_url ||
'無'}\n\n` +
                                      `確認無誤後請點擊「✅ 確認上架」。`;
return {
                        type: 'text',
            ...
            if (!proceed && currentState.data.step !== 'await_image_url') { 
                return { type: 'text', text: errorMessage, quickReply: { items: getCancelMenu() } };
}
            return; // Fallthrough if not handled
        }
        case 'editProduct': {
            const product = currentState.data.product;
const field = currentState.data.field;
            let newValue = text; 
            let isValid = true; 
            let errorMessage = '';
...
userConversationState.delete(userId);
        ...
        case 'deleteAnnouncement': {
            if (text === CONSTANTS.COMMANDS.TEACHER.CONFIRM_DELETE_ANNOUNCEMENT) {
                await executeDbQuery(client => client.query("DELETE FROM announcements WHERE id = $1", [currentState.data.ann_id]));
userConversationState.delete(userId);
                return '✅ 公告已成功刪除。';
            } else { 
                return '請點擊「確認刪除」或「取消操作」。';
}
        }
        case 'cancelCourse': {
            switch(currentState.data.type) {
                case 'batch':
                    if (text === CONSTANTS.COMMANDS.TEACHER.CONFIRM_BATCH_CANCEL) {
                        const backgroundStateData = { ...currentState.data };
userConversationState.delete(userId);
                        try {
                            (async () => {
                                await executeDbQuery(async (client) => {
                                    ...
                                    try {
                                        const coursesToCancelRes = await client.query("SELECT * FROM courses WHERE id 
LIKE $1 AND time > NOW() FOR UPDATE", [`${backgroundStateData.prefix}%`]);
                                        if (coursesToCancelRes.rows.length === 0) { 
                                            const errMsg = { type: 'text', text: `❌ 批次取消失敗：找不到可取消的「${backgroundStateData.prefix}」系列課程。`}; 
...
case 'single':
                    if (text === CONSTANTS.COMMANDS.TEACHER.CONFIRM_SINGLE_CANCEL) {
                        return executeDbQuery(async (client) => {
                            await client.query('BEGIN');
                            try {
                                const courseToCancelRes = await client.query("SELECT * FROM courses WHERE id = $1 FOR UPDATE", [currentState.data.course_id]);
    
              if (courseToCancelRes.rows.length === 0) { userConversationState.delete(userId); return "找不到該課程，可能已被取消。"; }
           ...
           await client.query("UPDATE users SET points = points + $1 WHERE id = $2", [course.points_cost, studentId]); 
                                ...
                                await client.query("DELETE FROM courses WHERE id = $1", [currentState.data.course_id]); 
                                userConversationState.delete(userId);
...
userConversationState.delete(userId); 
        ...
        case 'addCourseSeries': {
            switch (currentState.step) {
                case 'await_title': 
                    currentState.data.title = text;
                currentState.step = 'await_weekday';
                    userConversationState.set(userId, currentState);
...
return { type: 'text', text: `課程標題：「${text}」\n\n請問課程固定在每週的哪一天？`, quickReply: { items: weekdayButtons } };
            case 'await_start_time': 
                    ...
                    currentState.data.start_time = text;
                currentState.step = 'await_end_time';
                    userConversationState.set(userId, currentState);
return { type: 'text', text: `好的，開始時間是 ${text}。\n\n那『結束』時間是幾點呢？（例如：20:30）`, quickReply: { items: getCancelMenu() } };
            case 'await_end_time': 
                    ...
                    currentState.data.end_time = text;
                currentState.step = 'await_sessions';
                    userConversationState.set(userId, currentState);
...
if (isNaN(sessions) || sessions <= 0) { return { type: 'text', text: '堂數必須是正整數，請重新輸入。', quickReply: { items: getCancelMenu() } };
                    ...
                    currentState.data.sessions = sessions; 
                    currentState.step = 'await_capacity';
                userConversationState.set(userId, currentState);
...
if (isNaN(capacity) || capacity <= 0) { return { type: 'text', text: '名額必須是正整數，請重新輸入。', quickReply: { items: getCancelMenu() } };
                    ...
                    currentState.data.capacity = capacity; 
                    currentState.step = 'await_points';
                userConversationState.set(userId, currentState);
...
if (isNaN(points) || points < 0) { return { type: 'text', text: '點數必須是正整數或 0，請重新輸入。', quickReply: { items: getCancelMenu() } };
                    ...
                    currentState.data.points_cost = points; 
                    currentState.step = 'await_teacher';
                userConversationState.set(userId, currentState);
...
return { type: 'flex', altText: '請選擇要調整點數的學員', contents: { type: 'carousel', contents: userBubbles } };
            case 'await_operation':
                    if (text === CONSTANTS.COMMANDS.TEACHER.ADD_POINTS || text === CONSTANTS.COMMANDS.TEACHER.DEDUCT_POINTS) { 
                        currentState.data.operation = text === CONSTANTS.COMMANDS.TEACHER.ADD_POINTS ?
'add' : 'deduct'; 
                        currentState.step = 'await_amount'; 
                        userConversationState.set(userId, currentState);
                        return { type: 'text', text: `請輸入要 ${text === CONSTANTS.COMMANDS.TEACHER.ADD_POINTS ?
'增加' : '扣除'} 的點數數量 (純數字)：`, quickReply: { items: getCancelMenu() } };
} else { 
                        return '請點擊 `+ 加點` 或 `- 扣點` 按鈕。';
                    }
            case 'await_amount': 
...
if (isNaN(amount) || amount <= 0) { return { type: 'text', text: '點數格式不正確，請輸入一個大於 0 的正整數。', quickReply: { items: getCancelMenu() } };
                    ...
                    currentState.step = 'await_reason'; 
                    userConversationState.set(userId, currentState);
                    return { type: 'text', text: '請輸入調整原因（例如：活動獎勵、課程補償等）：', quickReply: { items: getCancelMenu() } };
            case 'await_reason': 
                    currentState.data.reason = text; 
                    currentState.step = 'await_confirmation';
                    userConversationState.set(userId, currentState);
                    const opText = currentState.data.operation === 'add' ?
`增加 ${currentState.data.amount} 點` : `扣除 ${currentState.data.amount} 點`; 
                    const summary = `請確認調整內容：\n\n對象：${currentState.data.targetStudent.name}\n操作：${opText}\n原因：${currentState.data.reason}`;
...
student = studentRes.rows[0];
             ...
             return `操作失敗：學員 ${student.name} 的點數不足以扣除 ${stateData.amount} 點。`;
                                }
...
// MA for Manual Adjust
                                const pointsChange = stateData.operation === 'add' ?
stateData.amount : -stateData.amount;
...
const notifyMessage = { type: 'text', text: `🔔 點數異動通知\n老師 ${user.name} 為您 ${opTextForStudent} 點。\n原因：${stateData.reason}\n您目前的點數為：${newPoints} 點。` };
                            ...
                            return `✅ 已成功為學員 ${student.name} ${stateData.operation === 'add' ? '增加' : '扣除'} ${stateData.amount} 點。`;
} catch (e) {
                                await clientDB.query('ROLLBACK');
...
if (res.rows.length === 0) {
        ...
        case 'replyToFeedback': {
            try {
                await executeDbQuery(client => 
                    client.query("UPDATE feedback_messages SET status = 'replied', teacher_reply = $1, is_student_read = false WHERE id = $2", [text, currentState.data.msgId])
                );
const studentId = currentState.data.studentId;
                const originalMessage = currentState.data.originalMessage;
                userConversationState.delete(userId);
...
await enqueuePushTask(studentId, notifyMessage);
        ...
        case 'teacherProfileEdit': {
            const step = currentState.step;
if (currentState.data.type === 'create') {
                switch (step) {
                    case 'await_name':
                        currentState.data.profileData.name = text;
                    currentState.step = 'await_bio';
                        setupConversationTimeout(userId, currentState, (u) => { enqueuePushTask(u, { type: 'text', text: '建立檔案操作逾時，自動取消。' }); });
                    userConversationState.set(userId, currentState);
                        return { type: 'text', text: '姓名已收到！\n接下來，請輸入您的個人簡介（例如您的教學風格、專業認證等），或輸入「無」表示留空：', quickReply: { items: getCancelMenu() } };
                case 'await_bio':
                        currentState.data.profileData.bio = text.trim().toLowerCase() === '無' ?
null : text;
                        currentState.step = 'await_image';
                        setupConversationTimeout(userId, currentState, (u) => { enqueuePushTask(u, { type: 'text', text: '建立檔案操作逾時，自動取消。' }); });
                    userConversationState.set(userId, currentState);
...
userConversationState.delete(userId);
                        ...
                        currentState.data.profileData.image_url = imageUrl;
                    currentState.step = 'await_confirmation';
                        currentState.data.newData = currentState.data.profileData;
                        userConversationState.set(userId, currentState);
                        return buildProfileConfirmationMessage(userId, currentState.data.newData);
                }
            } 
            else if (currentState.data.type === 'edit') {
...
return '❌ 圖片上傳失敗，請稍後再試。';
                ...
                currentState.data.newData = { [field]: value };
                currentState.step = 'await_confirmation';
                userConversationState.set(userId, currentState);
                return buildProfileConfirmationMessage(userId, currentState.data.newData);
}
...
userConversationState.delete(userId);
            ...
            case 'await_confirmation':
                    if (text === CONSTANTS.COMMANDS.ADMIN.CONFIRM_ADD_TEACHER) {
                        const targetUser = await getUser(currentState.data.targetUser.id);
...
return buildAdminPanelFlex();
      ...
      const stateData = { type: 'addTeacher', step: 'await_student_info', data: {} };
setupConversationTimeout(userId, stateData, (u) => {
...
return executeDbQuery(async (client) => {
   ...
   spacing: 'md',
                contents: [
                    { type: 'image', url: u.picture_url ||
placeholder_avatar, size: 'md', aspectRatio: '1:1', aspectMode: 'cover' },
                    { 
                        type: 'box', 
                        layout: 'vertical', 
...
console.log(`[DEBUG] 找到老師: ${row.name} (ID: ${row.id})`));
        }
...
0) {
        ...
        const teacherBubbles = pageTeachers.map(t => ({
            type: 'bubble',
            hero: { type: 'image', url: t.image_url ||
placeholder_avatar, size: 'full', aspectRatio: '1:1', aspectMode: 'cover' },
            body: {
         ...
         { type: 'text', text: t.bio ||
'這位老師尚未留下簡介。', wrap: true, size: 'sm', color: '#666666', margin: 'md' },
                ],
            },
        }));
...
query += ` ORDER BY timestamp DESC LIMIT $${paramIndex++} OFFSET $${paramIndex++}`;
        ...
        if (pageRows.length === 0 && page === 1) {
            return 
userId ? '這位學員沒有任何手動調整紀錄。' : '目前沒有任何手動調整紀錄。';
        }
        if (pageRows.length === 0) {
            return '沒有更多紀錄了。';
        }
...
const customParams = userId ? `&user_id=${userId}` : '';
        ...
        const footerContents = paginationBubble ?
paginationBubble.body.contents : [];
...
{ type: 'text', text: formatDateTime(msg.timestamp), size: 'xs', color: '#AAAAAA' },
     ...
     footer: {
                ...
                spacing: 'sm',
                contents: [
                    { type: 'button', style: 'primary', height: 
'sm', action: { type: 'postback', label: '💬 回覆此留言', data: `action=reply_feedback&msgId=${msg.id}&userId=${msg.user_id}` } },
...
text: headerText, weight: 'bold', size: 'lg', color: '#FFFFFF', wrap: true }], backgroundColor: '#343A40' },
...
return executeDbQuery(async (client) => {
        ...
        const hasNextPage = res.rows.length > CONSTANTS.PAGINATION_SIZE;
        const 
pageOrders = hasNextPage ? res.rows.slice(0, CONSTANTS.PAGINATION_SIZE) : res.rows;
...
return executeDbQuery(async (client) => {
        ...
        if 
(pageAnnouncements.length === 0 && page === 1) {
            return '目前沒有任何可刪除的公告。';
        }
            ...
            contents: [
                {
                 ...
                 flex: 4,
                    contents: [
                        ...
                        { type: 'text', text: 
`由 ${ann.creator_name} 於 ${formatDateTime(ann.created_at)} 發佈`, size: 'xxs', color: '#AAAAAA', margin: 'lg', wrap: true }
                    ]
                },
                {
                    type: 'box',
...
color: '#DE5246', height: 'sm', action: { type: 'postback', label: '刪除', data: `action=select_announcement_for_deletion&ann_id=${ann.id}` } }
                    ]
                }
            ]
        }));
...
return {
            ...
            contents: {
                ...
                size: 'giga',
                header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', 
text: '刪除舊公告', weight: 'bold', size: 'lg', color: '#FFFFFF', wrap: true }], backgroundColor: '#343A40' },
                body: { type: 'box', layout: 'vertical', paddingAll: 'none', contents: listItems.flatMap((item, index) => index === 0 ? [item] : [{ type: 'separator' }, item]) },
                footer: { type: 'box', layout: 'vertical', contents: footerContents }
            }
        };
...
placeholder_avatar,
    ...
    // =======================================================
                                ...
                                { type: 'text', text: (series.teacherBio ||
'').substring(0, 28) + '...', size: 'xs', color: '#888888', wrap: true, margin: 'xs' },
                                { type: 'separator', margin: 'md'},
                                {
...
[
                                ...
                                { type: 'text', text: isPreorder ?
'開放預購中' : `庫存: ${p.inventory}`, size: 'sm', color: isPreorder ? '#FF9E00' : '#666666', align: 'end', flex: 1, gravity: 'bottom' }
                            ]
                        },
...
'無描述', wrap: true, size: 'sm', color: '#666666', margin: 'md' },
                    { type: 'separator', margin: 'lg' },
                    {
                        type: 'box',
            ...
            hero: (product.image_url && product.image_url.startsWith('https')) ?
{
          ...
          { type: 'text', text: product.name, weight: 'bold', size: 'xl', wrap: true },
            ...
            footer: {
                type: 'box', layout: 'vertical', spacing: 
'sm',
                contents: [
                    {
                   ...
                   },
                    {
                        ...
                        action: { type: 'postback', label: '停止預購並下架', 
data: `action=stop_preorder_start&product_id=${product.id}` }
                    }
                ]
            }
        };
...
const rosterItems = rosterRes.rows.map(r => `• ${r.user_name} (x${r.quantity})`).join('\n');
       ...
       type: 'box', layout: 'vertical', spacing: 'md', paddingAll: 'lg',
                contents: [
                    ...
                    { type: 'text', text: rosterItems.length > 0 ?
rosterItems : '無', wrap: true, size: 'sm' }
                ]
            },
            footer: {
...
return executeDbQuery(async (client) => {
        const res = await client.query(`
            ...
            AND EXISTS (
                ...
                WHERE 
pp.product_id = p.id AND pp.status = 'active'
            )
...
app.listen(PORT, async () => {
  try {
    ...
    console.log(`Bot 版本 V41.0 (重構對話狀態管理)`);
...
const result = await executeDbQuery(async (client) => {
                ...
                try {
                    ...
                    if (productRes.rows.length === 0) throw 
new Error('找不到商品');
                    const product = productRes.rows[0];
        ...
        const notificationTasks = [];
                    for (const preorder of preorders) {
                        ...
                        // 建立正式訂單
                        await client.query(
                            `INSERT INTO product_orders (order_uid, user_id, user_name, 
product_id, product_name, points_spent, amount, status, payment_method)
...
if (count === 0) {
            ...
            return {
                ...
                quickReply: { items: [
                    { type: 'action', action: { type: 'postback', label: '✅ 確認', data: 
`action=execute_cancel_preorder&product_id=${productId}` } },
    ...
    backgroundColor: '#FF9E00' // 橘色
                    ...
                    body: {
                      ...
                      contents: [
                            ...
                            { type: 'text', text: `單價：${product.price} 元 (到貨後付款)`, size: 'sm', color: 
'#666666', margin: 'md' },
                            { type: 'separator', margin: 'lg' }
                        ]
                    },
...
// 2. 在 INSERT 語句中加入 preorder_uid 欄位與值
                await client.query(
                    ...
                    [preorder_uid, productId, 
userId, user.name, quantity]
                );
// =======================================================
        ...
        case 'cancel_announcement': {
            // 清除待處理的公告狀態，避免誤觸
            const currentState = userConversationState.get(userId);
            if (currentState && currentState.type === 'addAnnouncement') {
                userConversationState.delete(userId);
}
            return '好的，暫不發佈。';
}
                 ...
                 const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
                    return {
                     ...
                     type: 'bubble',
                            ...
                            body: { type: 'box', layout: 'vertical', paddingAll: 'lg', 
spacing: 'md', contents: [ { type: 'text', text: profile.name, weight: 'bold', size: 'xl' }, { type: 'text', text: profile.bio || '尚未填寫簡介', wrap: true, size: 'sm', color: '#666666' } ] },
                            footer: { type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: 'lg', contents: [ { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '✏️ 編輯姓名', data: `action=edit_teacher_profile_field&field=name` } }, { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '✏️ 
編輯簡介', data: `action=edit_teacher_profile_field&field=bio` } }, { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '📷 更換照片', data: `action=edit_teacher_profile_field&field=image_url` } }, ] }
                        }
                    };
} else {
        ...
        case 'create_teacher_profile_start': {
            const stateData = { type: 'teacherProfileEdit', step: 'await_name', data: { type: 'create', profileData: {} } };
setupConversationTimeout(userId, stateData, (u) => {
...
const promptMap = { name: '請輸入您想更新的姓名或暱稱：', bio: '請輸入您想更新的個人簡介 (可換行)：', image_url: '請直接上傳一張您想更換的個人照片：' };
            const stateData = { type: 'teacherProfileEdit', step: `await_${field}`, data: { type: 'edit' } };
setupConversationTimeout(userId, stateData, (u) => {
...
return { type: 'text', text: promptMap[field], quickReply: { items: getCancelMenu() } };
        ...
        case 'confirm_teacher_profile_update': {
            const currentState = userConversationState.get(userId);
if (!currentState || currentState.type !== 'teacherProfileEdit' || currentState.step !== 'await_confirmation' || !currentState.data.newData) { 
                return '確認操作已逾時或無效，請重新操作。';
}
            const newData = currentState.data.newData;
const isCreating = currentState.data.type === 'create';
            userConversationState.delete(userId);
...
const teacher_id = parseInt(data.get('teacher_id'), 10);
    if (!currentState || currentState.type !== 'addCourseSeries' || currentState.step !== 'await_teacher' || !teacher_id) { 
        return '操作已逾時或無效，請重新新增課程。';
}
    currentState.data.teacher_id = teacher_id;
    currentState.step = 'await_confirmation';
const teacher = await executeDbQuery(client => 
        client.query('SELECT name FROM teachers WHERE id = $1', [teacher_id])
    ).then(res => res.rows[0]);
currentState.data.teacher_name = teacher?.name || '未知老師';
    userConversationState.set(userId, currentState);

    const firstDate = getNextDate(currentState.data.weekday, currentState.data.start_time);
// <-- 使用 start_time
    const summary = `請確認課程資訊：\n\n` +
                  `標題：${currentState.data.title}\n` +
                  `老師：${currentState.data.teacher_name}\n` +
                  `時間：每${currentState.data.weekday_label} ${currentState.data.start_time} - ${currentState.data.end_time}\n` + // <-- 修改點
                  `堂數：${currentState.data.sessions} 堂\n` +
      
            `名額：${currentState.data.capacity} 位\n` +
                  `費用：${currentState.data.points_cost} 點/堂\n\n` +
...
label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } } 
            ]
        }
    };
}
...
const course = await getCourse(course_id);
            ...
            const stateData = { 
                type: 'bookingConfirmation', 
                step: 'await_waitlist_confirm', 
                data: { type: 'confirm_wait', course_id: course_id }
            };
setupConversationTimeout(userId, stateData, (u) => {
...
const result = await executeDbQuery(async (client) => {
                ...
                try {
                    ...
                    if (courseRes.rows.length === 0) { 
await client.query('ROLLBACK'); return '抱歉，找不到該課程，可能已被老師取消。'; }
                    const course = courseRes.rows[0];
...
return executeDbQuery(async (client) => {
                ...
                try {
                    const courseRes = await client.query('SELECT * FROM courses WHERE id = $1 FOR UPDATE', [course_id]);
                    if (courseRes.rows.length === 0) { await client.query('ROLLBACK'); return 
'抱歉，找不到該課程，可能已被老師取消。'; }
                    const course = courseRes.rows[0];
                    if ((course.students?.length || 0) < course.capacity) { await client.query('ROLLBACK'); return '好消息！這堂課剛好有名額釋出了，請回到列表直接點擊「預約課程」按鈕。'; }
                    if (course.waiting?.includes(userId)) { await client.query('ROLLBACK'); return '您已在候補名單中，請耐心等候通知。'; }
...
return {
        ...
        contents: {
            ...
            header: {
                type: 'box', layout: 'vertical',
                contents: [{ type: 'text', text: '確認訂單並選擇付款方式', weight: 'bold', size: 'lg', color: 
'#FFFFFF', wrap: true }],
                backgroundColor: '#52B69A'
            },
            body: {
    ...
    await executeDbQuery(async (client) => {
        await client.query(
            `INSERT INTO orders (order_id, user_id, user_name, points, amount, last_5_digits, status, timestamp, payment_method) 
             ...
             user_id 
= $2, user_name = $3, points = $4, amount = $5, last_5_digits = $6, status = $7, timestamp = $8, payment_method = $9`,
...
'找不到此訂單，可能已被其他老師處理。'; }
                    ...
                    await client.query("UPDATE orders SET status = 'completed' WHERE 
order_id = $1", [order_id]);
...
const pointsOrderCount = parseInt(pointsOrderRes.rows[0].count, 10) || 0;
                ...
                const productOrderCount = parseInt(productOrderRes.rows[0].count, 10) ||
0;
...
[
        ...
        case 'start_manual_adjust_history_search': {
            const stateData = { type: 'manualAdjustSearch', step: 'await_student_name' };
setupConversationTimeout(userId, stateData, (u) => {
...
return {
        ...
        case 'start_purchase_history_search': {
            const stateData = { type: 'purchaseHistorySearch', step: 'await_student_name' };
setupConversationTimeout(userId, stateData, u => enqueuePushTask(u, { type: 'text', text: '搜尋購點紀錄操作逾時，自動取消。' }));
return {
                type: 'text',
        ...
        case 'start_exchange_history_search': {
            const stateData = { type: 'exchangeHistorySearch', step: 'await_student_name' };
setupConversationTimeout(userId, stateData, u => enqueuePushTask(u, { type: 'text', text: '搜尋購買紀錄操作逾時，自動取消。' }));
return {
                type: 'text',
        ...
        case 'start_message_history_search': {
            const stateData = { type: 'messageHistorySearch', step: 'await_student_name' };
setupConversationTimeout(userId, stateData, u => enqueuePushTask(u, { type: 'text', text: '搜尋歷史留言操作逾時，自動取消。' }));
return {
                type: 'text',
        ...
        case 'confirm_add_product': {
            const currentState = userConversationState.get(userId);
if (!currentState || currentState.type !== 'addProduct' || currentState.step !== 'await_confirmation') {
                return '上架流程已逾時或中斷，請重新操作。';
            }
            
            // ====================== [修改] ======================
            // 根據我們之前設定的 isPreorder 標記，決定商品狀態
            const productStatus = currentState.data.isPreorder ?
'preorder' : 'available';
 ...
 VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, name`,
                    ...
                    [currentState.data.name, currentState.data.description, currentState.data.price, currentState.data.inventory, currentState.data.image_url, productStatus, userId, user.name]
                )
            ).then(res => res.rows[0]);
userConversationState.delete(userId);
...
const field = data.get('field');
            const product = await getProduct(productId);
            if (!product) return '找不到該商品。';
            const stateData = { type: 'editProduct', step: 'await_input', data: { product, field } };
setupConversationTimeout(userId, stateData, u => enqueuePushTask(u, { type: 'text', text: '編輯商品操作逾時，自動取消。' }));
...
const product = await getProduct(productId);
            if (!product) return '找不到該商品。';
            const stateData = { type: 'adjustInventory', step: 'await_amount', data: { product, originalInventory: product.inventory } };
setupConversationTimeout(userId, stateData, u => enqueuePushTask(u, { type: 'text', text: '調整庫存操作逾時，自動取消。' }));
...
return '取消訂單時發生錯誤，操作已復原。'; }
    ...
    const currentState = userConversationState.get(userId);
if (!currentState || currentState.type !== 'addCourseSeries' || currentState.step !== 'await_weekday') {
        return '新增課程流程已逾時或中斷。';
    }
    currentState.data.weekday = parseInt(data.get('day'), 10);
    currentState.data.weekday_label = WEEKDAYS.find(d => d.value === currentState.data.weekday).label;
currentState.step = 'await_start_time'; // <--- 修改點
    userConversationState.set(userId, currentState);
    return { 
        type: 'text', 
        text: `好的，課程固定在每${currentState.data.weekday_label}。\n\n請問『開始』時間是幾點？（請輸入四位數時間，例如：19:30）`, // <--- 修改點
        quickReply: { items: getCancelMenu() } 
    };
}
  ...
  spacing: 'sm',
                     ...
                     style: 'primary',
                        ...
                        type: 'postback',
                                    label: '✅ 直接發佈',
                                    data: 
'action=publish_prefilled_announcement'
                                }
                            },
                            {
        ...
        case 'publish_prefilled_announcement': {
            const currentState = userConversationState.get(userId);
if (!currentState || currentState.type !== 'addAnnouncement' || !currentState.data.content) {
                return '操作已逾時或無效，請重新操作。';
            }
            
            const contentToPublish = currentState.data.content;
            userConversationState.delete(userId);
await executeDbQuery(client => 
...
return '✅ 公告已成功頒佈！學員可在「最新公告」中查看。';
        ...
        case 'edit_prefilled_announcement': {
            const currentState = userConversationState.get(userId);
if (!currentState || currentState.type !== 'addAnnouncement') return '操作已逾時或無效，請重新操作。';
            
            currentState.step = 'await_content'; // 將狀態推進到等待輸入
            userConversationState.set(userId, currentState);
...
if (!courseTitle) return '找不到此課程系列。';
            ...
            const stateData = { type: 'cancelCourse', step: 'await_batch_confirm', data: { type: 'batch', prefix } };
setupConversationTimeout(userId, stateData, u => enqueuePushTask(u, { type: 'text', text: '取消課程操作逾時。' }));
...
const course = await getCourse(courseId);
            ...
            const stateData = { type: 'cancelCourse', step: 'await_single_confirm', data: { type: 'single', course_id: courseId } };
setupConversationTimeout(userId, stateData, u => enqueuePushTask(u, { type: 'text', text: '取消課程操作逾時。' }));
...
if (user.points < totalCost) return `抱歉，您的點數不足！\n預約 ${spotsToBook} 位需 ${totalCost} 點，您目前有 ${user.points} 點。`;
            ...
            const stateData = { 
                type: 'bookingConfirmation', 
                step: 'await_book_confirm', 
                data: { type: 'confirm_book', course_id: course.id, spots: spotsToBook } 
            };
setupConversationTimeout(userId, stateData, (u) => {
...
const course = await getCourse(course_id);
            ...
            const stateData = { 
                ...
                data: { type: isBooking ? 'cancel_book' : 'cancel_wait', course_id: course_id } 
            };
setupConversationTimeout(userId, stateData, (u) => enqueuePushTask(u, { type: 'text', text: '取消操作已逾時，自動放棄。' }));
...
if (msgRes.rows.length === 0) return '找不到這則留言，可能已被其他老師處理。';
            ...
            const stateData = { type: 'replyToFeedback', step: 'await_reply', data: { msgId, studentId, originalMessage } };
setupConversationTimeout(userId, stateData, (u) => enqueuePushTask(u, { type: 'text', text: '回覆留言操作逾時，自動取消。' }));
...
const targetName = decodeURIComponent(data.get('targetName'));
            ...
            const stateData = { type: 'addTeacher', step: 'await_confirmation', data: { targetUser: { id: targetId, name: targetName } } };
setupConversationTimeout(userId, stateData, u => { enqueuePushTask(u, { type: 'text', text: '授權老師操作逾時。' }).catch(e => console.error(e)); });
...
const targetName = decodeURIComponent(data.get('targetName'));
            ...
            const stateData = { type: 'removeTeacher', step: 'await_confirmation', data: { targetUser: { id: targetId, name: targetName } } };
setupConversationTimeout(userId, stateData, u => enqueuePushTask(u, { type: 'text', text: '移除老師操作逾時。' }));
...
const student = await getUser(studentId);
            ...
            const currentState = userConversationState.get(userId);
            if (!currentState || currentState.type !== 'manualAdjust') return '操作已逾時或無效。';

            currentState.step = 'await_operation';
            currentState.data.targetStudent = { id: student.id, name: student.name };
            userConversationState.set(userId, currentState);
...
if(annRes.rows.length === 0) return '找不到該公告。';
            const stateData = { type: 'deleteAnnouncement', step: 'await_confirmation', data: { ann_id } };
setupConversationTimeout(userId, stateData, u => enqueuePushTask(u, { type: 'text', text: '刪除公告操作逾時。' }));
...
= $1 FOR UPDATE', [failedTaskId]);
                        ...
                        await db.query(`INSERT INTO tasks (recipient_id, message_payload, status, retry_count, last_error) VALUES ($1, $2, 
'pending', 0, 'Retried from DLQ')`, [taskToRetry.recipient_id, taskToRetry.message_payload]);
...
if (!orderUID) return '操作無效，缺少訂單資訊。';
            ...
            const stateData = { type: 'shopPayment', step: 'await_last5', data: { orderUID } };
            setupConversationTimeout(userId, stateData, (u) => {
...
const action = postbackData.get('action');
            // [V41.0 Refactor] 判斷是否為延續性對話的 Postback
            const continuationActions = [
                'set_course_weekday', 'select_teacher_for_course', 'select_student_for_adjust',
                'edit_product_field', 'select_booking_spots', 'start_booking_confirmation', 'execute_booking',
                'confirm_product_purchase', 'execute_product_purchase', 'confirm_teacher_profile_update',
                'start_purchase_history_search', 'start_exchange_history_search', 'start_message_history_search',
                'start_manual_adjust_history_search', 'publish_prefilled_announcement', 'edit_prefilled_announcement',
                'confirm_join_waiting_list_start', 'execute_join_waiting_list',
                'confirm_product_preorder_start', 'execute_product_preorder'
            ];
        ...
        // 如果是新指令 (非延續性 Postback 或 @ 指令)，就清除舊狀態
