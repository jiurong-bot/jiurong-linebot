        ...
        contents: { 
            ...
            body: { 
                ...
                contents: [ 
                    ...
                    { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '📜 查看歷史留言', data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.TEACHER.MESSAGE_SEARCH)}` } } 
                ] 
            } 
        } 
    };
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingProductCreation[userId]) {
    ...
    case 'await_image':
    // 如果使用者輸入「無」，直接進入下一步
    if (event.message.type === 'text' && text.trim().toLowerCase() === '無') {
        state.profileData.image_url = null;
    ...
    (async () => {
        try {
            ...
            currentState.profileData.image_url = uploadResponse.url;
            currentState.step = 'await_confirmation';
            currentState.newData = currentState.profileData;
            ...
            await enqueuePushTask(userId, '❌ 圖片上傳失敗，請重新開始建立檔案流程。');
        }
    })();
...
async function showSingleCoursesForCancellation(prefix, page) {
    ...
    return withDatabaseClient(async (client) => {
        ...
        const hasNextPage = coursesRes.rows.length > CONSTANTS.PAGINATION_SIZE;
        // 【修正點】將錯誤的 res.rows 改為 coursesRes.rows
        const pageCourses = hasNextPage ? coursesRes.rows.slice(0, CONSTANTS.PAGINATION_SIZE) : coursesRes.rows;
...
app.listen(PORT, async () => {
  try {
    ...
    console.log(`Bot 版本 V34.0 (指令處理器重構)`);
