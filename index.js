...
if (!fs.existsSync(PURCHASE_FILE)) fs.writeFileSync(PURCHASE_FILE, JSON.stringify([], null, 2));
...
const teacherMenu = [
  ...
  { type: 'action', action: { type: 'message', label: '新增課程', text: '@新增課程 測試課程 7/15 19:00 10' } },
  { type: 'action', action: { type: 'message', label: '切換身份', text: '@切換身份' } }
];
  ...
  if (user.role === 'student') {
    ...
    if (msg === '@取消課程') {
      ...
      return replyWithMenu(
        event.replyToken,
        list ? `請輸入取消 課程ID，例如：取消 course_123\n\n${list}` : list,
        studentMenu
      );
    }

    if (/^取消 course/.test(msg)) {
      ...
      return replyWithMenu(event.replyToken, '✅ 已取消課程並退還點數', studentMenu);
    }
    ...
    if (/^預約 course_/.test(msg)) {
      ...
      return replyText(event.replyToken, '已額滿，無法預約');
    }
