  ...
  switch (stepData.step) {
    ...
    case 'confirm_purchase':
      if (text === COMMANDS.STUDENT.CONFIRM_BUY_POINTS) {
        ...
        // 觸發 handleStudentCommands 中的取消邏輯
        const simulatedEvent = { ...event, type: 'message', message: { type: 'text', id: 'simulated_cancel_id', text: COMMANDS.STUDENT.CANCEL_PURCHASE } };
...
async function handleEvent(event) {
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (user.role === 'admin' && action === 'select_teacher_for_removal') {
          ...
          pendingTeacherRemoval[userId] = { 
            step: 'await_confirmation',
            targetUser: { id: targetId, name: targetName }
          };
          setupConversationTimeout(userId, pendingTeacherRemoval, 'pendingTeacherRemoval', (u) => push(u, '移除老師操作逾時，自動取消。').catch(e => console.error(e)));
          return reply(replyToken, `您確定要移除老師「${targetName}」的身份嗎？`, [
