...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingManualAdjust[userId]) {
    ...
    switch (state.step) {
      case 'await_student_search':
        ...
        const userBubbles = res.rows.map(u => ({ type: 'bubble', body: { type: 'box', layout: 'horizontal', spacing: 'md', contents: [ { type: 'image', url: u.picture_url || placeholder_avatar, size: 'md', aspectRatio: '1:1', aspectMode: 'cover', cornerRadius: '50%' }, { type: 'box', layout: 'vertical', justifyContent: 'center', contents: [ { type: 'text', text: u.name, weight: 'bold', size: 'lg', wrap: true } ] } ] }, footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', style: 'primary', color: '#1A759F', height: 'sm', action: { type: 'postback', label: '選擇此學員', data: `action=select_student_for_adjust&studentId=${u.id}` } }] } }));
      ...
      case 'await_amount':
        ...
        if (isNaN(amount) || amount <= 0) {
          return reply(replyToken, '點數格式不正確，請輸入一個大於 0 的正整數。');
        }
        state.amount = amount;
      ...
      case 'await_reason':
        ...
        return reply(replyToken, summary, [
          { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST, text: COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST } },
      ...
      case 'await_confirmation':
        if (text === COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST) {
          ...
          try {
            ...
            const studentRes = await client.query('SELECT * FROM users WHERE id = $1 FOR UPDATE', [state.targetStudent.id]);
            ...
            const historyEntry = {
              action: `手動調整：${state.operation === 'add' ? '+' : '-'}${state.amount}點`,
              reason: state.reason,
              time: new Date().toISOString(),
              operator: user.name
            };
            const newHistory = student.history ? [...student.history, historyEntry] : [historyEntry];
            await client.query('UPDATE users SET points = $1, history = $2 WHERE id = $3', [newPoints, JSON.stringify(newHistory), student.id]);
  ...
  if (pendingAnnouncementCreation[userId]) { /* ... */ return; }
  if (pendingReply[userId]) { /* ... */ return; }
  if (pendingStudentSearchQuery[userId]) { /* ... */ return; }
  if (pendingMessageSearchQuery[userId]) { /* ... */ return; }
  if (pendingProductCreation[userId]) { /* ... */ return; }
  ...
  if (text === COMMANDS.TEACHER.POINT_MANAGEMENT) {
    const pointMenu = [
      { type: 'action', action: { type: 'message', label: '待確認清單', text: COMMANDS.TEACHER.PENDING_ORDERS } },
      { type: 'action', action: { type: 'message', label: '手動調整點數', text: COMMANDS.TEACHER.MANUAL_ADJUST_POINTS } },
    ];
    return reply(replyToken, '請選擇點數管理功能：', pointMenu);
  }

  if (text === COMMANDS.TEACHER.STUDENT_MANAGEMENT) {
      const studentMenu = [
        { type: 'action', action: { type: 'message', label: '查詢學員', text: COMMANDS.TEACHER.SEARCH_STUDENT } },
        { type: 'action', action: { type: 'message', label: '查看留言', text: COMMANDS.TEACHER.VIEW_MESSAGES } },
      ];
      return reply(replyToken, '請選擇學員管理功能：', studentMenu);
  }
    ...
    if (event.type === 'message' && (event.message.type === 'text' || event.message.type === 'image')) {
        ...
        if (user.role === 'teacher') {
          if (action === 'select_student_for_adjust') {
              ...
              if (!targetStudent) {
                  return reply(replyToken, '找不到該學員，可能已被刪除。');
              }
              pendingManualAdjust[userId] = {
                  step: 'await_operation',
                  targetStudent: { id: targetStudent.id, name: targetStudent.name }
              };
              return reply(replyToken, `您要為學員「${targetStudent.name}」進行何種點數調整？`, [
