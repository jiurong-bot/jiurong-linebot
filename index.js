const express = require('express'); const line = require('@line/bot-sdk'); const fs = require('fs'); const path = require('path');
...
// === 升級為老師 === if (msg === '@我是老師') { user.role = 'teacher'; writeJSON(DATA_FILE, db); return replyText(event.replyToken, '✅ 您已成為老師，請重新輸入指令'); }

// === 老師功能 === if (user.role === 'teacher') { if (msg === '@今日名單') { const today = new Date().toISOString().slice(0, 10); const list = Object.entries(courses) .filter(([, c]) => c.date.includes(today)) .map(([id, c]) => ${id}: ${c.name} (${c.date})\n學員：${c.students.map(id => id).join(', ') || '無'}) .join('\n\n') || '今日尚無課程'; return replyText(event.replyToken, list); }

if (/^@查學員/.test(msg)) { const targetId = msg.split(' ')[1]; const student = db[targetId]; if (!student) return replyText(event.replyToken, '找不到該學員'); const history = student.history.map(h => `${h.courseId} - ${h.time}`).join('\n') || '無紀錄'; return replyText(event.replyToken, `學員點數：${student.points}\n預約紀錄：\n${history}`); } if (/^@加點/.test(msg)) { const [, targetId, amount] = msg.split(' '); if (!db[targetId]) return replyText(event.replyToken, '找不到該學員'); db[targetId].points += parseInt(amount); writeJSON(DATA_FILE, db); return replyText(event.replyToken, `✅ 已加點成功，學員目前點數：${db[targetId].points}`); } if (/^@扣點/.test(msg)) { const [, targetId, amount] = msg.split(' '); if (!db[targetId]) return replyText(event.replyToken, '找不到該學員'); db[targetId].points -= parseInt(amount); writeJSON(DATA_FILE, db); return replyText(event.replyToken, `✅ 已扣點成功，學員目前點數：${db[targetId].points}`); } if (/^@課程取消/.test(msg)) { const courseId = msg.split(' ')[1]; const course = courses[courseId]; if (!course) return replyText(event.replyToken, '找不到該課程'); course.students.forEach(uid => { if (db[uid]) db[uid].points += 1; }); course.students = []; writeJSON(DATA_FILE, db); writeJSON(COURSE_FILE, courses); return replyText(event.replyToken, '✅ 已取消課程並退還所有學員點數'); } return replyText(event.replyToken, '老師功能指令無效，請重新輸入'); 
