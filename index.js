...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingCourseCancellation[userId]) {
    ...
    switch(state.type) {
      case 'batch':
        if (text === COMMANDS.TEACHER.CONFIRM_BATCH_CANCEL) {
          console.log(`[批次取消] User ${userId} 開始執行 prefix=${state.prefix} 的批次取消。`);
          const client = await pgPool.connect();
          console.log(`[批次取消] 成功從資料庫連接池取得連線。`);
          try {
            await client.query('BEGIN');
            console.log(`[批次取消] Transaction 已開始。`);

            const coursesToCancelRes = await client.query("SELECT * FROM courses WHERE id LIKE $1 AND time > NOW() FOR UPDATE", [`${state.prefix}%`]);
            console.log(`[批次取消] 查詢到 ${coursesToCancelRes.rows.length} 堂要取消的課程。`);
            ...
            console.log(`[批次取消] 建立受影響學員列表完成，共 ${affectedUsers.size} 位。`);
            ...
            for (const [studentId, refundAmount] of affectedUsers.entries()) {
              if (refundAmount > 0) {
                ...
                console.log(`[批次取消] 已退還 ${refundAmount} 點給學員 ${studentId}。`);
              }
            }
            ...
            console.log(`[批次取消] 已從資料庫刪除課程。`);

            await client.query('COMMIT');
            console.log(`[批次取消] Transaction 已提交。`);
    ...
    try {
      await pgPool.query("UPDATE feedback_messages SET status = 'replied', teacher_reply = $1 WHERE id = $2", [text, state.msgId]);
