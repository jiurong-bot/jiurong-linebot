      ...
      const updatedUsersWithFreshProfiles = await Promise.all(
          foundUsers.map(async (dbUser) => {
              try {
                  ...
                  dbUser.picture_url = profile.pictureUrl; // 使用最新的 URL
                  dbUser.name = profile.displayName; // 同步最新的名稱
                  ...
                  return dbUser; // 若 API 失敗，則回傳資料庫內的舊資料
              }
          })
      );
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (user.role === 'teacher') {
            if (action === 'add_announcement_start') {
              ...
              setupConversationTimeout(userId, pendingCourseCreation, 'pendingCourseCreation', (u) => push(u, '新增課程逾時，自動取消。請重新新增。').catch(e => console.error(e)));
              await reply(event.replyToken, '請輸入課程名稱（例如：哈達瑜伽）：', [{ type: 'action', action: { type: 'message', label: COMMANDS.STUDENT.CANCEL_ADD_COURSE, text: COMMANDS.STUDENT.CANCEL_ADD_COURSE } }]); return; 
            }
...
app.listen(PORT, async () => {
  ...
  await cleanCoursesDB(); // 啟動時先清理一次

  console.log(`✅ 伺服器已啟動，監聽埠號 ${PORT}`);
  console.log(`Bot 版本: V7.2 (Refactored)`);
