...
const ALL_COMMANDS = Array.from(new Set([
    ...Object.values(COMMANDS.ADMIN),
    ...Object.values(COMMANDS.TEACHER),
    ...Object.values(COMMANDS.STUDENT)
]));
/**
 * 檢查所有必要的環境變數是否已設定。
...
function checkEnvironmentVariables() {
    const requiredEnvVars = [
        ...
        // 'ADMIN_RICH_MENU_ID', // 此項目已改為選用，不再強制檢查
        'SELF_URL'
    ];
...
const PENDING_STATES = [
    pendingCourseCreation, pendingPurchase, pendingManualAdjust,
    pendingStudentSearchQuery, pendingBookingConfirmation, pendingFeedback,
    pendingReply, pendingMessageSearchQuery, pendingAnnouncementCreation,
    pendingTeacherAddition, pendingTeacherRemoval, pendingProductCreation, pendingCourseCancellation
];

function setupConversationTimeout(userId, conversationState, onTimeout) {
...
async function clearPendingState(userId, pushMessage = true) {
    let stateCleared = false;
    for (const state of PENDING_STATES) {
        if (state[userId]) {
            if (state[userId].timeoutId) clearTimeout(state[userId].timeoutId);
            delete state[userId];
            stateCleared = true;
        }
    }
    if (stateCleared && pushMessage) {
        try {
            await push(userId, '已取消先前的操作。');
        } catch (e) {
            console.error(`向 ${userId} 推播取消訊息失敗: `, e.message);
        }
    }
    return stateCleared;
}
...
async function showTeacherListForRemoval(replyToken, page) {
    const client = await pgPool.connect();
    try {
        const totalRes = await client.query("SELECT COUNT(*) FROM users WHERE role = 'teacher'");
        const totalCount = parseInt(totalRes.rows[0].count, 10);
        
        const offset = (page - 1) * PAGINATION_SIZE;
        const teacherRes = await client.query("SELECT id, name FROM users WHERE role = 'teacher' ORDER BY name LIMIT $1 OFFSET $2", [PAGINATION_SIZE, offset]);
        
        const pageTeachers = teacherRes.rows;
        ...
        const hasNextPage = (offset + pageTeachers.length) < totalCount;
        ...
        const altText = totalPages > 1 ? `請選擇要移除的老師 (第${page}頁)` : '請選擇要移除的老師';
...
async function handleAdminCommands(event, userId) {
  ...
  const isMainMenuCommand = Object.values(COMMANDS.ADMIN).includes(text);
  if (isMainMenuCommand) {
      await clearPendingState(userId, true);
  }
  ...
  if (pendingTeacherAddition[userId]) {
    ...
    if (text === COMMANDS.ADMIN.PANEL) {
      ...
      setupConversationTimeout(userId, pendingTeacherAddition, (u) => push(u, '授權老師操作逾時，自動取消。').catch(e => console.error(e)));
...
async function showPendingOrders(replyToken, page = 1) {
    const client = await pgPool.connect();
    try {
        const totalRes = await client.query("SELECT COUNT(*) FROM orders WHERE status = 'pending_confirmation'");
        const totalCount = parseInt(totalRes.rows[0].count, 10);
        
        const offset = (page - 1) * PAGINATION_SIZE;
        const ordersRes = await client.query(`SELECT * FROM orders WHERE status = 'pending_confirmation' ORDER BY timestamp ASC LIMIT $1 OFFSET $2`, [PAGINATION_SIZE, offset]);
        
        const pageOrders = ordersRes.rows;
        ...
        const totalPages = Math.ceil(totalCount / PAGINATION_SIZE);
        const altText = totalPages > 1 ? `待確認購點訂單列表 (第${page}頁)` : '待確認購點訂單列表';

        return reply(replyToken, { type: 'flex', altText: altText, contents: { type: 'carousel', contents: orderBubbles } });
    } catch (err) {
...
async function showStudentSearchResults(replyToken, query, page = 1) {
    const client = await pgPool.connect();
    try {
        const totalRes = await client.query(`SELECT COUNT(*) FROM users WHERE role = 'student' AND (LOWER(name) LIKE $1 OR id = $2)`, [`%${query.toLowerCase()}%`, query]);
        const totalCount = parseInt(totalRes.rows[0].count, 10);

        const offset = (page - 1) * PAGINATION_SIZE;
        const res = await client.query(`SELECT id, name, picture_url, points FROM users WHERE role = 'student' AND (LOWER(name) LIKE $1 OR id = $2) ORDER BY name LIMIT $3 OFFSET $4`, [`%${query.toLowerCase()}%`, query, PAGINATION_SIZE, offset]);
        
        const pageUsers = res.rows;
        ...
        const hasNextPage = (offset + pageUsers.length) < totalCount;
        ...
        const totalPages = Math.ceil(totalCount / PAGINATION_SIZE);
        const altText = totalPages > 1 ? `學員查詢結果 (第${page}頁)`: '學員查詢結果';

        return reply(replyToken, { type: 'flex', altText: altText, contents: { type: 'carousel', contents: userBubbles } });
    } catch (err) {
...
async function showUnreadMessages(replyToken, page = 1) {
    const client = await pgPool.connect();
    try {
        const totalRes = await client.query("SELECT COUNT(*) FROM feedback_messages WHERE status <> 'replied'");
        const totalCount = parseInt(totalRes.rows[0].count, 10);

        const offset = (page - 1) * PAGINATION_SIZE;
        const messagesRes = await client.query("SELECT * FROM feedback_messages WHERE status <> 'replied' ORDER BY timestamp DESC LIMIT $1 OFFSET $2", [PAGINATION_SIZE, offset]);
        
        const pageMessages = messagesRes.rows;
        ...
        const hasNextPage = (offset + pageMessages.length) < totalCount;
        ...
        const totalPages = Math.ceil(totalCount / PAGINATION_SIZE);
        const altText = totalPages > 1 ? `學員留言列表 (第${page}頁)` : `學員留言列表`;
        const textMessage = page === 1 ? '以下是尚未回覆的學員留言：' : (totalPages > 1 ? `學員留言 (第${page}頁)` : '學員留言');

        const messageArray = [];
        if(page === 1) {
            messageArray.push({ type: 'text', text: textMessage });
        }
        messageArray.push({ type: 'flex', altText: altText, contents: { type: 'carousel', contents: messageBubbles } });
        
        return reply(replyToken, messageArray);
...
async function showCourseSeries(replyToken, page = 1) {
    const client = await pgPool.connect();
    try {
        const totalRes = await client.query("SELECT COUNT(DISTINCT SUBSTRING(id, 1, 2)) FROM courses WHERE time > NOW()");
        const totalCount = parseInt(totalRes.rows[0].count, 10);
        
        const offset = (page - 1) * (PAGINATION_SIZE -1); // Reserve one slot for "add" button
        ...
        const pageSeries = seriesRes.rows;
        ...
        const courseGroupPromises = pageSeries.map(async (row) => {
            ...
            return {
                ...
                pointsCost: details.points_cost
            };
        });
        ...
        const hasNextPage = (offset + pageSeries.length) < totalCount;
        ...
        const totalPages = Math.ceil(totalCount / (PAGINATION_SIZE - 1));
        const altText = totalPages > 1 ? `課程管理選單 (第${page}頁)` : '課程管理選單';
        const textMessage = page === 1 ? '以下為各課程系列的管理選項：' : (totalPages > 1 ? `課程系列 (第${page}頁)` : '課程系列');

        const message = page === 1
            ? [{ type: 'text', text: textMessage }, { type: 'flex', altText: altText, contents: { type: 'carousel', contents: courseBubbles } }]
            : { type: 'flex', altText: altText, contents: { type: 'carousel', contents: courseBubbles } };
...
async function handleTeacherCommands(event, userId) {
  ...
  const isMainMenuCommand = Object.values(COMMANDS.TEACHER).includes(text) || ALL_COMMANDS.includes(text);
  if (isMainMenuCommand) {
      await clearPendingState(userId, true);
  }
  ...
  if (pendingCourseCancellation[userId]) {
    ...
    const cancelMenu = [{ type: 'action', action: { type: 'message', label: cancelText, text: cancelText } }];
    switch (state.step) {
        case 'await_title':
            state.title = text;
        ...
        case 'await_time':
            if (!/^\d{2}:\d{2}$/.test(text)) {
                return reply(replyToken, '時間格式不正確，請輸入四位數時間，例如：19:30', cancelMenu);
            }
            state.time = text;
            state.step = 'await_sessions';
            return reply(replyToken, '請問這個系列總共要開設幾堂課？（請輸入數字）', cancelMenu);
       case 'await_sessions':
            const sessions = parseInt(text, 10);
            if (isNaN(sessions) || sessions <= 0) {
                return reply(replyToken, '堂數必須是正整數，請重新輸入。', cancelMenu);
            }
            state.sessions = sessions;
            state.step = 'await_capacity';
            return reply(replyToken, '請問每堂課的名額限制？（請輸入數字）', cancelMenu);
        case 'await_capacity':
            const capacity = parseInt(text, 10);
            if (isNaN(capacity) || capacity <= 0) {
                return reply(replyToken, '名額必須是正整數，請重新輸入。', cancelMenu);
            }
            state.capacity = capacity;
            state.step = 'await_points';
            return reply(replyToken, '請問每堂課需要消耗多少點數？（請輸入數字）', cancelMenu);
        case 'await_points':
            const points = parseInt(text, 10);
            if (isNaN(points) || points < 0) {
                return reply(replyToken, '點數必須是正整數或 0，請重新輸入。', cancelMenu);
            }
            ...
            return reply(replyToken, summary, [
                ...
                { type: 'action', action: { type: 'message', label: cancelText, text: cancelText } }
            ]);
        case 'await_confirmation':
            if (text === '✅ 確認新增') {
                const client = await pgPool.connect();
                ...
                return reply(replyToken, '請點擊「✅ 確認新增」或「❌ 取消」。');
            }
    }
    ...
    const cancelMenu = [{ type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST, text: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST } }];
    switch (state.step) {
      case 'await_student_search':
        ...
        if (res.rows.length === 0) {
          return reply(replyToken, `找不到學員「${text}」。請重新輸入或取消操作。`, cancelMenu);
        }
        const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
      ...
      case 'await_operation':
        if (text === COMMANDS.TEACHER.ADD_POINTS || text === COMMANDS.TEACHER.DEDUCT_POINTS) {
          ...
          return reply(replyToken, `請輸入要 ${text === COMMANDS.TEACHER.ADD_POINTS ? '增加' : '扣除'} 的點數數量 (純數字)：`, cancelMenu);
        } else {
      ...
      case 'await_amount':
        ...
        if (isNaN(amount) || amount <= 0) { return reply(replyToken, '點數格式不正確，請輸入一個大於 0 的正整數。', cancelMenu); }
        state.amount = amount;
        state.step = 'await_reason';
        return reply(replyToken, '請輸入調整原因（例如：活動獎勵、課程補償等）：', cancelMenu);
    ...
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
        ...
        setupConversationTimeout(userId, pendingManualAdjust, (u) => push(u, '手動調整點數逾時，自動取消。').catch(e => console.error(e)));
        ...
        setupConversationTimeout(userId, pendingStudentSearchQuery, (u) => {
            if (pendingStudentSearchQuery[u]) {
                delete pendingStudentSearchQuery[u];
                push(u, '查詢學員逾時，自動取消。').catch(e => console.error(e));
            }
        });
        return reply(replyToken, '請輸入您想查詢的學員姓名或 User ID：', [{ type: 'action', action: { type: 'message', label: '❌ 取消', text: '❌ 取消' } }]);
...
async function showPurchaseHistory(replyToken, userId, page) {
    const client = await pgPool.connect();
    try {
        const totalRes = await client.query("SELECT COUNT(*) FROM orders WHERE user_id = $1", [userId]);
        const totalCount = parseInt(totalRes.rows[0].count, 10);

        const offset = (page - 1) * PAGINATION_SIZE;
        const res = await client.query(`SELECT * FROM orders WHERE user_id = $1 ORDER BY timestamp DESC LIMIT $2 OFFSET $3`, [userId, PAGINATION_SIZE, offset]);
        
        const pageOrders = res.rows;
        ...
        const totalPages = Math.ceil(totalCount / PAGINATION_SIZE);
        const altText = totalPages > 1 ? `購點紀錄 (第${page}頁)` : '購點紀錄';
        const textMessage = page === 1 ? '以下是您近期的購點紀錄：' : altText;

        const message = page === 1
            ? [{ type: 'text', text: textMessage }, { type: 'flex', altText: altText, contents: { type: 'carousel', contents: historyBubbles } }]
            : { type: 'flex', altText: altText, contents: { type: 'carousel', contents: historyBubbles } };
...
async function handleStudentCommands(event, userId) {
  ...
  const isMainMenuCommand = Object.values(COMMANDS.STUDENT).includes(text);
  if (isMainMenuCommand) {
      await clearPendingState(userId, true);
  }
  ...
  if (pendingBookingConfirmation[userId]) {
    ...
    if (text === COMMANDS.STUDENT.LATEST_ANNOUNCEMENT) {
      ...
      setupConversationTimeout(userId, pendingFeedback, (u) => push(u, '留言逾時，自動取消。').catch(e => console.error(e)));
      ...
      return reply(replyToken, flexMenu);
        ...
        try {
            ...
            if (orderRes.rows.length > 0) {
                ...
                setupConversationTimeout(userId, pendingPurchase, (u) => push(u, '輸入後五碼逾時，自動取消。').catch(e => console.error(e)));
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V16.2 (智慧取消與UX優化)`);
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (user.role === 'admin') {
            if (action === 'select_teacher_for_removal') {
                ...
                setupConversationTimeout(userId, pendingTeacherRemoval, (u) => push(u, '移除老師操作逾時，自動取消。').catch(e => console.error(e)));
        ...
        if (user.role === 'teacher') {
          if (action === 'add_course_start') {
              ...
              setupConversationTimeout(userId, pendingCourseCreation, (u) => push(u, '新增課程逾時，自動取消。').catch(e => console.error(e)));
          ...
          if (action === 'set_course_weekday') {
           ...
           if (action === 'cancel_course_group_confirm') {
                ...
                setupConversationTimeout(userId, pendingCourseCancellation, (u) => push(u, '取消課程操作逾時。').catch(e => console.error(e)));
            ...
            if (action === 'reply_feedback') {
                ...
                if (msgRes.rows.length > 0) {
                    ...
                    setupConversationTimeout(userId, pendingReply, (u) => push(u, '回覆留言逾時，自動取消。').catch(e => console.error(e)));
                    return reply(replyToken, '請輸入您要回覆的內容，或輸入「❌ 取消」。', [{ type: 'action', action: { type: 'message', label: '❌ 取消', text: '❌ 取消' } }]);
                }
            }
        } 
