/*
 * index.js - V21.0 (邏輯修正版)
 * =================================================================
 * 功能簡介 (Functional Overview):
 * - 重構 handleEvent 核心邏輯，讓 ADMIN_USER_ID 的指令固定由 Admin Handler 處理，徹底解決角色切換的競態條件問題。
 * - 移除了 V20.2 中的偵錯專用日誌，是可供長期部署的穩定版本。
 *
...
async function handleEvent(event) {
    ...
    // --- V21.0 核心修正邏輯 ---
    // 只要是 Admin User 發來的訊息或事件，都優先由 Admin Handler 處理
    if (userId === ADMIN_USER_ID) {
        // 如果指令是 @管理模式，則確保角色被設為 admin
        if (event.type === 'message' && event.message.text && event.message.text.trim() === COMMANDS.ADMIN.PANEL) {
            if (user.role !== 'admin') {
        ...
        // 將事件交給 Admin Handler，讓它來判斷如何處理模擬身份等指令
        return handleAdminCommands(event, user, userState);
    }

    // --- 對於非 Admin User，才根據資料庫中的角色來判斷 ---
...
app.get('/', (req, res) => res.send('九容瑜伽 LINE Bot (V21.0 邏輯修正版) 正常運作中。'));

// --- 伺服器啟動 ---
app.listen(PORT, async () => {
  await initializeDatabase();
  console.log(`✅ 伺服器已啟動，監聽埠號 ${PORT}`);
  console.log(`Bot 版本: V21.0 (邏輯修正版)`);
