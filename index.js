...
const CONSTANTS = {
  ...
  INTERVALS: {
    ...
    REMINDER_CHECK_INTERVAL_MS: 1000 * 60 * 5, // [V24.0] 此常數保留但不再用於課程提醒
    CONVERSATION_TIMEOUT_MS: 1000 * 60 * 5,
    NOTIFICATION_CACHE_DURATION_MS: 1000 * 30, // 快取 30 秒
  },
  PAGINATION_SIZE: 9, // 用於分頁功能的每頁項目數
  PURCHASE_PLANS: [
                ...
                contents: [
                    {
                        ...
                        action: { type: 'postback', label: '➕ 購買點數', data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.STUDENT.BUY_POINTS)}` } // [V23.0 修改]
                    },
                    {
                        type: 'button', style: 'secondary', height: 'sm',
                        action: { type: 'postback', label: '📜 查詢購點紀錄', data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.STUDENT.PURCHASE_HISTORY)}` } // [V23.0 修改]
                    }
                ]
            }
        }
...
const teacherMenu = [];
const studentMenu = [
    { type: 'action', action: { type: 'postback', label: '點數查詢', data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.STUDENT.POINTS)}` } }, // [V23.0 修改]
    { type: 'action', action: { type: 'postback', label: '預約課程', data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.STUDENT.BOOK_COURSE)}` } }, // [V23.0 修改]
    { type: 'action', action: { type: 'postback', label: '我的課程', data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.STUDENT.MY_COURSES)}` } }, // [V23.0 修改]
    { type: 'action', action: { type: 'postback', label: '最新公告', data: `action=run_command&text=${encodeURIComponent(CONSTANTS.COMMANDS.STUDENT.LATEST_ANNOUNCEMENT)}` } } // [V23.0 修改]
];
const WEEKDAYS = [
...
// [新增] 主要函式：顯示失敗任務列表
async function showFailedTasks(replyToken, page) {
    ...
    try {
        ...
        if (pageTasks.length === 0 && page === 1) {
            return reply(replyToken, '✅ 太好了！目前沒有任何失敗的任務。');
        }
        if (pageTasks.length === 0) {
            return reply(replyToken, '沒有更多失敗的任務了。');
        }
        ...
        return reply(replyToken, { type: 'flex', altText: '失敗任務列表', contents: { type: 'carousel', contents: taskBubbles } });
    } catch (err) {
        await handleError(err, replyToken, '查詢失敗任務列表');
    } finally {
        if (client) client.release();
    }
}
...
ℹ️ 「待處理任務」是系統即將要發送的排程訊息 (如課程提醒)。若「失敗任務」數量持續增加，請檢查 Worker 紀錄。
    ...
    await handleError(err, replyToken, '查詢系統狀態');
  } finally {
    if (db) db.release();
  }
}
...
async function showTeacherListForRemoval(replyToken, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢老師列表');
    } finally {
        if (client) client.release();
    }
}
  ...
  if (pendingTeacherAddition[userId]) {
    ...
    switch (state.step) {
      ...
      const adminMenu = [
        ...
        { type: 'action', action: { type: 'message', label: failedTasksLabel, text: CONSTANTS.COMMANDS.ADMIN.FAILED_TASK_MANAGEMENT } }, // 使用動態標籤
...
  return showFailedTasks(replyToken, 1); // 顯示第一頁
        ...
        try {
            ...
            await handleError(err, replyToken, '切換推播通知設定');
        } finally {
            if (db) db.release();
        }
...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingProductCreation[userId]) {
    ...
    let errorMessage = '';
    switch (state.step) {
        case 'await_name': state.name = text; state.step = 'await_description'; await reply(replyToken, '請輸入商品描述 (可換行)，或輸入「無」：', getCancelMenu()); break;
        case 'await_description': state.description = text === '無' ? null : text; state.step = 'await_price'; await reply(replyToken, '請輸入商品兌換價格 (點數，純數字)：', getCancelMenu()); break;
        case 'await_price':
            const price = parseInt(text, 10);
            if (isNaN(price) || price < 0) { proceed = false; errorMessage = '價格格式不正確，請輸入一個非負整數。'; } 
            else { state.price = price; state.step = 'await_inventory'; await reply(replyToken, '請輸入商品初始庫存 (純數字)：', getCancelMenu()); }
            break;
        case 'await_inventory':
            ...
            else { state.inventory = inventory; state.step = 'await_image_url'; await reply(replyToken, '請直接上傳一張商品圖片，或輸入「無」：', getCancelMenu()); }
            break;
        case 'await_image_url':
            ...
            if (!proceedToNextStep) { await reply(event.replyToken, errorImageUrlMessage, getCancelMenu()); return; }
            ...
            await reply(event.replyToken, summaryText, [ { type: 'action', action: { type: 'postback', label: '✅ 確認上架', data: 'action=confirm_add_product' } }, { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } } ]);
            break;
    }
    if (!proceed && state.step !== 'await_image_url') { await reply(replyToken, errorMessage, getCancelMenu()); }
    ...
    if (!isValid) { return reply(replyToken, errorMessage, getCancelMenu()); }
    ...
    if(isNaN(numValue)) { return reply(replyToken, '格式錯誤，請輸入一個整數 (正數為增加，負數為減少)。', getCancelMenu()); }
    const newInventory = product.inventory + numValue;
    if(newInventory < 0) { return reply(replyToken, `庫存調整失敗，調整後庫存 (${newInventory}) 不可小於 0。`, getCancelMenu()); }
    ...
    switch (state.step) {
      ...
      case 'await_confirmation':
        ...
        } else { return reply(replyToken, '請點擊「確認頒佈」或「取消操作」。'); }
    }
    ...
    } else { return reply(replyToken, '請點擊「確認刪除」或「取消操作」。'); }
      ...
      case 'single':
         if (text === CONSTANTS.COMMANDS.TEACHER.CONFIRM_SINGLE_CANCEL) {
            ...
            try {
              ...
              if (courseToCancelRes.rows.length === 0) { delete pendingCourseCancellation[userId]; return reply(replyToken, "找不到該課程，可能已被取消。"); }
            ...
            } catch (e) { await client.query('ROLLBACK'); delete pendingCourseCancellation[userId]; console.error('單堂取消課程失敗:', e); return reply(replyToken, '取消課程時發生錯誤，請稍後再試。');
            } finally { if(client) client.release(); }
         }
        break;
    }
    ...
    switch (state.step) {
        case 'await_title': state.title = text; state.step = 'await_weekday'; const weekdayButtons = WEEKDAYS.map(day => ({ type: 'action', action: { type: 'postback', label: day.label, data: `action=set_course_weekday&day=${day.value}` } })); return reply(replyToken, `課程標題：「${text}」\n\n請問課程固定在每週的哪一天？`, weekdayButtons);
        case 'await_time': if (!/^\d{2}:\d{2}$/.test(text)) { return reply(replyToken, '時間格式不正確，請輸入四位數時間，例如：19:30', getCancelMenu()); } state.time = text; state.step = 'await_sessions'; return reply(replyToken, '請問這個系列總共要開設幾堂課？（請輸入數字）', getCancelMenu());
        case 'await_sessions': const sessions = parseInt(text, 10); if (isNaN(sessions) || sessions <= 0) { return reply(replyToken, '堂數必須是正整數，請重新輸入。', getCancelMenu()); } state.sessions = sessions; state.step = 'await_capacity'; return reply(replyToken, '請問每堂課的名額限制？（請輸入數字）', getCancelMenu());
        case 'await_capacity': const capacity = parseInt(text, 10); if (isNaN(capacity) || capacity <= 0) { return reply(replyToken, '名額必須是正整數，請重新輸入。', getCancelMenu()); } state.capacity = capacity; state.step = 'await_points'; return reply(replyToken, '請問每堂課需要消耗多少點數？（請輸入數字）', getCancelMenu());
        case 'await_points':
            const points = parseInt(text, 10); if (isNaN(points) || points < 0) { return reply(replyToken, '點數必須是正整數或 0，請重新輸入。', getCancelMenu()); }
            ...
            return reply(replyToken, summary, [ { type: 'action', action: { type: 'message', label: '✅ 確認新增', text: '✅ 確認新增' } }, { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } } ]);
        case 'await_confirmation':
            if (text === '✅ 確認新增') {
                const client = await pgPool.connect();
                try {
                ...
                } catch (e) { await client.query('ROLLBACK'); console.error("新增課程系列失敗:", e); delete pendingCourseCreation[userId]; return reply(replyToken, '新增課程時發生錯誤，請稍後再試。');
                } finally { if(client) client.release(); }
            } else { return reply(replyToken, '請點擊「✅ 確認新增」或「❌ 取消操作」。'); }
    }
    ...
    switch (state.step) {
      case 'await_student_search':
        ...
        if (res.rows.length === 0) { return reply(replyToken, `找不到學員「${text}」。請重新輸入或取消操作。`, getCancelMenu()); }
        const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
      ...
      case 'await_operation':
        if (text === CONSTANTS.COMMANDS.TEACHER.ADD_POINTS || text === CONSTANTS.COMMANDS.TEACHER.DEDUCT_POINTS) { state.operation = text === CONSTANTS.COMMANDS.TEACHER.ADD_POINTS ? 'add' : 'deduct'; state.step = 'await_amount'; return reply(replyToken, `請輸入要 ${text === CONSTANTS.COMMANDS.TEACHER.ADD_POINTS ? '增加' : '扣除'} 的點數數量 (純數字)：`, getCancelMenu()); } 
        else { return reply(replyToken, '請點擊 `+ 加點` 或 `- 扣點` 按鈕。'); }
      case 'await_amount': const amount = parseInt(text, 10); if (isNaN(amount) || amount <= 0) { return reply(replyToken, '點數格式不正確，請輸入一個大於 0 的正整數。', getCancelMenu()); } state.amount = amount; state.step = 'await_reason'; return reply(replyToken, '請輸入調整原因（例如：活動獎勵、課程補償等）：', getCancelMenu());
      case 'await_reason': state.reason = text; state.step = 'await_confirmation'; const opText = state.operation === 'add' ? `增加 ${state.amount} 點` : `扣除 ${state.amount} 點`; const summary = `請確認調整內容：\n\n對象：${state.targetStudent.name}\n操作：${opText}\n原因：${state.reason}`; return reply(replyToken, summary, [ { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST, text: CONSTANTS.COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST } }, { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } } ]);
      case 'await_confirmation':
        if (text === CONSTANTS.COMMANDS.TEACHER.CONFIRM_MANUAL_ADJUST) {
          ...
          try {
            ...
            if (newPoints < 0) { await clientDB.query('ROLLBACK'); delete pendingManualAdjust[userId]; return reply(replyToken, `操作失敗：學員 ${student.name} 的點數不足以扣除 ${state.amount} 點。`); }
            ...
            delete pendingManualAdjust[userId]; await reply(replyToken, `✅ 已成功為學員 ${student.name} ${state.operation === 'add' ? '增加' : '扣除'} ${state.amount} 點。`);
          ...
          } catch (e) { await clientDB.query('ROLLBACK'); console.error('手動調整點數失敗:', e); delete pendingManualAdjust[userId]; return reply(replyToken, '❌ 操作失敗，資料庫發生錯誤，請稍後再試。');
    ...
    try {
      ...
      delete pendingReply[userId];
      await handleError(err, replyToken, '回覆留言');
    } finally {
        if(client) client.release();
    }
    ...
    if (text === CONSTANTS.COMMANDS.TEACHER.COURSE_MANAGEMENT) {
        ...
        return reply(replyToken, '請輸入新課程系列的標題（例如：高階空中瑜伽），或按「取消」來放棄操作。', getCancelMenu());
        ...
        return reply(replyToken, '請輸入您想查詢的學員姓名或留言關鍵字：', getCancelMenu());
        ...
        try {
            ...
            if (result === 'timeout') {
                ...
                const reportText = await generateReportTask(); await enqueuePushTask(userId, { type: 'text', text: reportText });
            } else { await reply(replyToken, result); }
        } catch (err) { console.error('❌ 即時生成點數報表失敗:', err); await reply(replyToken, '❌ 產生報表時發生錯誤，請稍後再試。'); }
...
async function showStudentSearchResults(replyToken, query, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '搜尋學員');
    } finally {
        if(client) client.release();
    }
}
...
async function showPurchaseHistory(replyToken, userId, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢購點紀錄');
    } finally {
        if(client) client.release();
    }
}
...
async function showUnreadMessages(replyToken, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢未回覆留言');
    } finally {
        if(client) client.release();
    }
}
...
async function showHistoricalMessages(replyToken, query, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '搜尋歷史留言');
    } finally {
        if (client) client.release();
    }
}
...
async function showPendingShopOrders(replyToken, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢待處理商品訂單');
    } finally {
        if (client) client.release();
    }
}
...
async function showAnnouncementsForDeletion(replyToken, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢公告列表');
    } finally {
        if (client) client.release();
    }
}
...
async function showCourseSeries(replyToken, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢課程系列');
    } finally {
        if (client) client.release();
    }
}
...
async function showPendingOrders(replyToken, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢待確認訂單');
    } finally {
        if(client) client.release();
    }
}
...
async function showAvailableCourses(replyToken, userId, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢可預約課程');
    } finally {
        if(client) client.release();
    }
}
...
async function showMyCourses(replyToken, userId, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢我的課程');
    } finally {
        if(client) client.release();
    }
}
...
    } catch (err) {
        await handleError(err, replyToken, '查詢個人留言');
    } finally {
        if (client) client.release();
    }
                     }
async function showShopProducts(replyToken, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢商城商品');
    } finally {
        if(client) client.release();
    }
}
...
async function showSingleCoursesForCancellation(replyToken, prefix, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢單堂課程');
    } finally {
        if(client) client.release();
    }
}
...
async function showProductManagementList(replyToken, page = 1, filter = null) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢商品管理列表');
    } finally {
        if(client) client.release();
    }
}
...
async function showStudentExchangeHistory(replyToken, userId, page = 1) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢兌換紀錄');
    } finally {
        if(client) client.release();
    }
}
...
async function showCourseRosterSummary(replyToken, page) {
    ...
    try {
        ...
        await handleError(err, replyToken, '查詢課程報名概況');
    } finally {
        if (client) client.release();
    }
}
...
async function showCourseRosterDetails(replyToken, courseId) {
    ...
    try {
        ...
        await handleError(err, replyToken, '顯示課程詳細名單');
    } finally {
        if (client) client.release();
    }
}
  ...
  if (pendingBookingConfirmation[userId]) {
    ...
    if (text === CONSTANTS.COMMANDS.STUDENT.BOOK_COURSE) {
        ...
        try {
            ...
            await handleError(err, replyToken, '查詢最新公告');
        } finally {
            if (client) client.release();
        }
...
                    return reply(replyToken, `✅ 已成功刪除失敗任務 #${failedTaskId}。`);
                } else {
                    return reply(replyToken, '找不到該失敗任務，可能已被刪除。');
                }
            } catch (err) {
                console.error(`❌ 刪除失敗任務 ${failedTaskId} 失敗:`, err);
                return reply(replyToken, '刪除任務時發生錯誤。');
            }
        }
        ...
        if (action === 'select_announcement_for_deletion') {
            if (user.role === 'teacher') {
                ...
                const menu = [
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.TEACHER.CONFIRM_DELETE_ANNOUNCEMENT, text: CONSTANTS.COMMANDS.TEACHER.CONFIRM_DELETE_ANNOUNCEMENT } },
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } }
                ];
        ...
        if (user.role === 'admin') {
            if (action === 'select_teacher_for_removal') {
                ...
                return reply(replyToken, `您確定要移除老師「${targetName}」的權限嗎？該用戶將會變回學員身份。`, [
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.ADMIN.CONFIRM_REMOVE_TEACHER, text: CONSTANTS.COMMANDS.ADMIN.CONFIRM_REMOVE_TEACHER } },
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } }
                ]);
            }
        }
        ...
        if (user.role === 'teacher') {
          ...
          if (action === 'set_course_weekday') {
           ...
           if (action === 'cancel_course_group_confirm') {
                ...
                return reply(replyToken, message, [
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.TEACHER.CONFIRM_BATCH_CANCEL, text: CONSTANTS.COMMANDS.TEACHER.CONFIRM_BATCH_CANCEL } },
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } }
                ]);
            }
            if (action === 'confirm_single_course_cancel') {
                ...
                return reply(replyToken, message, [
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.TEACHER.CONFIRM_SINGLE_CANCEL, text: CONSTANTS.COMMANDS.TEACHER.CONFIRM_SINGLE_CANCEL } },
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } }
                ]);
            }
            if (action === 'select_student_for_adjust') {
              ...
              return reply(replyToken, `您要為學員「${targetStudent.name}」進行何種點數調整？`, [ { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.TEACHER.ADD_POINTS, text: CONSTANTS.COMMANDS.TEACHER.ADD_POINTS } }, { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.TEACHER.DEDUCT_POINTS, text: CONSTANTS.COMMANDS.TEACHER.DEDUCT_POINTS } }, { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } } ]);
            }
            if (action === 'view_student_details') {
                ...
                try {
                    ...
                    await handleError(err, replyToken, '查詢學員詳細資料');
                } finally {
                    if (client) client.release();
                }
            }
        ...
        if (user.role === 'student') {
            if (action === 'select_purchase_plan') {
                ...
                const selectedPlan = CONSTANTS.PURCHASE_PLANS.find(p => p.points === planPoints);
                if (selectedPlan) {
...
try {
    ...
    // 這邊失敗沒關係，不影響主要預約流程，只需記錄錯誤即可
            ...
            if (action === 'confirm_cancel_booking_start') {
                ...
                return reply(replyToken, message, [
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING, text: CONSTANTS.COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING } },
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } }
                ]);
            }
            if (action === 'confirm_cancel_waiting_start') {
                ...
                return reply(replyToken, message, [
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.STUDENT.CONFIRM_CANCEL_WAITING, text: CONSTANTS.COMMANDS.STUDENT.CONFIRM_CANCEL_WAITING } },
                    { type: 'action', action: { type: 'message', label: CONSTANTS.COMMANDS.GENERAL.CANCEL, text: CONSTANTS.COMMANDS.GENERAL.CANCEL } }
                ]);
            }
