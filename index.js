...
async function handleStudentCommands(event) {
    ...
    if (text === COMMANDS.STUDENT.CONTACT_US) {
        await setUserState(userId, 'awaiting_feedback');
        const flexMessage = {
          type: 'flex',
          altText: '聯絡我們',
          contents: {
    ...
    if (text === COMMANDS.STUDENT.PURCHASE_HISTORY) {
        ...
        user.history.slice(-5).reverse().forEach(record => { historyMessage += `・${record.action || '未知操作'} (${formatDateTime(record.time)})\n`; });
    ...
    if (text === COMMANDS.STUDENT.BOOK_COURSE) {
        ...
        if (upcomingCourses.length === 0) {
          return reply(replyToken, '未來七天内沒有您可以預約的新課程。');
        }
...
async function handleTeacherPostbacks(event, action, data) {
    const { replyToken, source: { userId } } = event;
    const { orderId, prefix, courseId, msgId, studentId, plan } = Object.fromEntries(data);
    const now = Date.now();
    ...
    switch (action) {
        ...
        case 'history_announcements_show':
            const annRes = await pgPool.query('SELECT * FROM announcements ORDER BY created_at DESC LIMIT 10');
            if (annRes.rows.length === 0) return reply(replyToken, '目前沒有歷史公告。');
            const annBubbles = annRes.rows.map(ann => ({
                type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: `公告 #${ann.id}`, color: '#ffffff', weight: 'bold' }], backgroundColor: '#1A759F', paddingAll: 'lg' },
                body: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: ann.content, wrap: true }] },
                footer: { type: 'box', layout: 'vertical', spacing: 'sm', contents: [{ type: 'text', text: `發布時間: ${formatDateTime(ann.created_at)}`, size: 'xs', color: '#aaaaaa', align: 'center' }, { type: 'button', style: 'secondary', color: '#de5246', height: 'sm', action: { type: 'postback', label: '刪除此公告', data: `action=delete_announcement&annId=${ann.id}` } }] }
            }));
            return reply(replyToken, { type: 'flex', altText: '歷史公告列表', contents: { type: 'carousel', contents: annBubbles } });

        case 'delete_announcement':
            await pgPool.query('DELETE FROM announcements WHERE id = $1', [annId]);
            return reply(replyToken, `公告 #${annId} 已刪除。`);
            
        case 'start_student_search':
            await setUserState(userId, 'awaiting_student_search');
            return reply(replyToken, '請輸入學員姓名或ID來查詢：');

        case 'start_message_search':
            await setUserState(userId, 'awaiting_message_search');
            return reply(replyToken, '請輸入留言內容、學員姓名或回覆內容來查詢：');

        case 'reply_feedback':
            const msgToReply = (await pgPool.query("SELECT * FROM feedback_messages WHERE id = $1", [msgId])).rows[0];
            if (!msgToReply) return reply(replyToken, '該留言已不存在。');
            
            await setUserState(userId, 'awaiting_reply', { msgId: msgId, targetUserId: userId });
            return reply(replyToken, `請輸入您要回覆給「${msgToReply.user_name}」的訊息：`);

        case 'mark_feedback_read':
            await pgPool.query("UPDATE feedback_messages SET status = 'read' WHERE id = $1", [msgId]);
            return reply(replyToken, `留言 #${msgId} 已標示為已讀。`);

        case 'confirm_order':
            const transactionClientConfirm = await pgPool.connect();
            try {
                await transactionClientConfirm.query('BEGIN');
                const orderToConfirmRes = await transactionClientConfirm.query('SELECT * FROM orders WHERE order_id = $1 FOR UPDATE', [orderId]);
                const orderToConfirm = orderToConfirmRes.rows[0];
                if (!orderToConfirm || orderToConfirm.status !== 'pending_confirmation') throw new Error('訂單狀態不正確或已處理。');
                
                const userToUpdateRes = await transactionClientConfirm.query('SELECT * FROM users WHERE id = $1 FOR UPDATE', [orderToConfirm.user_id]);
                const userToUpdate = userToUpdateRes.rows[0];
                if (!userToUpdate) throw new Error('找不到學員資料。');
                
                userToUpdate.points += orderToConfirm.points;
                if (!Array.isArray(userToUpdate.history)) userToUpdate.history = [];
                userToUpdate.history.push({ id: orderId, action: `購點成功 (入帳 ${orderToConfirm.points} 點)`, time: new Date().toISOString() });
                await saveUser(userToUpdate, transactionClientConfirm);
                
                await pgPool.query("UPDATE orders SET status = 'completed', transaction_time = NOW() WHERE order_id = $1", [orderId]);
                await transactionClientConfirm.query('COMMIT');
                
                push(userToUpdate.id, `✅ 您的訂單 #${orderId} 已確認並入帳。\n已新增 ${orderToConfirm.points} 點，目前點數為 ${userToUpdate.points} 點。`).catch(e => console.error(e));
                return reply(replyToken, `已確認訂單 #${orderId}，並已為學員 **${userToUpdate.name || ' '}** 加點完成。`);
            } catch (err) {
                await transactionClientConfirm.query('ROLLBACK');
                console.error(`❌ 確認訂單 #${orderId} 失敗:`, err.message);
                return reply(replyToken, `確認訂單失敗：${err.message}`);
            } finally {
                transactionClientConfirm.release();
            }

        case 'reject_order':
            const transactionClientReject = await pgPool.connect();
            try {
                await transactionClientReject.query('BEGIN');
                const orderToRejectRes = await transactionClientReject.query('SELECT * FROM orders WHERE order_id = $1 FOR UPDATE', [orderId]);
                const orderToReject = orderToRejectRes.rows[0];
                if (!orderToReject || orderToReject.status !== 'pending_confirmation') throw new Error('訂單狀態不正確或已處理。');
                
                await pgPool.query("UPDATE orders SET status = 'rejected' WHERE order_id = $1", [orderId]);
                await transactionClientReject.query('COMMIT');
                
                push(orderToReject.user_id, `❌ 您的訂單 #${orderId} 已被老師退回。\n請至「點數管理」重新操作或確認匯款資訊是否有誤。`).catch(e => console.error(e));
                return reply(replyToken, `已退回訂單 #${orderId}。`);
            } catch (err) {
                await transactionClientReject.query('ROLLBACK');
                console.error(`❌ 退回訂單 #${orderId} 失敗:`, err.message);
                return reply(replyToken, `退回訂單失敗：${err.message}`);
            } finally {
                transactionClientReject.release();
            }

        case 'start_course_creation':
        case 'add_course_start':
            await setUserState(userId, 'awaiting_course_creation', { step: 1 });
            return reply(replyToken, '請輸入課程系列名稱（例如：初級瑜伽課程）：');

        case 'cancel_course_group_confirm':
            const courseCountRes = await pgPool.query("SELECT COUNT(*) FROM courses WHERE id LIKE $1 AND time > NOW()", [`${prefix}%`]);
            const courseCount = courseCountRes.rows[0].count;
            if (courseCount === 0) return reply(replyToken, `此系列代碼 ${prefix} 的課程已全部結束或不存在。`);
            
            const cancelConfirmMsg = { type: 'flex', altText: `確認批次取消 ${prefix} 系列課程`, contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [{ type: 'text', text: `您確定要批次取消系列代碼為「${prefix}」的所有共 ${courseCount} 堂課程嗎？`, wrap: true, size: 'md' }, { type: 'button', style: 'primary', color: '#de5246', action: { type: 'postback', label: '✅ 確認取消', data: `action=cancel_course_group&prefix=${prefix}`, displayText: `確認取消系列課程 ${prefix}` } }, { type: 'button', style: 'secondary', action: { type: 'postback', label: '❌ 放棄', data: `action=run_command&text=${COMMANDS.TEACHER.COURSE_LIST}` } }] } } };
            return reply(replyToken, cancelConfirmMsg);

        case 'cancel_course_group':
            const transactionClientGroupCancel = await pgPool.connect();
            try {
                await transactionClientGroupCancel.query('BEGIN');
                const coursesToCancel = (await transactionClientGroupCancel.query("SELECT * FROM courses WHERE id LIKE $1 AND time > NOW() FOR UPDATE", [`${prefix}%`])).rows;
                if (coursesToCancel.length === 0) throw new Error('沒有找到可取消的課程。');
                
                const refunds = {};
                for (const course of coursesToCancel) {
                    for (const studentId of course.students) {
                        refunds[studentId] = (refunds[studentId] || 0) + course.points_cost;
                    }
                }
                
                for (const studentId in refunds) {
                    const userRes = await transactionClientGroupCancel.query('SELECT * FROM users WHERE id = $1 FOR UPDATE', [studentId]);
                    const user = userRes.rows[0];
                    if (user) {
                        user.points += refunds[studentId];
                        if (!user.history) user.history = [];
                        user.history.push({ action: `課程系列「${prefix}」批次取消 (退 ${refunds[studentId]} 點)`, time: new Date().toISOString() });
                        await saveUser(user, transactionClientGroupCancel);
                        push(studentId, `‼️ 老師已批次取消課程系列「${coursesToCancel[0].title.split(' - ')[0]}」。已退還您共 ${refunds[studentId]} 點。`).catch(e => console.error(e));
                    }
                }
                
                await deleteCoursesByPrefix(prefix, transactionClientGroupCancel);
                await transactionClientGroupCancel.query('COMMIT');
                return reply(replyToken, `已成功批次取消系列代碼「${prefix}」的 ${coursesToCancel.length} 堂課程，並退還學員點數。`);
            } catch (err) {
                await transactionClientGroupCancel.query('ROLLBACK');
                console.error(`❌ 批次取消課程系列 ${prefix} 失敗:`, err.message);
                return reply(replyToken, `批次取消失敗：${err.message}`);
            } finally {
                transactionClientGroupCancel.release();
            }

        case 'manage_course_group':
            const coursesInGroup = Object.values(await getAllCourses())
                .filter(c => c.id.startsWith(prefix) && new Date(c.time).getTime() > now)
                .sort((a,b) => new Date(a.time) - new Date(b.time));
            
            if (coursesInGroup.length === 0) return reply(replyToken, `系列代碼 ${prefix} 的課程已全部結束或不存在。`);
            
            const groupBubbles = coursesInGroup.map(c => ({
                type: 'bubble',
                header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: c.title, weight: 'bold', wrap: true, color: '#ffffff' }], backgroundColor: '#1A759F' },
                body: {
                    type: 'box', layout: 'vertical', spacing: 'md',
                    contents: [
                        { type: 'text', text: `時間：${formatDateTime(c.time)}`, size: 'sm' },
                        { type: 'text', text: `狀態：報名 ${c.students.length}/${c.capacity}`, size: 'sm' }
                    ]
                },
                footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', style: 'primary', color: '#de5246', height: 'sm', action: { type: 'postback', label: '取消這堂課', data: `action=cancel_course_confirm&courseId=${c.id}` } }] }
            }));
            
            const backButton = { type: 'bubble', body: { type: 'box', layout: 'vertical', paddingAll: 'xxl', contents: [{ type: 'box', layout: 'vertical', contents: [{ type: 'text', text: '←', size: 'xxl', weight: 'bold', color: '#CCCCCC', align: 'center' }, { type: 'text', text: '返回課程管理', size: 'md', weight: 'bold', color: '#AAAAAA', align: 'center', margin: 'md' }], justifyContent: 'center', alignItems: 'center', height: '150px' }], }, action: { type: 'postback', label: '返回課程管理', data: `action=run_command&text=${COMMANDS.TEACHER.COURSE_LIST}` } };
            groupBubbles.push(backButton);
            
            return reply(replyToken, [{ type: 'text', text: `系列代碼 ${prefix} 的單堂課程：` }, { type: 'flex', altText: `系列代碼 ${prefix} 的課程列表`, contents: { type: 'carousel', contents: groupBubbles.slice(0, 10) } }]);

        case 'cancel_course_confirm':
            const courseToCancel = await getCourse(courseId);
            if (!courseToCancel) return reply(replyToken, '課程不存在或已被取消。');
            const confirmCancelMsg = { type: 'flex', altText: '確認取消課程', contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [{ type: 'text', text: `您確定要取消「${courseToCancel.title}」這堂課嗎？`, wrap: true, size: 'md' }, { type: 'text', text: `本堂課共有 ${courseToCancel.students.length} 位預約學員，取消後將會退還點數並通知學員。`, size: 'sm', wrap: true }, { type: 'button', style: 'primary', color: '#de5246', action: { type: 'postback', label: '✅ 確認取消', data: `action=cancel_course&courseId=${courseId}` } }, { type: 'button', style: 'secondary', action: { type: 'postback', label: '❌ 放棄', data: `action=manage_course_group&prefix=${courseId.substring(0,2)}` } }] } } };
            return reply(replyToken, confirmCancelMsg);

        case 'cancel_course':
            const transactionClientSingleCancel = await pgPool.connect();
            try {
                await transactionClientSingleCancel.query('BEGIN');
                const courseToDel = (await transactionClientSingleCancel.query('SELECT * FROM courses WHERE id = $1 FOR UPDATE', [courseId])).rows[0];
                if (!courseToDel) throw new Error('課程已不存在。');
                
                const userIdsToNotify = [...courseToDel.students, ...courseToDel.waiting];
                if (userIdsToNotify.length > 0) {
                    const usersRes = await transactionClientSingleCancel.query(`SELECT * FROM users WHERE id = ANY($1) FOR UPDATE`, [userIdsToNotify]);
                    for (const user of usersRes.rows) {
                        if (courseToDel.students.includes(user.id)) {
                            user.points += courseToDel.points_cost;
                            if(!user.history) user.history = [];
                            user.history.push({ id: courseToDel.id, action: `課程取消退點：${courseToDel.title} (退 ${courseToDel.points_cost} 點)`, time: new Date().toISOString() });
                            await saveUser(user, transactionClientSingleCancel);
                            push(user.id, `‼️ 老師已取消課程「${courseToDel.title}」。已退還 ${courseToDel.points_cost} 點。`).catch(e => console.error(e));
                        } else if (courseToDel.waiting.includes(user.id)) {
                            push(user.id, `‼️ 老師已取消您候補中的課程「${courseToDel.title}」。`).catch(e => console.error(e));
                        }
                    }
                }
                
                await deleteCourse(courseId, transactionClientSingleCancel);
                await transactionClientSingleCancel.query('COMMIT');
                return reply(replyToken, `課程「${courseToDel.title}」已成功取消，並退還已預約學員的點數。`);
            } catch(err) {
                await transactionClientSingleCancel.query('ROLLBACK');
                console.error(`❌ 取消課程 ${courseId} 失敗:`, err.message);
                return reply(replyToken, `取消失敗：${err.message}`);
            } finally {
                transactionClientSingleCancel.release();
            }
        ...
        case 'manual_adjust_points':
        case 'start_manual_adjust':
            await setUserState(userId, 'awaiting_manual_adjust', { step: 'awaiting_student_info' });
            return reply(replyToken, '請輸入欲調整點數的學員姓名或ID：');
        
        case 'select_manual_adjust_student':
            const student = await getUser(studentId);
            if (!student) return reply(replyToken, '找不到此學員。');
            
            const newStateData = { step: 'awaiting_operation', targetUserId: student.id, targetUserName: student.name, currentPoints: student.points };
            await setUserState(userId, 'awaiting_manual_adjust', newStateData);
            
            const operationFlex = { type: 'flex', altText: '選擇操作', contents: { type: 'bubble',
                body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [
                        { type: 'text', text: `您選擇了學員：${student.name || ' '}`, weight: 'bold', wrap: true },
                        { type: 'text', text: `目前點數：${student.points} 點`, size: 'sm' },
                        { type: 'button', style: 'primary', color: '#52b69a', action: { type: 'message', label: COMMANDS.TEACHER.ADD_POINTS, text: COMMANDS.TEACHER.ADD_POINTS } },
                        { type: 'button', style: 'primary', color: '#de5246', action: { type: 'message', label: COMMANDS.TEACHER.DEDUCT_POINTS, text: COMMANDS.TEACHER.DEDUCT_POINTS } },
                        { type: 'button', style: 'secondary', action: { type: 'message', label: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST, text: COMMANDS.TEACHER.CANCEL_MANUAL_ADJUST } }
                    ]}
        ...
        case 'show_student_detail':
            const detailedUser = await getUser(studentId);
            if (!detailedUser) return reply(replyToken, '找不到此學員。');
            const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';

            const historyRecords = (detailedUser.history?.length > 0) 
                ? detailedUser.history.slice(-5).reverse().map(record => ({ type: 'text', text: `・${record.action || '未知操作'} (${formatDateTime(record.time)})`, size: 'sm', color: '#666666', wrap: true }))
                : [{ type: 'text', text: '尚無歷史記錄', size: 'sm', color: '#999999' }];
            
            const detailFlex = {
                type: 'flex', altText: `學員 ${detailedUser.name || ' '} 的資訊`,
                contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [
                        { type: 'box', layout: 'horizontal', spacing: 'md', alignItems: 'center', contents: [{ type: 'image', url: detailedUser.picture_url || placeholder_avatar, aspectRatio: '1:1', size: 'md', flex: 0, cornerRadius: '100px' }, { type: 'text', text: detailedUser.name || ' ', weight: 'bold', size: 'xl', wrap: true }] },
                        { type: 'box', layout: 'vertical', spacing: 'sm', margin: 'lg', contents: [
                            { type: 'box', layout: 'baseline', spacing: 'sm', contents: [{ type: 'text', text: '剩餘點數', color: '#aaaaaa', size: 'sm', flex: 3 }, { type: 'text', text: `${detailedUser.points} 點`, wrap: true, color: '#666666', size: 'sm', flex: 5, weight: 'bold' }] },
                            { type: 'box', layout: 'baseline', spacing: 'sm', contents: [{ type: 'text', text: '學員 ID', color: '#aaaaaa', size: 'sm', flex: 3 }, { type: 'text', text: detailedUser.id, wrap: true, color: '#666666', size: 'xxs', flex: 5 }] }
                        ]},
                        { type: 'separator', margin: 'xxl' },
                        { type: 'text', text: '近期記錄 (最多5筆)', weight: 'bold', size: 'md', margin: 'lg' }, ...historyRecords
                    ]
                }
            };
        ...
        default:
            return reply(replyToken, '我不懂你的意思。');
    }
}

async function handleStudentPostbacks(event, action, data) {
    const { replyToken, source: { userId } } = event;
    const { courseId, type, plan, orderId } = Object.fromEntries(data);
    const now = Date.now();
    const user = await getUser(userId);

    switch (action) {
        case 'select_purchase_plan':
            const selectedPlan = PURCHASE_PLANS.find(p => p.points.toString() === plan);
            if (!selectedPlan) return reply(replyToken, '無效的點數方案。');
            
            const confirmPurchaseFlex = { type: 'flex', altText: '確認購買點數', contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [
                { type: 'text', text: `您選擇購買 ${selectedPlan.points} 點方案。`, weight: 'bold' },
                { type: 'text', text: `金額：$${selectedPlan.amount}`, size: 'lg' },
            ...
            await setUserState(userId, 'awaiting_purchase_confirmation', { points: selectedPlan.points, amount: selectedPlan.amount, userId: userId });
            return reply(replyToken, confirmPurchaseFlex);
            
        case 'input_last5_card_trigger':
        case 'edit_last5_card_trigger':
            const pendingOrdersRes = await pgPool.query(`SELECT * FROM orders WHERE user_id = $1 AND (status = 'pending_payment' OR status = 'pending_confirmation' OR status = 'rejected') ORDER BY timestamp DESC LIMIT 1`, [userId]);
            const pendingOrder = pendingOrdersRes.rows[0];
            if (!pendingOrder) {
                await clearUserState(userId);
                return reply(replyToken, '目前沒有待處理的訂單可輸入後五碼。');
            }
            await setUserState(userId, 'awaiting_purchase_input', { orderId: pendingOrder.order_id });
            const inputLast5Flex = { type: 'flex', altText: '輸入匯款後五碼', contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [
                { type: 'text', text: `請輸入您的匯款帳號後五碼以完成訂單 #${pendingOrder.order_id}。`, wrap: true },
                { type: 'button', action: { type: 'message', label: '取消', text: COMMANDS.STUDENT.CANCEL_INPUT_LAST5 }, style: 'secondary', margin: 'md'}
            ]}}};
            return reply(replyToken, inputLast5Flex);
            
        case 'confirm_booking':
            const courseToBook = await getCourse(courseId);
            if (!courseToBook) return reply(replyToken, '課程不存在或已被取消。');
            if (new Date(courseToBook.time).getTime() < now) return reply(replyToken, '課程已過期，無法預約。');
            if (user.points < courseToBook.points_cost) return reply(replyToken, `您的點數不足，此課程需要 ${courseToBook.points_cost} 點。`);
            
            let confirmBookingMsg, actionCmd, abandonCmd;
            if (type === 'book') {
                confirmBookingMsg = `您確定要預約課程「${courseToBook.title}」嗎？\n預約成功將扣除 ${courseToBook.pointsCost} 點。`;
                actionCmd = COMMANDS.STUDENT.CONFIRM_BOOKING;
                abandonCmd = COMMANDS.STUDENT.ABANDON_BOOKING;
            } else if (type === 'wait') {
                confirmBookingMsg = `課程「${courseToBook.title}」已額滿，您確定要加入候補名單嗎？\n候補成功將預先扣除 ${courseToBook.pointsCost} 點，若最終未補上，點數將自動退還。`;
                actionCmd = COMMANDS.STUDENT.CONFIRM_BOOKING;
                abandonCmd = COMMANDS.STUDENT.ABANDON_BOOKING;
            }
            
            await setUserState(userId, 'awaiting_booking_confirmation', { courseId: courseId, actionType: type });
            const confirmBookingFlex = { type: 'flex', altText: '確認預約課程', contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [
                { type: 'text', text: confirmBookingMsg, wrap: true },
                { type: 'button', style: 'primary', action: { type: 'message', label: '✅ 確認', text: actionCmd } },
                { type: 'button', style: 'secondary', action: { type: 'message', label: '❌ 放棄', text: abandonCmd } }
            ]}}};
        ...
        case 'cancel_booking_confirm':
            const courseToCancel = await getCourse(courseId);
            if (!courseToCancel) return reply(replyToken, '課程不存在或已被取消。');
            if (new Date(courseToCancel.time).getTime() - now < EIGHT_HOURS_IN_MS) return reply(replyToken, '課程即將開始（不足8小時），無法取消。');
            
            await setUserState(userId, 'awaiting_booking_confirmation', { courseId: courseId, actionType: 'cancel_book' });
            const confirmCancelFlex = { type: 'flex', altText: '確認取消課程', contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [
                { type: 'text', text: `您確定要取消預約課程「${courseToCancel.title}」嗎？`, wrap: true },
                { type: 'text', text: `取消後將退還 ${courseToCancel.pointsCost} 點。`, size: 'sm', color: '#de5246' },
                { type: 'button', style: 'primary', color: '#de5246', action: { type: 'message', label: '✅ 確認取消', text: COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING } },
                { type: 'button', style: 'secondary', action: { type: 'message', label: '❌ 放棄', text: COMMANDS.STUDENT.ABANDON_CANCEL_BOOKING } }
            ]}}};
            return reply(replyToken, confirmCancelFlex);

        case 'cancel_waiting_confirm':
            const courseToCancelWaiting = await getCourse(courseId);
            if (!courseToCancelWaiting) return reply(replyToken, '課程不存在或已被取消。');
            
            await setUserState(userId, 'awaiting_booking_confirmation', { courseId: courseId, actionType: 'cancel_wait' });
            const confirmCancelWaitingFlex = { type: 'flex', altText: '確認取消候補', contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [
                { type: 'text', text: `您確定要取消課程「${courseToCancelWaiting.title}」的候補嗎？`, wrap: true },
                { type: 'button', style: 'primary', color: '#8d99ae', action: { type: 'message', label: '✅ 確認取消', text: COMMANDS.STUDENT.CONFIRM_CANCEL_WAITING } },
                { type: 'button', style: 'secondary', action: { type: 'message', label: '❌ 放棄', text: COMMANDS.STUDENT.ABANDON_CANCEL_WAITING } }
            ]}}};
            return reply(replyToken, confirmCancelWaitingFlex);
...
async function handleEvent(event) {
    ...
    } else if (!user.picture_url || !user.name) {
        try {
            const profile = await client.getProfile(userId);
    ...
    if (event.type === 'postback') {
        ...
        if (user.role === 'teacher') {
            return handleTeacherPostbacks(event, action, data);
        } else {
            return handleStudentPostbacks(event, action, data);
        }
    }
}
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V7.5.7 (最終完整修正版)`);
