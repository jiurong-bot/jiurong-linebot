...
/**
 * [V36.0 重構] 顯示可預約課程，改為以「課程系列」分組的卡片式介面
 * @param {string} userId - 使用者 ID
 * @param {URLSearchParams} postbackData - 從 postback 事件來的 URLSearchParams 物件
 * @returns {Promise<object|string>} - Flex Message 物件或無資料時的文字訊息
 */
async function showAvailableCourses(userId, postbackData) {
    return executeDbQuery(async (client) => {
        ...
        coursesRes.rows.forEach(course => {
            ...
            if (!courseSeries[prefix]) {
                ...
                courseSeries[prefix] = {
                    ...
                    sessions: [] // 準備存放該系列的所有堂次
                };
            }
            courseSeries[prefix].sessions.push(course);
        });

        // 3. 處理週次切換 和 系列內部 "顯示更多" 的邏輯
        const weekIdentifier = postbackData.get('week_start');
        const showMorePrefix = postbackData.get('show_more');
        const seriesPage = parseInt(postbackData.get('series_page') || '1', 10);
        
        const targetDate = weekIdentifier ? new Date(weekIdentifier) : new Date();
        const { weekStart, weekEnd } = getWeekDates(targetDate);

        // 篩選出在本週有課的系列
        const allSeries = Object.values(courseSeries);
        const seriesInWeek = allSeries.filter(series => 
            series.sessions.some(session => {
                const sessionTime = new Date(session.time);
                return sessionTime >= weekStart && sessionTime <= weekEnd;
            })
        );
        
        if (seriesInWeek.length === 0) {
            const weekFormat = new Intl.DateTimeFormat('zh-TW', { month: '2-digit', day: '2-digit' });
            const dateRangeStr = `${weekFormat.format(weekStart)} - ${weekFormat.format(weekEnd)}`;
            return `抱歉，${dateRangeStr} 這一週沒有可預約的課程。`;
        }

        // 4. 產生 Flex Message 卡片
        const placeholder_avatar = 'https://i.imgur.com/s43t5tQ.jpeg';
        const seriesBubbles = seriesInWeek.map(series => {
            // 處理系列內部分頁
            let currentPage = 1;
            if (series.prefix === showMorePrefix) {
            ...
            if (hasMoreSessions) {
                ...
                footerContents.push({
                    ...
                    action: {
                        type: 'postback',
                        label: '顯示更多日期 ➡️',
                        data: `action=view_available_courses&week_start=${weekIdentifier || ''}&show_more=${series.prefix}&series_page=${nextSeriesPage}`
                    }
                });
            }
            ...
            return {
                type: 'bubble',
                size: 'giga',
                hero: { type: 'image', url: series.teacherImageUrl || placeholder_avatar, size: 'full', aspectRatio: '1:1', aspectMode: 'cover' },
                body: { type: 'box', layout: 'vertical', paddingAll: 'xl', spacing: 'md', contents: bodyContents },
                ...(footerContents.length > 0 && { footer: { type: 'box', layout: 'vertical', spacing: 'sm', contents: footerContents } })
            };
        });
        ...
        // 只有當上一週不是過去式時，才顯示 "上一週" 按鈕
        const showPrevWeekButton = getWeekDates(prevWeek).weekEnd > new Date();

        const weekNavButtons = [];
        if (showPrevWeekButton) {
            weekNavButtons.push({
                type: 'button', style: 'link', height: 'sm',
                action: { type: 'postback', label: '⬅️ 上一週', data: `action=view_available_courses&week_start=${prevWeek.toISOString().split('T')[0]}` }
            });
        }
        weekNavButtons.push({
            type: 'button', style: 'link', height: 'sm',
            action: { type: 'postback', label: '下一週 ➡️', data: `action=view_available_courses&week_start=${nextWeek.toISOString().split('T')[0]}` }
        });
        ...
        // 6. 組合最終訊息
        const weekFormat = new Intl.DateTimeFormat('zh-TW', { timeZone: 'Asia/Taipei', month: 'short', day: 'numeric' });
        const headerText = `🗓️ ${weekFormat.format(weekStart)} - ${weekFormat.format(weekEnd)} 可預約課程`;
        const flexMessage = { type: 'flex', altText: headerText, contents: { type: 'carousel', contents: seriesBubbles } };
        
        // 只有在第一次進入時 (沒有 weekIdentifier) 才顯示引導文字
        if (!weekIdentifier) {
