...
async function handleEvent(event) {
  ...
  try {
    ...
    return replyText(event.replyToken, '⚠️ 系統錯誤，請稍後再試');
  }
  ...
  if (!db[userId]) {
    try {
      ...
      return replyText(event.replyToken, '⚠️ 無法取得資料，請稍後再試');
    }
  }
  ...
  if (pendingTeacherLogin[userId]) {
    if (msg === TEACHER_PASSWORD) {
      ...
      writeJSON(DATA_FILE, db);
      return replyWithMenu(event.replyToken, '✅ 老師登入成功', teacherMenu);
    } else {
      delete pendingTeacherLogin[userId];
  ...
  if (msg === '@我是老師') {
    ...
    return replyWithMenu(event.replyToken, '請輸入老師密碼（四位數）', studentMenu);
  }
  if (msg === '@我是學員') {
    user.role = 'student';
    writeJSON(DATA_FILE, db);
    return replyWithMenu(event.replyToken, '✅ 已切換為學員身份', studentMenu);
  }
  if (msg === '@切換身份') {
    return replyWithMenu(event.replyToken, '請選擇您的身份：', [
      {
        type: 'action',
  ...
  if (user.role === 'student') {
    ...
    return replyWithMenu(event.replyToken, '請先設定您的身份。', [
      {
        type: 'action',
        action: { type: 'message', label: '我是學員', text: '@我是學員' }
      },
...
function handleStudentCommands(event, userId, msg, user, db, courses) {
  ...
  if (msg === '@課程查詢') {
    const list = Object.entries(courses)
      .map(([id, c]) =>
        `${id}: ${c.name} (${c.date})\n尚餘名額：${c.max - c.students.length}`)
      .join('\n\n') || '目前尚無可查詢課程。';
    return replyWithMenu(replyToken, `📘 課程一覽：\n${list}`, studentMenu);
  }
  ...
  if (/^預約 course_/.test(msg)) {
    ...
    if (course.students.includes(userId)) return replyWithMenu(replyToken, '⚠️ 您已預約此課程', studentMenu);
    if (user.points <= 0) return replyWithMenu(replyToken, '⚠️ 點數不足，無法預約', studentMenu);
    ...
    if (course.students.length < course.max) {
      ...
      return replyWithMenu(replyToken, `✅ 預約成功，已扣除 1 點。\n課程：${course.name}\n時間：${course.date}`, studentMenu);
    } else {
      if (!course.waitlist.includes(userId)) {
        course.waitlist.push(userId);
        writeJSON(COURSE_FILE, courses);
      }
      return replyWithMenu(replyToken, '⚠️ 課程已額滿，您已加入候補名單', studentMenu);
    }
  }
  ...
  if (msg === '@我的課程') {
    const my = user.history.map(h => {
      ...
      return c
        ? `${c.name}（${c.date}）\n預約時間：${new Date(h.time).toLocaleString()}`
        : `❌ 已刪除課程 ${h.courseId}`;
    }).join('\n\n') || '尚無預約紀錄';
    return replyWithMenu(replyToken, `📖 我的課程：\n${my}`, studentMenu);
  }
  ...
  if (msg.startsWith('@取消課程')) {
    ...
    if (idx >= 0) {
      ...
      if (course.waitlist.length > 0) {
        ...
        if (db[promotedUserId]) {
          ...
          client.pushMessage(promotedUserId, {
            type: 'text',
            text: `✅ 候補轉正通知：${course.name}（${course.date}）`,
          }).catch(console.error);
        }
      }
      ...
      return replyWithMenu(replyToken, '⚠️ 您未預約此課程', studentMenu);
    }
  }
  ...
  return replyWithMenu(replyToken, '📌 請使用下方選單或輸入正確指令。', studentMenu);
}
...
function handleTeacherCommands(event, userId, msg, user, db, courses) {
  ...
  if (msg.startsWith('@新增課程')) {
    ...
    writeJSON(COURSE_FILE, courses);
    return replyWithMenu(replyToken, `✅ 已新增課程：${name}（${date}），名額 ${max} 人`, teacherMenu);
  }
  ...
  if (msg.startsWith('@取消課程')) {
    ...
    writeJSON(COURSE_FILE, courses);
    writeJSON(DATA_FILE, db);
    return replyWithMenu(replyToken, `✅ 已取消課程 ${courseId} 並退回所有學生點數`, teacherMenu);
  }
  ...
  if (msg === '@查學員') {
    const list = Object.entries(db)
      ...
      .map(([id, u]) =>
        `🙋 ${u.name || id.slice(-4)}｜點數：${u.points}｜預約 ${u.history.length} 堂`)
      .join('\n') || '⚠️ 沒有學員資料';
    return replyWithMenu(replyToken, list, teacherMenu);
  }
  ...
  if (/^@加點 /.test(msg)) {
    ...
    writeJSON(DATA_FILE, db);
    return replyWithMenu(replyToken, `✅ 已為 ${db[targetId].name} 加點 ${amount} 點`, teacherMenu);
  }
  ...
  if (/^@扣點 /.test(msg)) {
    ...
    db[targetId].points -= parseInt(amount);
    writeJSON(DATA_FILE, db);
    return replyWithMenu(replyToken, `✅ 已為 ${db[targetId].name} 扣點 ${amount} 點`, teacherMenu);
  }
  ...
  if (msg === '@統計報表') {
    const summary = Object.entries(db)
      .filter(([_, u]) => u.role === 'student')
      .map(([id, u]) =>
        `📊 ${u.name || id.slice(-4)}：${u.history.length} 堂`)
      .join('\n') || '尚無任何學員預約紀錄';
    return replyWithMenu(replyToken, `📈 本月課程統計：\n${summary}`, teacherMenu);
  }
    ...
    upcoming.forEach(([id, c]) => {
      c.students.forEach(uid => {
        client.pushMessage(uid, {
          type: 'text',
          text: `⏰ 課程提醒：「${c.name}」將於 ${c.date} 開始，請準時出席！`
...
}, 10 * 60 * 1000); // 每 10 分鐘執行

// 🌐 keep-alive 機制（每 5 分鐘 ping 自己一次）
setInterval(() => {
