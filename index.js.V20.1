    ...
    if (stateName === 'booking_confirmation') {
        ...
        switch (stateData.type) {
            case 'book':
                ...
                break;
            case 'cancel_book':
                // ... cancel_book and cancel_wait logic
                break;
        }
        return;
    }
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (action === 'run_command') {
            ...
            if (commandText) {
                ...
                // V20.1 修正: 重新獲取最新的 user 物件來處理 postback
...
app.post('/webhook', (req, res) => {
  ...
  req.body.events.forEach(event => {
      ...
      handleEvent(event).catch(err => {
          ...
          // V20.1 修正: 即使 ADMIN_USER_ID 未設定，也要記錄原始錯誤
          const errorMessage = `一個事件處理失敗:\n${err.message}`;
          console.error(errorMessage); // 確保錯誤一定會被記錄
          if (ADMIN_USER_ID) {
              push(ADMIN_USER_ID, errorMessage).catch(e => {
                  console.error('❌ 連錯誤通知都發送失敗:', e.message);
              });
          }
      });
  });
...
app.get('/', (req, res) => res.send('九容瑜伽 LINE Bot (V20.1 錯誤處理強化版) 正常運作中。'));

// --- 伺服器啟動 ---
app.listen(PORT, async () => {
  await initializeDatabase();
  console.log(`✅ 伺服器已啟動，監聽埠號 ${PORT}`);
  console.log(`Bot 版本: V20.1 (錯誤處理強化版)`);
