/*
 * index.js - V19.2 (角色追蹤偵錯版)
 * =================================================================
 * 功能簡介 (Functional Overview):
 * - 此檔案為 Line Bot 的主服務程式。
 * - 負責接收與驗證來自 LINE 平台的 Webhook 請求。
 * - 根據使用者角色（管理員、老師、學員）將事件路由至對應的指令處理函式。
 * - 啟動 Express 伺服器並設定背景排程任務。
 *
 * 相依模組版本 (Module Dependencies):
 * - utils.js: V19.0
 * - jobs.js:  V16.0 (或更新)
 *
 * 主要功能選單 (Feature Menus):
 * * [老師介面]
 * - 課程管理: 新增/查看/取消課程
 * - 點數管理: 審核訂單/手動調點
 * - 學員管理: 查詢學員/查看與回覆留言
...
async function handleAdminCommands(event, userId, userState) {
  ...
  switch(text) {
    ...
    case COMMANDS.ADMIN.SIMULATE_STUDENT:
      console.log(`[偵錯] 進入 @模擬學員身份 處理區塊。`);
      console.log(`[偵錯] 當前角色為 '${user.role}'，準備更新為 'student'。`);
      user.role = 'student';
      await saveUser(user);
      console.log(`[偵錯] saveUser 完成，已將角色更新為 'student'。`);
      if(STUDENT_RICH_MENU_ID) await client.linkRichMenuToUser(userId, STUDENT_RICH_MENU_ID);
      return push(userId, '您已切換為「學員」模擬身份。\n若要返回，請手動輸入「@管理模式」。');

    case COMMANDS.ADMIN.SIMULATE_TEACHER:
      console.log(`[偵錯] 進入 @模擬老師身份 處理區塊。`);
      console.log(`[偵錯] 當前角色為 '${user.role}'，準備更新為 'teacher'。`);
      user.role = 'teacher';
      await saveUser(user);
      console.log(`[偵錯] saveUser 完成，已將角色更新為 'teacher'。`);
...
async function handleEvent(event) {
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (userId === ADMIN_USER_ID && text === COMMANDS.ADMIN.PANEL) {
            console.log(`[偵錯] 進入 @管理模式 特殊處理區塊。`);
            if (user.role !== 'admin') {
                console.log(`[偵錯] 當前角色為 '${user.role}'，準備更新為 'admin'。`);
                user.role = 'admin';
                await saveUser(user);
                console.log(`[偵錯] saveUser 完成，已將角色更新為 'admin'。`);
...
app.get('/', (req, res) => res.send('九容瑜伽 LINE Bot (V19.2 角色追蹤偵錯版) 正常運作中。'));

// --- 伺服器啟動 ---
app.listen(PORT, async () => {
  await initializeDatabase();
  console.log(`✅ 伺服器已啟動，監聽埠號 ${PORT}`);
  console.log(`Bot 版本: V19.2 (角色追蹤偵錯版)`);
