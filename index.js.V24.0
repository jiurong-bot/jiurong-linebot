 ...
 * - 核心路由重構：採納使用者建議，handleEvent 預設完全信任資料庫中的角色，僅對「@管理模式」指令進行特殊判斷，根除競態條件。
 * - 新增 welcomed 旗標：在使用者資料中新增 'welcomed' 屬性，確保歡迎詞只對真正的全新使用者發送一次。
 * - 此版本為經過多次除錯後的最終穩定版本。
 *
 * 相依模組版本 (Module Dependencies):
 * - utils.js: V19.0 (應可兼容，因 saveUser 通常能處理彈性 JSON 欄位)
 * - jobs.js:  V16.0 (或更新)
 * =================================================================
 */
...
async function handleAdminCommands(event, user, userState) {
  ...
  // 在 V24.0，此函式只處理純粹的管理員指令，不再需要負責委派
  // @管理模式 的指令已由 handleEvent 攔截處理
...
app.get('/', (req, res) => res.send('九容瑜伽 LINE Bot (V24.0 穩定版) 正常運作中。'));
