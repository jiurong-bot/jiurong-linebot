...
async function initializeDatabase() {
  try {
    ...
    // 修正：將 name 相關欄位加回 NOT NULL 確保資料完整性
...
async function saveUser(user, dbClient = pgPool) {
  try {
    ...
    await dbClient.query(
        ...
        [user.id, user.name || '未命名用戶', user.points, user.role, historyJson, user.last_seen_announcement_id || 0, user.pictureUrl || null]
    );
  } catch (err) {
