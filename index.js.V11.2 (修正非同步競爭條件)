...
async function handleTeacherCommands(event, userId) {
  ...
  if (pendingAnnouncementCreation[userId]) {
    ...
    const confirmationButtons = [ { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT, text: COMMANDS.TEACHER.CONFIRM_ADD_ANNOUNCEMENT } }, { type: 'action', action: { type: 'message', label: COMMANDS.TEACHER.CANCEL_ADD_ANNOUNCEMENT, text: COMMANDS.TEACHER.CANCEL_ADD_ANNOUNCEMENT } } ];

    switch (state.step) {
        case 'await_content':
            state.content = text; state.step = 'await_confirmation';
            const confirmMsg = { type: 'flex', altText: '請確認公告內容', contents: { type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: '請確認公告內容', weight: 'bold', color: '#FFFFFF' }], backgroundColor: '#1A759F' }, body: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: text, wrap: true }] }, } };
                ...
                try {
                    ...
                    (async () => {
                        try {
                            ...
                            const announcementMessage = { type: 'flex', altText: '來自老師的最新公告', contents: { type: 'bubble', header: { type: 'box', layout: 'vertical', backgroundColor: '#de5246', contents: [ { type: 'text', text: '‼️ 最新公告', color: '#ffffff', weight: 'bold', size: 'lg' } ]}, body: { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: state.content, wrap: true } ]}, footer: { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: `發布時間: ${formatDateTime(newAnn.created_at)}`, size: 'xs', color: '#aaaaaa', align: 'center' } ]} } };
                            for (const student of studentsRes.rows) { await push(student.id, announcementMessage); }
                            console.log(`📢 公告已成功推播給 ${studentsRes.rows.length} 位學員。`);
                        } catch (e) { console.error('❌ 推播公告失敗:', e); }
                    })();
                } catch (e) {
  ...
  if (pendingReply[userId]) {
    ...
    delete pendingReply[userId];
    await reply(replyToken, '已成功回覆學員。');
  ...
  if (pendingStudentSearchQuery[userId]) {
      try {
          ...
          if (foundUsers.length === 0) { return reply(replyToken, `找不到學員「${query}」。`); }
          const updatedUsersWithFreshProfiles = await Promise.all(
              foundUsers.map(async (dbUser) => {
                  try {
                      ...
                      dbUser.picture_url = profile.pictureUrl; dbUser.name = profile.displayName;
                      pgPool.query('UPDATE users SET name = $1, picture_url = $2 WHERE id = $3', [profile.displayName, profile.pictureUrl, dbUser.id]).catch(e => console.error(`背景更新用戶 ${dbUser.id} 資料失敗:`, e.message));
                      return dbUser;
                  } catch (e) { console.error(`查詢用戶 ${dbUser.id} 最新資料失敗:`, e.message); return dbUser; }
              })
          );
          const placeholder_avatar = 'https://i.imgur.com/8l1Yd2S.png';
          if (updatedUsersWithFreshProfiles.length === 1) {
              const foundUser = updatedUsersWithFreshProfiles[0];
              const historyRecords = (foundUser.history?.length > 0) ? foundUser.history.slice(-5).reverse().map(record => ({ type: 'text', text: `・${record.action} (${formatDateTime(record.time)})`, size: 'sm', color: '#666666', wrap: true })) : [{ type: 'text', text: '尚無歷史記錄', size: 'sm', color: '#999999' }];
  ...
  if (pendingManualAdjust[userId]) { /* Logic for manual point adjustment */ }
  
  if (pendingProductCreation[userId]) {
      const state = pendingProductCreation[userId];
      switch (state.step) {
          case 'await_name': state.name = text; state.step = 'await_description'; return reply(replyToken, '請輸入商品描述：');
          case 'await_description': state.description = text; state.step = 'await_price'; return reply(replyToken, '請輸入商品價格 (純數字)：');
          case 'await_price':
              const price = parseInt(text, 10);
              if (isNaN(price) || price <= 0) { return reply(replyToken, '價格格式不正確，請輸入一個正整數。'); }
              state.price = price; state.step = 'await_image';
              return reply(replyToken, '請傳送一張商品圖片 (或輸入「無」表示不上傳)：');
          case 'await_image':
              let imageUrl = null;
              if (event.message.type === 'image') { imageUrl = 'https://via.placeholder.com/512'; } 
  ...
  if (text === COMMANDS.TEACHER.STUDENT_MANAGEMENT || text === COMMANDS.TEACHER.POINT_MANAGEMENT) {
      const studentManagementMenu = [ { type: 'action', action: { type: 'message', label: '查詢特定學員', text: COMMANDS.TEACHER.SEARCH_STUDENT } }, { type: 'action', action: { type: 'message', label: '手動調整點數', text: COMMANDS.TEACHER.MANUAL_ADJUST_POINTS } }, { type: 'action', action: { type: 'message', label: '查看學員留言', text: COMMANDS.TEACHER.VIEW_MESSAGES } }, { type: 'action', action: { type: 'message', label: '查詢留言內容', text: COMMANDS.TEACHER.MESSAGE_SEARCH } } ];
      return reply(replyToken, '請選擇學員管理功能：', studentManagementMenu);
  }

  if (text === COMMANDS.TEACHER.SEARCH_STUDENT) {
      pendingStudentSearchQuery[userId] = {};
      setupConversationTimeout(userId, pendingStudentSearchQuery, 'pendingStudentSearchQuery', (u) => push(u, '查詢學員逾時，自動取消。').catch(e => console.error(e)));
  ...
  if (text === COMMANDS.TEACHER.VIEW_MESSAGES) {
      try {
          ...
          const messageBubbles = messagesRes.rows.map(msg => { const headerColor = msg.status === 'read' ? '#52b69a' : '#6a7d8b'; return { type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: `來自 ${msg.user_name}`, color: '#ffffff', weight: 'bold' }], backgroundColor: headerColor, paddingAll: 'lg' }, body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [{ type: 'text', text: msg.message, wrap: true }, { type: 'text', text: `時間: ${formatDateTime(msg.timestamp)}`, size: 'xs', color: '#aaaaaa', margin: 'md' }] }, footer: { type: 'box', layout: 'vertical', spacing: 'sm', contents: [{ type: 'button', style: 'primary', color: '#52b69a', height: 'sm', action: { type: 'postback', label: '✅ 標為已讀', data: `action=mark_feedback_read&msgId=${msg.id}` } }, { type: 'button', style: 'primary', color: '#1a759f', height: 'sm', action: { type: 'postback', label: '▶️ 回覆', data: `action=reply_feedback&msgId=${msg.id}&userId=${msg.user_id}` } }] } }; });
  ...
  if (text === COMMANDS.TEACHER.MESSAGE_SEARCH) {
      pendingMessageSearchQuery[userId] = {};
      setupConversationTimeout(userId, pendingMessageSearchQuery, 'pendingMessageSearchQuery', (u) => push(u, '查詢留言逾時，自動取消。').catch(e => console.error(e)));
      return reply(replyToken, '請輸入您想查詢的留言關鍵字 (可查姓名、留言內容、老師回覆)：');
  }
  ...
  if (text === COMMANDS.TEACHER.DELETE_ANNOUNCEMENT) {
      try {
          ...
          if (announcementsRes.rows.length === 0) { return reply(replyToken, '目前沒有任何可刪除的公告。'); }
          const announcementBubbles = announcementsRes.rows.map(ann => ({ type: 'bubble', body: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: ann.content, wrap: true, size: 'sm' }, { type: 'text', text: `發布於: ${formatDateTime(ann.created_at)}`, size: 'xs', color: '#aaaaaa', margin: 'md' }] }, footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', style: 'primary', color: '#DE5246', action: { type: 'postback', label: '刪除此公告', data: `action=confirm_delete_announcement&annId=${ann.id}` } }] } }));
  ...
  if (text === COMMANDS.TEACHER.VIEW_PRODUCTS) {
      try {
          ...
          const productBubbles = productsRes.rows.map(p => ({ type: 'bubble', hero: p.image_url ? { type: 'image', url: p.image_url, size: 'full', aspectRatio: '20:13', aspectMode: 'cover' } : undefined, body: { type: 'box', layout: 'vertical', contents: [ { type: 'text', text: p.name, weight: 'bold', size: 'xl' }, { type: 'text', text: `$${p.price}`, size: 'lg', margin: 'md' }, { type: 'text', text: p.description, wrap: true, size: 'sm', margin: 'md' }, ]}, footer: { type: 'box', layout: 'vertical', contents: [ { type: 'button', style: 'primary', color: '#DE5246', action: { type: 'postback', label: '下架此商品', data: `action=remove_product&productId=${p.id}` } } ]} }));
  ...
  if (text === COMMANDS.TEACHER.REPORT) {
      try {
          ...
          return reply(replyToken, '生成營運報告時發生錯誤，請稍後再試。');
      }
  }
...
async function handlePurchaseFlow(event, userId) {
  ...
  if (text === COMMANDS.STUDENT.CANCEL_INPUT_LAST5 || text === COMMANDS.STUDENT.RETURN_POINTS_MENU) { 
      ...
      return reply(replyToken, [{type: 'text', text: '已返回點數管理主選單。'}, flexMenu]);
  }
  ...
  switch (stepData.step) {
    case 'input_last5':
      ...
      if (!/^\d{5}$/.test(last5Digits)) { return reply(replyToken, '您輸入的匯款帳號後五碼格式不正確，請輸入五位數字。'); }
      ...
      try {
        ...
        if (!orderInTransaction || (orderInTransaction.status !== 'pending_payment' && orderInTransaction.status !== 'pending_confirmation' && orderInTransaction.status !== 'rejected')) {
          ...
          delete pendingPurchase[userId];
          return reply(replyToken, '此訂單狀態不正確或已處理，請重新開始購點流程。', [returnToPointsMenuBtn]);
        }
        ...
        delete pendingPurchase[userId];
        return reply(replyToken, '提交後五碼時發生錯誤，請稍後再試。', [returnToPointsMenuBtn]);
      } finally {
    ...
    case 'confirm_purchase':
      if (text === COMMANDS.STUDENT.CONFIRM_BUY_POINTS) {
        ...
        try {
          ...
          delete pendingPurchase[userId];
          return reply(replyToken, '確認購買時發生錯誤，請稍後再試。', [returnToPointsMenuBtn]);
        } finally {
...
async function handleStudentCommands(event, userId) {
  ...
  if (pendingBookingConfirmation[userId]) { /* Booking confirmation logic */ }
  ...
  if (text === COMMANDS.STUDENT.BOOK_COURSE) {
      try {
          const allCourses = Object.values(await getAllCourses());
          const now = Date.now();
          const availableCourses = allCourses.filter(c => new Date(c.time).getTime() > now && c.students.length < c.capacity && !c.students.includes(userId) && !c.waiting.includes(userId));
          ...
          const courseBubbles = availableCourses.slice(0, 10).map(c => {
              const remainingSpots = c.capacity - c.students.length;
              return { type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: '可預約', color: '#ffffff', weight: 'bold' }], backgroundColor: '#52b69a', paddingAll: 'lg' }, body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [{ type: 'text', text: c.title, weight: 'bold', size: 'lg', wrap: true }, { type: 'text', text: formatDateTime(c.time), size: 'md' }, { type: 'text', text: `剩餘名額：${remainingSpots} 位`, color: '#666666', size: 'sm' }, { type: 'text', text: `費用：${c.pointsCost} 點`, color: '#1A759F', weight: 'bold', size: 'sm' }] }, footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', style: 'primary', height: 'sm', action: { type: 'postback', label: '預約此課程', data: `action=confirm_booking_start&courseId=${c.id}` } }] } };
          });
          return reply(replyToken, { type: 'flex', altText: '可預約的課程列表', contents: { type: 'carousel', contents: courseBubbles } });
      } catch(err) {
  ...
  if (text === COMMANDS.STUDENT.MY_COURSES) {
      try {
          ...
          const courseBubbles = myCourses.slice(0, 10).map(c => {
              ...
              const statusText = isBooked ? '✅ 已預約' : '🕒 候補中';
              const headerColor = isBooked ? '#1a759f' : '#ff9e00';
              const waitingPosition = isBooked ? '' : `您的候補順位為第 ${c.waiting.indexOf(userId) + 1} 位`;
              ...
              return { type: 'bubble', header: { type: 'box', layout: 'vertical', contents: [{ type: 'text', text: statusText, color: '#ffffff', weight: 'bold' }], backgroundColor: headerColor, paddingAll: 'lg' }, body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [{ type: 'text', text: c.title, weight: 'bold', size: 'lg', wrap: true }, { type: 'text', text: formatDateTime(c.time), size: 'md' }, { type: 'text', text: waitingPosition, size: 'sm', color: '#666666' }] }, footer: { type: 'box', layout: 'vertical', contents: [{ type: 'button', style: 'primary', color: '#DE5246', height: 'sm', action: { type: 'postback', label: actionLabel, data: `action=${postbackAction}&courseId=${c.id}` } }] } };
          });
  ...
  if (text === COMMANDS.STUDENT.INPUT_LAST5_CARD_TRIGGER || text === COMMANDS.STUDENT.EDIT_LAST5_CARD_TRIGGER) {
    return reply(replyToken, '請點擊「點數管理」選單中對應的按鈕來輸入或修改後五碼。');
  }
    ...
    if (event.type === 'message' && (event.message.type === 'text' || event.message.type === 'image')) {
        ...
        if (user.role === 'admin') {
            if (action === 'select_teacher_for_removal') {
                ...
                return reply(replyToken, `您確定要移除老師「${targetName}」的權限嗎？該用戶將變回學員身份。`, [
