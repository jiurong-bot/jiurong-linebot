...
async function setupRichMenus() {
  try {
    // 1. 建立學員 Rich Menu
    const studentRichMenuObject = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'student_rich_menu.json'), 'utf8'));
    ...
    // 上傳學員 Rich Menu 圖片
    const studentImageStream = fs.createReadStream(path.resolve(__dirname, 'student_rich_menu.png'));
    await client.setRichMenuImage(studentRichMenuId, studentImageStream);
    console.log('✅ 學員 Rich Menu 圖片上傳成功。');

    // 2. 建立老師 Rich Menu
    const teacherRichMenuObject = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'teacher_rich_menu.json'), 'utf8'));
    ...
    // 上傳老師 Rich Menu 圖片
    const teacherImageStream = fs.createReadStream(path.resolve(__dirname, 'teacher_rich_menu.png'));
