// index.js - V6.4 (重構指令與流程處理，強化錯誤日誌與交易)
...
async function handleTeacherCommands(event, userId) {
  ...
  const { pendingReply, pendingStudentSearchQuery, pendingMessageSearchQuery, pendingManualAdjust, pendingCourseCreation } = CONVERSATION_STATES;
  const now = Date.now();
...
async function handleTeacherCommandsPostback(event, userId) {
    ...
    const action = data.get('action');
    const replyToken = event.replyToken;
    const { pendingManualAdjust, pendingReply } = CONVERSATION_STATES;
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V6.4 (重構指令與流程處理，強化錯誤日誌與交易)`);
