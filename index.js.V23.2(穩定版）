    ...
    try {
        ...
        if (res.rows.length > 0) {
            ...
            res.rows.forEach(order => {
                push(order.user_id, `訂單取消通知：\n您的訂單 (ID: ...${order.order_id.slice(-6)}) 因超過24小時未完成付款，系統已自動為您取消。\n如有需要請重新購買，謝謝。`).catch(e => {
                    console.error(`發送逾時訂單取消通知給 ${order.user_name} (${order.user_id}) 失敗。`);
                });
            });
        }
    } catch (err) {
            ...
            if (/^\d{5}$/.test(text)) {
                ...
                try {
                    ...
                    if (orderRes.rows.length > 0) {
                        ...
                        if (TEACHER_ID) {
                            push(TEACHER_ID, `🔔 購點審核通知\n學員 ${user.name} 已提交匯款資訊。\n訂單ID: ${order_id}\n後五碼: ${text}\n請至「點數管理」->「待確認點數訂單」審核。`).catch(e => console.error(e));
                        }
                    } else {
                        delete pendingPurchase[userId];
...
async function showCourseSeries(replyToken, page = 1) {
    ...
    try {
        ...
        const courseGroupPromises = pageSeries.map(async (row) => {
            ...
            return {
                prefix: prefix,
                mainTitle: getCourseMainTitle(details.title), // [V23.2 修改]
          ...
          try {
            ...
            (async () => {
              ...
              try {
                ...
                const courseMainTitle = getCourseMainTitle(coursesToCancel[0].title); // [V23.2 修改]
         ...
         if (text === COMMANDS.TEACHER.CONFIRM_SINGLE_CANCEL) {
            ...
            try {
              ...
              for (const studentId of course.students) {
                 ...
                 push(studentId, `課程取消通知：\n老師已取消您預約的課程「${getCourseMainTitle(course.title)}」，已歸還 ${course.points_cost} 點至您的帳戶。`).catch(e => console.error(e)); // [V23.2 修改]
              }
              await client.query("DELETE FROM courses WHERE id = $1", [state.course_id]);
              await client.query('COMMIT');
              delete pendingCourseCancellation[userId];
              return reply(replyToken, `✅ 已成功取消課程「${getCourseMainTitle(course.title)}」。`); // [V23.2 修改]
            } catch (e) {
                await client.query('ROLLBACK');
                delete pendingCourseCancellation[userId];
    ...
    if (text === COMMANDS.TEACHER.COURSE_MANAGEMENT) {
        ...
        const menu = {
            ...
            contents: {
                ...
                body: { type: 'box', layout: 'vertical', spacing: 'md', paddingAll: 'lg',
                    contents: [
                        { type: 'button', style: 'secondary', height: 'sm', action: { type: 'postback', label: '✅ 確認處理訂單', data: `action=run_command&text=${encodeURIComponent(COMMANDS.TEACHER.PENDING_POINT_ORDERS)}` } },
...
async function showAvailableCourses(replyToken, userId, page) {
    ...
    try {
        ...
        const courseItems = pageCourses.map(c => {
            ...
            const mainTitle = getCourseMainTitle(c.title); // [V23.2 修改]
            return {
...
async function showMyCourses(replyToken, userId, page) {
    ...
    try {
        ...
        const courseBubbles = pageCourses.map(c => {
            ...
            const courseMainTitle = getCourseMainTitle(c.title); // [V23.2 修改]
    ...
    switch (state.type) {
        case 'book':
            if (text === COMMANDS.STUDENT.CONFIRM_BOOKING) {
                ...
                try {
                    ...
                    const historyEntry = { action: `預約課程：${getCourseMainTitle(course.title)}`, pointsChange: -course.points_cost, time: new Date().toISOString() }; // [V23.2 修改]
        ...
        case 'cancel_book':
            if (text === COMMANDS.STUDENT.CONFIRM_CANCEL_BOOKING) {
                ...
                try {
                    ...
                    const historyEntry = { action: `取消預約：${getCourseMainTitle(course.title)}`, pointsChange: +course.points_cost, time: new Date().toISOString() }; // [V23.2 修改]
                    ...
                    if (newWaiting.length > 0) {
                        ...
                        if (promotedUser) {
                             push(promotedUserId, `🎉 候補成功通知 🎉\n您候補的課程「${getCourseMainTitle(course.title)}」已有空位，已為您自動預約成功！`).catch(err => console.error(err)); // [V23.2 修改]
                        }
                    }
                    ...
                    delete pendingBookingConfirmation[userId];
                    return reply(replyToken, `✅ 已為您取消「${getCourseMainTitle(course.title)}」的預約，並歸還 ${course.points_cost} 點。`); // [V23.2 修改]
                } catch (e) {
                    await client.query('ROLLBACK');
        ...
        case 'cancel_wait':
            if (text === COMMANDS.STUDENT.CONFIRM_CANCEL_WAITING) {
                ...
                delete pendingBookingConfirmation[userId];
                return reply(replyToken, `✅ 已為您取消「${getCourseMainTitle(course.title)}」的候補。`); // [V23.2 修改]
            } else if (text === COMMANDS.GENERAL.CANCEL) {
                delete pendingBookingConfirmation[userId];
                return reply(replyToken, '已放棄取消操作。');
            }
    ...
    try {
        ...
        for (const course of upcomingCourses) {
            if (!sentReminders[course.id]) {
                const reminderMsg = `🔔 課程提醒：\n您的課程「${getCourseMainTitle(course.title)}」即將在約一小時後 (${formatDateTime(course.time)}) 開始，請準備上課！`; // [V23.2 修改]
                for (const studentId of course.students) {
...
app.listen(PORT, async () => {
  ...
  console.log(`Bot 版本: V23.2 (介面優化更新)`); // [V23.2] 更新版本號
...
async function handleEvent(event) {
    ...
    if (event.type === 'follow') {
        ...
        try {
            ...
            // 嘗試發送歡迎詞 (若額度用完會失敗，但不影響後續)
            push(userId, `歡迎 ${user.name}！感謝您加入九容瑜伽。`).catch(err => {
                console.error(`發送歡迎詞給新用戶 ${userId} 失敗:`, err.message);
            });
    ...
    if ((event.type === 'message' && (event.message.type === 'text' || event.message.type === 'image'))) {
        ...
        if (action === 'confirm_order' || action === 'reject_order') {
            ...
            try {
                ...
                if (action === 'confirm_order') {
                    ...
                    await push(order.user_id, `❗ 訂單退回通知\n您購買 ${order.points} 點的訂單已被老師退回，請確認您的匯款金額或後五碼是否正確，並至「點數查詢」重新提交。`).catch(err => {
        ...
        if (user.role === 'teacher') {
          ...
          if (action === 'set_course_weekday') {
           ...
           if (action === 'cancel_course_group_confirm') {
                ...
                const courseMainTitle = getCourseMainTitle(courseRes.rows[0].title); // [V23.2 修改]
        ...
        if (user.role === 'student') {
            ...
            if (action === 'confirm_cancel_booking_start') {
                ...
                const message = `您確定要取消預約以下課程嗎？\n\n課程：${getCourseMainTitle(course.title)}\n時間：${formatDateTime(course.time)}\n\n取消後將歸還 ${course.points_cost} 點，確認嗎？`; // [V23.2 修改]
                return reply(replyToken, message, [
            ...
            if (action === 'confirm_cancel_waiting_start') {
                ...
                const message = `您確定要取消候補以下課程嗎？\n\n課程：${getCourseMainTitle(course.title)}\n時間：${formatDateTime(course.time)}`; // [V23.2 修改]
                return reply(replyToken, message, [
